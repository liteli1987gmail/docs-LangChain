...
当您直接调用 LLM 时（在 [LangChain](/oss/python/langchain/overview) 或 LangSmith [支持的集成](/langsmith/integrations) 之外），您需要提供特定的元数据，以便 LangSmith 可以显示 token 计数、计算成本，并让您使用正确的提供商和模型在 [Playground](/langsmith/prompt-engineering-concepts#playground) 中打开[运行](/langsmith/observability-concepts#runs)。
