...
description: 了解如何使用文件附件创建数据集示例，并在使用多模态内容运行 LangSmith 评估时在提示词和评估器中使用它们。
---

LangSmith 允许您使用文件附件（如图像、音频文件或文档）创建数据集示例，并在使用多模态内容运行评估时在提示词和评估器中使用它们。

虽然您可以通过 base64 编码在示例中包含多模态数据，但这种方法效率低下——编码后的数据比原始二进制文件占用更多空间，导致到 LangSmith 的传输速度变慢。使用附件 invece 提供两个关键好处：

- 由于更高效的二进制文件传输，上传和下载速度更快。
- 在 LangSmith UI 中增强不同文件类型的可视化。

本指南介绍如何创建带附件的示例、构建使用这些附件的多模态提示词和评估器，以及使用多模态内容运行评估——选择 [**UI**](#ui) 或 [**SDK**](#sdk) following tabs to get started。
...
您可以通过几种不同的方式向数据集中的示例添加附件。

#### 从现有运行

将运行添加到 LangSmith 数据集时，可以有选择地将附件从源运行传播到目标示例。要了解更多信息，请参阅[在应用程序中管理数据集](/langsmith/manage-datasets-in-application#manually-from-a-tracing-project)。

![将带附件的追踪添加到数据集](/langsmith/images/add-trace-with-attachments-to-dataset.png)

#### 从头开始

您可以直接从 LangSmith UI 创建带附件的示例。点击数据集 UI 的 **Examples** 选项卡中的 `+ Example` 按钮。然后使用"Upload Files"按钮上传附件：

![创建带附件的示例](/langsmith/images/create-example-with-attachments.png)

上传后，您可以在 LangSmith UI 中查看带附件的示例。每个附件都将呈现预览以便轻松检查。![带示例的附件](/langsmith/images/attachments-with-examples.png)

## 2. 创建一个多模态提示词

LangSmith UI 允许您在评估多模态模型时在提示词中包含附件：

首先，点击您想要添加多模态内容的消息中的文件图标。接下来，为每个示例要包含的附件添加模板变量。

- 如果您想包含一个特定的附件，您可以使用建议的变量名，如 `{{attachment.file_name}}`，这会将附件中具有 `file_name` 的文件映射并传递给评估器
...
您可以创建使用数据示例中多模态内容的评估器。

由于您的数据集已经包含带附件的示例（已在步骤 1 中添加），您可以直接在评估器中引用它们。为此：
...
然后评估器可以将这些附件与模型输出一起用于判断质量。例如，您可以创建一个评估器来：

- 检查图像描述是否与实际图像内容匹配。
- 验证转录是否准确反映音频。
- 验证从 PDF 提取的文本是否正确。

您还可以创建不使用附件的纯文本评估器，而是评估模型的文本输出：

- OCR → 文本更正：使用视觉模型从文档中提取文本，然后评估提取输出的准确性。
- 语音转文本 → 转录质量：使用语音模型将音频转录为文本，然后根据您的参考评估转录质量。

<Tip>
如果您的追踪在输入或输出中包含 base64 编码的多模态内容（例如，如果您按照[记录多模态追踪](/langsmith/log-multimodal-traces)指南），您不需要附件来评估它们。在评估器提示词中使用标准变量映射——如 `{{input}}` 或 `{{output}}`——base64 内容将正确传递给 LLM 评估器进行可视化和评估。
</Tip>

有关定义自定义评估器的更多信息，请参阅 [LLM as Judge](/langsmith/llm-as-judge) 指南。
...
您可以使用 `uploadExamplesMultipart` 方法上传带附件的示例。

请注意，这与目前不支持附件的标准 `createExamples` 方法不同。每个附件都需要 `Uint8Array` 或 `ArrayBuffer` 作为数据类型。

* `Uint8Array`：用于直接处理二进制数据。
* `ArrayBuffer`：表示固定长度的二进制数据，可以根据需要转换为 `Uint8Array`。

请注意，您不能在 TypeScript SDK 中直接传入文件路径，因为在所有运行时环境中都不支持访问本地文件。
...
<Info>
除了作为字节传入外，附件还可以指定为本地文件的路径。为此，请为附件 `data` 值传入一个路径并指定参数 `dangerously_allow_filesystem=True`：
...
现在我们有一个包含带附件示例的数据集，我们可以定义一个目标函数来在这些示例上运行。以下示例简单地使用 OpenAI 的 GPT-4o 模型来回答有关图像和音频剪辑的问题。

#### Python

您正在评估的目标函数必须有两个位置参数才能使用与示例关联的附件，第一个必须称为 `inputs`，第二个必须称为 `attachments`。

* `inputs` 参数是一个字典，包含示例的输入数据，不包括附件。
* `attachments` 参数是一个字典，将附件名称映射到包含预签名 url、mime_type 和文件内容的读取器的字典。您可以使用预签名 url 或读取器来获取文件内容。attachments 字典中的每个值都是一个具有以下结构的字典：
...
`config` 将包含 `attachments`，这是一个将附件名称映射到以下形式对象的对象：
...
<Note>您也可以在 UI 中定义一个引用这些附件输入和输出的多模态评估器。基于 UI 的评估器在每个实验上自动运行——包括从 SDK 调用时运行。有关说明，请参阅 [**UI**](#ui) 选项卡。</Note>

与上述相同的规则适用于确定评估器是否应该接收附件。

下面的评估器使用 LLM 来判断推理和答案是否一致。要了解有关如何定义基于 LLM 的评估器的更多信息，请参阅[如何定义 LLM-as-a-judge 评估器](/langsmith/llm-as-judge)。
...
与现有示例一样，当您使用附件更新它们时，数据集会被版本控制。因此，您可以导航到数据集版本历史记录以查看对每个示例所做的更改。要了解更多信息，请参阅[在 UI 中创建和管理数据集](/langsmith/manage-datasets-in-application)。
...
* 如果您传入不存在的附件名称到 `retain` 或 `rename`，将引发错误。
* 新附件优先于 `attachments` 和 `attachment_operations` 字段中出现的相同附件名称的现有附件。
