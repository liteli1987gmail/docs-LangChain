...
[LangSmith](https://smith.langchain.com/) 为不断增长的流行 [LLM 提供商](#llm-提供商) 和[代理框架](#代理框架)以及 [Deep Agents](/oss/python/deepagents/overview)、[LangChain](/oss/python/langchain/overview) 和 [LangGraph](/oss/python/langgraph/overview) 提供集成。有关设置和使用，请参阅本页列出的指南。
...
**使用 LangChain？** LangChain 提供了统一的接口来访问 100+ 个 LLM 提供商，允许您通过设置环境变量来切换模型。[初始化模型](/oss/python/langchain/models#initialize-a-model)，LangSmith 将自动追踪您的应用程序。
</Callout>
