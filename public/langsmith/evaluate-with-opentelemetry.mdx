---
title: 如何使用 OpenTelemetry 运行评估
sidebarTitle: 使用 OpenTelemetry
---

本指南向您展示如何使用 OpenTelemetry 追踪和 LangSmith 运行评估。
...
如果您已经在使用 OpenTelemetry 追踪您的 LLM 应用程序，您可以通过将追踪路由到实验会话来运行评估。当您想要评估使用 OpenTelemetry 检测但不使用 LangSmith SDK 的 [`evaluate()`](https://reference.langchain.com/python/langsmith/client/Client/evaluate) 函数的应用程序时，这种方法很有用。
...
4. 对数据集中的每个示例运行您的应用程序。

## 先决条件

本指南假设您具备：

* 一个使用 OpenTelemetry 检测的应用程序，该应用程序将追踪发送到 LangSmith。
* 在 LangSmith 中创建了一个包含要评估示例的数据集。您可以通过 [LangSmith UI](/langsmith/evaluation-concepts#datasets) 或通过 [SDK](/langsmith/manage-datasets-programmatically) 创建数据集。
...
本指南假设已在 LangSmith 中创建一个包含要评估示例的数据集。您可以通过 [LangSmith UI](/langsmith/evaluation-concepts#datasets) 或通过 [SDK](/langsmith/manage-datasets-programmatically) 创建数据集。

实验会话将所有评估追踪组合在一起。使用 LangSmith 客户端创建一个：
...
此外，您可以在 LangSmith UI 中创建评估器并将它们绑定到您的数据集。对于在 UI 中定义并绑定到您的数据集的评估器，它们将自动在实验追踪上运行。
...
首先，您需要一个使用 OpenTelemetry 进行追踪的应用程序。此示例使用 Strands 代理，但您可以使用任何 OpenTelemetry 检测的应用程序。通过在 OTEL 头中包含实验 ID 来设置 OpenTelemetry 以将追踪路由到您的实验会话。在这一步中的一般想法是让一个代理或应用程序已经用 OpenTelemetry 检测了。

<Note>
由于 `Strands TypeScript SDK` 目前不支持 `OpenTelemetry` 可观测性（截至 2026 年 2 月），因此未为此步骤提供 TypeScript 示例。</Note>
...
为每个应用程序运行添加必需的 span 属性。这些属性将每个追踪链接到实验和特定的数据集示例。
...
导航到 [LangSmith UI](https://smith.langchain.com) 中的您的实验以分析结果。
