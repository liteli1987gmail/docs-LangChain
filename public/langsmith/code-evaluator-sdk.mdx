...
代码评估器是接受数据集示例和生成的应用程序输出并返回一个或多个指标的函数。这些函数可以直接传入 [`evaluate()`](https://reference.langchain.com/python/langsmith/client/Client/evaluate) 或 [`aevaluate()`](https://reference.langchain.com/python/langsmith/client/Client/aevaluate) 函数。
...
* `run: Run`：由应用程序在给定示例上生成的完整[运行](/langsmith/run-data-format)对象。
* `example: Example`：完整的数据集[示例](/langsmith/example-data-format)，包括示例输入、输出（如果有）和元数据（如果有）。
* `inputs: dict`：对应数据集中单个示例的输入字典。
* `outputs: dict`：应用程序在给定 `inputs` 上生成的输出字典。
* `reference_outputs/referenceOutputs: dict`：与示例关联的参考输出字典（如果有）。

对于大多数用例，您只需要 `inputs`、`outputs` 和 `reference_outputs`。只有在需要一些超出应用程序实际输入和输出的额外追踪或示例元数据时，`run` 和 `example` 才有用。

使用 JS/TS 时，这些都应作为单个对象参数的一部分传入。
...
* `dict`：形式为 `{"score" | "value": ..., "key": ...}` 的字典允许您自定义指标类型（"score" 用于数值，"value" 用于分类）和指标名称。这在例如您想将整数记录为分类指标时很有用。

仅 Python

* `int | float | bool`：这被解释为可以平均、排序等的连续指标。函数名用作指标名称。
...
* [评估聚合实验结果](/langsmith/summary)：定义汇总评估器，计算整个实验的指标。
