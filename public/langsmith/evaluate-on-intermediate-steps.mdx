...
虽然在许多场景下，评估任务的最终输出就足够了，但在某些情况下，您可能希望评估管道的中间步骤。

例如，对于检索增强生成（RAG），您可能希望：

1. 评估检索步骤以确保检索到相对于输入查询的正确文档。
2. 评估生成步骤以确保根据检索到的文档生成了正确的答案。

在本指南中，我们将使用一个简单的自定义评估器来评估第一个标准，并使用一个基于 LLM 的评估器来评估第二个标准，以突出两种场景。

为了评估管道的中间步骤，您的评估器函数应该遍历和处理 `run`/`rootRun` 参数，这是一个包含管道中间步骤的 `Run` 对象。

## 1. 定义您的 LLM 管道

下面的 RAG 管道包括：1）根据输入问题生成 Wikipedia 查询，2）从 Wikipedia 检索相关文档，以及 3）根据检索到的文档生成答案。
...
此管道将产生类似于以下内容的追踪：![evaluation_intermediate_trace.png](/langsmith/images/evaluation-intermediate-trace.png)

## 2. 创建数据集和示例以评估管道

我们正在构建一个包含几个示例的非常简单的数据集来评估管道。
...
如上所述，我们将定义两个评估器：一个评估检索文档相对于输入查询的相关性，另一个评估生成的答案相对于检索文档的幻觉。我们将使用 LangChain LLM 包装器以及 [`with_structured_output`](https://reference.langchain.com/python/langchain-core/language_models/chat_models/BaseChatModel/with_structured_output) 来定义幻觉的评估器。

关键在于评估器函数应该遍历 `run`/`rootRun` 参数以访问管道的中间步骤。然后评估器可以处理中间步骤的输入和输出，以根据所需的标准进行评估。

示例使用 `langchain` 以方便，但这不是必需的。
...
实验将包含评估的结果，包括评估器的分数和评论：![evaluation_intermediate_experiment.png](/langsmith/images/evaluation-intermediate-experiment.png)
