...
---
在 LangSmith Deployment 中，线程并未明确关联到特定的代理。
这意味着您可以在同一线程上运行多个代理，这允许不同的代理从初始代理的进度继续。

在这个示例中，我们将创建两个代理，然后在同一线程上调用它们。
您将看到第二个代理将使用第一个代理在线程中生成的[检查点](/oss/python/langgraph/persistence#checkpoints)作为上下文来响应。
...
现在，我们可以在默认助手上运行它，并看到第二个助手知道初始问题，并且可以回答"你呢？"这个问题：
