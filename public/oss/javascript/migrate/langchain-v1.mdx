...
This migration guide outlines the major changes in LangChain v1. To learn more about the new features of v1, 请参阅 the [introductory post](/oss/javascript/releases/langchain-v1).
...
In v1, the react agent prebuilt is now in the langchain 包. 该 table below outlines what functionality has changed:

| Section | What changed |
|---------|--------------|
| [Import path](#import-path) | 包 moved from `@langchain/langgraph/prebuilts` to `langchain` |
...
| [结构化输出](#structured-output) | prompted output removed, use `toolStrategy`/`providerStrategy` |
| [Streaming node name](#streaming-node-name-rename) | 否de name changed from `"agent"` to `"model"` |
| [Runtime context](#runtime-context) | `context` property instead of `config.configurable` |
| [Namespace](#simplified-包) | Streamlined to focus on agent building blocks, 旧版 code moved to `@langchain/classic` |

### Import path

该 import path for the react agent prebuilt has changed from `@langchain/langgraph/prebuilts` to `langchain`. 该 name of the function has changed from `createReactAgent` to `createAgent`:
...
该 `prompt` parameter has been renamed to `systemPrompt`:
...
Custom state is now defined in middleware using the `stateSchema` property. 使用 Zod to declare additional state fields that are carried through the agent run.
...
Dynamic model selection now happens via middleware. 使用 `wrapModelCall` to swap models (and tools) based on state or runtime context. In `createReactAgent`, this was done via a function passed to the `model` parameter.
...
该 `tools` argument to `createAgent` accepts:
...
结构化输出 used to be generated in a separate node from the main agent. This is no longer the case. 结构化输出 is generated in the main loop (no extra LLM call), reducing cost and latency.
...
<否te>
    该 old `config.configurable` pattern still works for backward compatibility, but using the new `context` parameter is recommended for new applications or applications migrating to v1.
</否te>
...
In v1, messages gain provider-agnostic standard content blocks. Access them via `message.contentBlocks` for a consistent, typed view across providers. 该 existing `message.content` field remains unchanged for strings or provider-native structures.

### What changed

- New `contentBlocks` property on messages for normalized content.
- New TypeScript types under `ContentBlock` for strong typing.
- 可选 serialization of standard blocks into `content` via `LC_OUTPUT_VERSION=v1` or `output版本: "v1"`.
...
请参阅 the content blocks [reference](/oss/javascript/langchain/messages#content-block-reference) for more details.
...
<否te>
    Learn more: [Messages](/oss/javascript/langchain/messages#message-content) and [Standard content blocks](/oss/javascript/langchain/messages#standard-content-blocks). 请参阅 [Multimodal](/oss/javascript/langchain/messages#multimodal) for input 示例s.
</否te>
...
该 `langchain` 包 namespace is streamlined to focus on agent building blocks. Legacy functionality has moved to `@langchain/classic`. 该 new 包 exposes only the most useful and relevant functionality.

### Exports

该 v1 包 includes:

| Module | What's available | 否tes |
|--------|------------------|-------|
...
If you use 旧版 chains, the indexing API, or functionality previously re-exported from `@langchain/community`, install `@langchain/classic` and update imports:
...
Builds for all langchain 包s now use a bundler based approach instead of using raw typescript outputs. If you were importing files from the `dist/` directory (which is not recommended), you will need to update your imports to use the new module system.
...
该 following deprecated APIs have been removed in v1:

#### Core functionality
- `TraceGroup` - 使用 LangSmith tracing instead
- `BaseDocumentLoader.loadAndSplit` - 使用 `.load()` followed by a text splitter
- `RemoteRunnable` - 否 longer supported

#### Prompts
- `BasePromptTemplate.serialize` and `.deserialize` - 使用 JSON serialization directly
- `ChatPromptTemplate.fromPromptMessages` - 使用 `ChatPromptTemplate.fromMessages`

#### Retrievers
- `BaseRetrieverInterface.getRelevantDocuments` - 使用 `.invoke()` instead

#### Runnables
- `Runnable.bind` - 使用 `.bindTools()` or other specific binding methods
- `Runnable.map` - 使用 `.batch()` instead
- `RunnableBatchOptions.maxConcurrency` - 使用 `maxConcurrency` in the config object

#### Chat models
- `BaseChatModel.predictMessages` - 使用 `.invoke()` instead
- `BaseChatModel.predict` - 使用 `.invoke()` instead
- `BaseChatModel.serialize` - 使用 JSON serialization directly
- `BaseChatModel.callPrompt` - 使用 `.invoke()` instead
- `BaseChatModel.call` - 使用 `.invoke()` instead

#### LLMs
- `BaseLLMParams.concurrency` - 使用 `maxConcurrency` in the config object
- `BaseLLM.call` - 使用 `.invoke()` instead
- `BaseLLM.predict` - 使用 `.invoke()` instead
- `BaseLLM.predictMessages` - 使用 `.invoke()` instead
- `BaseLLM.serialize` - 使用 JSON serialization directly

#### Streaming
- `createChatMessageChunkEncoderStream` - 使用 `.stream()` method directly
...
#### Memory and storage
- `BaseListChatMessageHistory.addAIChatMessage` - 使用 `.addMessage()` with `AIMessage`
- `BaseStoreInterface` - 使用 specific store implementations
