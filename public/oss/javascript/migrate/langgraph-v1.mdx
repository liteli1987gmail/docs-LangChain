...
This guide outlines changes in LangGraph v1 and how to migrate from previous versions. For a high-level overview of what's new, 请参阅 the [release notes](/oss/javascript/releases/langgraph-v1).
...
| `toLangGraphEventStream` removed | 使用 `graph.stream` with the desired `encoding` format |
...
LangGraph v1 deprecates the `createReactAgent` prebuilt. 使用 LangChain's `createAgent`, which runs on LangGraph and adds a flexible middleware system.

请参阅 the LangChain v1 docs for details:
...
请参阅 [Interrupts](/oss/javascript/langgraph/interrupts) to learn more.

---

## Event stream encoding

该 low-level `toLangGraphEventStream` helper is removed. Streaming responses are handled by the SDK; when using low-level clients, select the wire format via an `encoding` option passed to `graph.stream`.
...
Builds for all langgraph 包s now use a bundler based approach instead of using raw typescript outputs. If you were importing files from the `dist/` directory (which is not recommended), you will need to update your imports to use the new module system.
