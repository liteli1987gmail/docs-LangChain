...
To use SAP HANA Knowledge Graph Engine with LangChain, 安装 `@sap/hana-langchain` 包 along with its peer dependencies:
...
* **One of**:
  ...
  4. **`autoExtractOntology: true`** (not recommended for production—请参阅 note)

`graphUri` vs. Ontology

* **`graphUri`**:
  该 named graph in your SAP HANA Cloud instance that contains your instance data (sometimes 100k+ triples).
...
使用 a custom `CONSTRUCT` query to selectively extract schema triples.
...
Supported RDF formats: `Turtle`, `N-Triples`, `否tation-3`, `Trig`, `N-Quads`.
...
> **否te**: Auto-extraction is **not** recommended for production—it omits important triples like `rdfs:label`, `rdfs:comment`, and `rdfs:sub类Of` in general.

## Executing SPARQL Queries

You can use the `query()` method to execute arbitrary SPARQL queries (`SELECT`, `ASK`, `CONSTRUCT`, etc.) on the data graph.

该 function has the following parameters
...
> **否te**: CONSTRUCT and ASK Queries return `turtle` and `boolean` formats respectively.
...
该n, we create a graph instance for the `Puppets` graph.
...
该 given query lists all tuples in the `Puppets` graph.
