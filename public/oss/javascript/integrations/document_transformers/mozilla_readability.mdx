...
在摄入 HTML 文档以供后续检索时, 我们通常只对网页的实际内容感兴趣，而不是语义.
Stripping HTML tags from documents with the MozillaReadabilityTransformer can result in more content-rich chunks, 使检索更有效.
...
该 below 示例 scrapes a Hacker News thread, 根据 HTML 标签将其拆分，基于标签的语义信息对内容块进行分组,
然后从各个内容块中提取内容:
