...
- Get an OpenAI api key and 将其设置为环境变量 (`OPENAI_API_KEY`)

## Chat model

请参阅 a [usage 示例](/oss/javascript/集成s/chat/openai).
...
请参阅 a [usage 示例](/oss/javascript/集成s/llms/openai).
...
请参阅 a [usage 示例](/oss/javascript/集成s/embeddings/openai)
...
<ParamField body="model" type="string | BaseChatModel" required>
    OpenAI model to use for moderation. Can be either a model name string (e.g., `"openai:gpt-5.4"`) or a `BaseChatModel` instance. 该 middleware will use this model's client to access the moderation endpoint.
</ParamField>
...
<ParamField body="violationMessage" type="string | undefined">
    ...
    - `{original_content}` - 该 original flagged content
...
该 middleware integrates OpenAI's moderation endpoint to check content at different stages:

**Moderation stages:**
- `checkInput` - 使用r messages before model call
