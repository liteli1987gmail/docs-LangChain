...
这将帮助您开始使用 IBM watsonx.ai [聊天模型](/oss/javascript/langchain/models)。有关所有 `IBM watsonx.ai` 功能和配置的详细文档，请前往 [IBM watsonx.ai](https://reference.langchain.com/javascript/langchain-community/chat_models/ibm/ChatWatsonx)。
...
- 您必须提供 `spaceId`、`projectId` 或 `idOrName`（部署 ID），除非您使用轻量级引擎（无需指定任何一个即可工作，参阅 [watsonx.ai 文档](https://www.ibm.com/docs/en/cloud-paks/cp-data/5.0.x?topic=install-choosing-installation-mode)）
- 根据配置的服务实例的区域，使用正确的 serviceUrl。
...
要使用模型网关与 Langchain，您需要先创建一个提供商并通过 `@ibm-cloud/watsonx-ai` SDK 或 `watsonx.ai` 接口添加模型。请参阅此文档：
- [接口](https://cloud.ibm.com/apidocs/watsonx-ai#create-watsonxai-provider)。
...
有关所有 `IBM watsonx.ai` 功能和配置的详细文档，请前往 API 参考：[API 文档](https://reference.langchain.com/javascript/langchain-community/chat_models/ibm/ChatWatsonx)
