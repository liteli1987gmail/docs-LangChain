...
Anthropic 模型与大多数其他模型的一个关键区别在于，单个 Anthropic [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage) 的内容可以是单个字符串或**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/javascript/langchain/tools)时，工具调用是消息内容的一部分（同时也暴露在标准化的 `AIMessage.tool_calls` 字段中）：
...
以下是在包含 LangChain [概念文档](/oss/javascript/concepts/)的系统消息中缓存部分的示例：
...
[详细了解工具调用](/oss/javascript/langchain/tools)
