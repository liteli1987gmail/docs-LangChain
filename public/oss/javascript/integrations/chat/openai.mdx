...
<Note>
    **Chat Completions API 兼容性**
    ...
    `ChatOpenAI` 与 OpenAI 的（旧版）[Chat Completions API](https://platform.openai.com/docs/guides/completions) 完全兼容。如果您想连接支持 Chat Completions API 的其他模型提供商，可以这样做——请参阅[说明](/oss/javascript/integrations/chat#chat-completions-api)。
</Note>
...
某些代理或第三方提供商提供的 API 接口与 OpenAI 基本相同，但不支持最近添加的 `stream_options` 参数来返回流式使用量。您可以通过禁用流式使用量来使用 [`ChatOpenAI`](https://reference.langchain.com/javascript/langchain-openai/ChatOpenAI) 访问这些提供商，如下所示：
...
如果您使用的模型支持自定义工具，您可以使用 [`ChatOpenAI`](https://reference.langchain.com/javascript/langchain-openai/ChatOpenAI) 类和 `customTool` 函数来创建自定义工具。
...
请注意，响应包含结构化的[内容块](/oss/javascript/langchain/messages/#message-content)，其中包括响应的文本和 OpenAI [注释](https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses#output-and-citations)（引用其来源）。输出消息还将包含来自任何工具调用的信息。
