...
本笔记本介绍如何使用 [`SitemapLoader`](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/sitemap/SitemapLoader) 类将 sitemap 加载到 `Document` 中。
...
传入的 URL 必须包含 sitemap 的 `.xml` 路径，否则将附加默认的 `/sitemap.xml`。
...
或者，如果您只想加载 sitemap 而不想加载 sitemap 中每个页面的内容，可以使用 `parseSitemap` 方法：
