...
This guide demonstrates how to use `OracleVS` (the LangChain vector store 集成 for Oracle AI Vector Search) to:
...
该 示例 below assumes that you already created a table with vector and metadata columns and generated embeddings using [`Oracle嵌入`](/oss/javascript/集成s/embeddings/oracleai).
...
If your application already manages an Oracle Database connection pool, pass the pool directly to `OracleVS`. 该 store acquires and releases connections as needed.
...
Nested logical clauses are also supported. For 示例:
...
Oracle Database can speed up similarity queries by creating vector indexes on the `embedding` column. 该 `@oracle/langchain-oracledb` helpers expose a `createIndex` utility that provisions either HNSW (default) or IVF indexes.

### HNSW index (default)

使用 HNSW when you want a graph-based index that balances recall and latency. Omit `idxType` to use the default configuration or override parameters such as `neighbors`, `efConstruction`, and `accuracy`.
...
- Load content with [`OracleDocLoader`](/oss/javascript/集成s/document_loaders/file_loaders/oracleai)
- Generate embeddings with [`Oracle嵌入`](/oss/javascript/集成s/embeddings/oracleai)
- Summarize documents using [`OracleSummary`](/oss/javascript/集成s/tools/oracleai)
