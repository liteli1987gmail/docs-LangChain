...
Once you have your instance running, make sure you have the endpoint and the admin key (query keys can be used only to search document, not to index, update or delete). 该 endpoint is the URL of your instance which you can find in the Azure Portal, under the "概述" section of your instance. 该 admin key can be found under the "Keys" section of your instance. 该n you need to set the following environment variables:
...
## Example: index docs, vector search and LLM 集成

Below is an 示例 that indexes documents from a file in Azure AI Search, runs a hybrid search query, and finally uses a chain to answer a question in natural language based on the retrieved documents.
