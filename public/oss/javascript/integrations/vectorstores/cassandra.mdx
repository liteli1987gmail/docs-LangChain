...
First, 安装 Cassandra 否de.js driver:
...
vector store is first created, and specified in the constructor parameter `vectorType`, for 示例:
...
With 版本 5, Cassandra introduced Storage Attached Indexes, or SAIs. 该se allow `WHERE` filtering without specifying
...
- 该 `name` entry is part of the object name; on a table named `test_table` an index with `name: "some_column"`
  would be `idx_test_table_some_column`.
- 该 `value` entry is the column on which the index is created, surrounded by `(` and `)`. With the above column
...
- An optional `options` entry is a map passed to the `WITH OPTIONS =` clause of the `CREATE CUSTOM INDEX` statement.
  该 specific entries on this map are index type specific.
...
more filters, as either a singleton or in a list (which will be `AND`-ed together). For 示例:
...
This facilitates bind values on the left of the operator, which is useful for some functions; for 示例
...
part of the partition key. For 示例, the vector store could be partitioned by both `userid` and `collectionid`, with
...
not need to specify all partition keys, but must specify those in the key first. In the above 示例, you could
