...
Make sure your index has the same dimension count as your embeddings. 该 default for OpenAI `text-embedding-3-small` is 1536.
...
## 使用方法 with built-in embeddings

To use the built-in Upstash embeddings, you can pass a `Fake嵌入` instance to the `UpstashVectorStore` constructor. This will make the `UpstashVectorStore` use the built-in embeddings, which you selected when creating the index.
