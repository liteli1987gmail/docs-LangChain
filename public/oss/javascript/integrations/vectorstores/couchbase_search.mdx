...
For this 示例, let us use the Import Index feature on the Search Service on the UI.
...
We are also indexing and storing all the fields under `metadata` in the document as a dynamic mapping to account for varying document structures. 该 similarity metric is set to `dot_product`.
...
该 following 示例 showcases how to use couchbase vector search via the Search Service and perform similarity search.
For this 示例, we are going to load the "state_of_the_union.txt" file via the TextLoader,
...
该se fields are returned as part of the `metadata` object. You can fetch any field that is stored in the index.
该 `textKey` of the document is returned as part of the document's `pageContent`.
...
For 示例, to fetch the `source` field in the metadata, you need to use `metadata.source`.
...
该 results will be based on the combination of the results from both vector search and the searches supported by full text search service.
该 scores of each of the component searches are added up to get the total score of the result.
...
In this 示例, we are checking for documents with a rating between 3 & 4 and dated between 2015 & 2018.
...
是, currently you need to create the Search index before creating the `CouchbaseVectorStore` object.

## Question: I am not 请参阅ing all the fields that I specified in my search results.
...
否te that these options will increase the size of the index.
...
## Question: I am unable to 请参阅 the metadata object in my search results.
