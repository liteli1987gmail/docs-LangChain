---
title: "Azure Cosmos DB for 否SQL 集成"
description: "Integrate with the Azure Cosmos DB for 否SQL vector store using LangChain JavaScript."
---
...
Once you have your instance running, make sure you have the connection string. You can find them in the Azure Portal, under the "Settings / Keys" section of your instance. 该n you need to set the following environment variables:
...
否w, if the attacker enters `123 OR 1=1`, the input will be treated as a literal string value to match, and not as part of the query structure.
...
Below is an 示例 that indexes documents from a file in Azure Cosmos DB for 否SQL, runs a vector search query, and finally uses a chain to answer a question in natural language
...
All search types are accessed through the unified `similaritySearch` and `similaritySearchWithScore` methods using the `searchType` parameter in the filter options. Search types are available as constants from `AzureCosmosDB否SQLSearchType`:

可用 search types:

- `AzureCosmosDB否SQLSearchType.Vector` (default): Standard vector similarity search
- `AzureCosmosDB否SQLSearchType.VectorScoreThreshold`: Vector search with minimum score filter
- `AzureCosmosDB否SQLSearchType.FullTextSearch`: Full-text search using FullTextContains (preview)
- `AzureCosmosDB否SQLSearchType.FullTextRanking`: Full-text search with BM25 ranking (preview)
- `AzureCosmosDB否SQLSearchType.Hybrid`: Hybrid vector + full-text search using RRF (preview)
- `AzureCosmosDB否SQLSearchType.HybridScoreThreshold`: Hybrid search with score threshold (preview)
