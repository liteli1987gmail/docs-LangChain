...
This guide provides a quick overview for getting started with `MemoryVectorStore` [vector stores](/oss/javascript/集成s/vectorstores).
...
To use in-memory vector stores, you'll need to 安装 `langchain` 包:
...
该re are no required credentials to use in-memory vector stores.
...
该 filter is optional, and must be a predicate function that takes a document as input, and returns `true` or `false` depending on whether the document should be returned.
