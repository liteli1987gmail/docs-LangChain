...
- **Query and Index Service**: 使用s Couchbase's Query service with SQL++ instead of the Search service
- **否 Index 必填**: Does not require a pre-configured search index for basic operations
- **SQL++ Syntax**: Supports WHERE clauses and SQL++ query syntax for filtering
- **Vector Functions**: 使用s `APPROX_VECTOR_DISTANCE` function for similarity calculations
...
该 Query vector store supports creating vector indexes to improve search performance. 该re are two types of indexes available:
...
| **使用 Case** | Pure vector similarity search | Mixed vector and scalar queries |
...
## Basic vector search 示例

该 following 示例 showcases how to use Couchbase Query vector search and perform similarity search.
...
1. **Create Indexes**: 使用 `createIndex()` to create appropriate vector indexes for better performance
2. **Choose Index Type**:
   - 使用 **Hyperscale indexes** for pure vector search workloads where you primarily perform similarity searches
   - 使用 **Composite indexes** for mixed queries that combine vector similarity with scalar field filtering
3. **Tune Parameters**: Adjust `indexScanNprobes` and `indexTrainlist` based on your data size and performance requirements
4. **Filter Early**: 使用 WHERE clauses to reduce the search space before vector calculations
...
If you 请参阅 errors related to insufficient training data, you may need to:
...
否, unlike the Search-based `CouchbaseSearchVectorStore`, the Query-based implementation can work without pre-created indexes. However, creating appropriate vector indexes (Hyperscale or Composite) will significantly improve query performance.

### When should I use hyperscale vs. composite indexes?

- 使用 **Hyperscale indexes** when you primarily perform vector similarity searches with minimal filtering on other fields
- 使用 **Composite indexes** when you frequently combine vector similarity with filtering on scalar fields in the same query
...
是, both can work on the same document structure. However, they use different services (Search vs Query) and have different indexing requirements.

## 相关内容

- Vector store [概念指南](/oss/javascript/集成s/vectorstores)
- Vector store [操作指南](/oss/javascript/集成s/vectorstores)
