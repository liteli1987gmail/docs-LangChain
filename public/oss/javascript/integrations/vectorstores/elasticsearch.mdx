...
To use Elasticsearch vector stores, you'll need to 安装 `@langchain/community` 集成 包.
...
该re are three ways to get started with Elasticsearch:
...
该 `.env` file contains `ES_LOCAL_URL` and `ES_LOCAL_API_KEY` that you can use to connect:
...
该 `ElasticVectorSearch` class provides Elasticsearch as a vector store with support for both standard vector search and hybrid search.
