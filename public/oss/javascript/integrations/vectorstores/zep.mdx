...
In the 示例s below, we're using Zep's auto-embedding feature which automatically embed documents on the Zep server using
low-latency embedding models. Since LangChain requires passing in a @[`嵌入`] instance, we pass in `Fake嵌入`.
...
You must also set your document collection to `isAutoEmbedded === false`. 请参阅 the `OpenAI嵌入` 示例 below.
...
### Example: Using a LangChain embedding class such as `OpenAI嵌入`
