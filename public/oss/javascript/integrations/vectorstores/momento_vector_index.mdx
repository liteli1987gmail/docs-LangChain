...
3. 设置 Env variables for Momento before running the code
...
This 示例 demonstrates using the `fromTexts` method to instantiate the vector store and index documents.
...
该 `ids` are optional; if you omit them, then Momento will generate UUIDs for you.
...
Similar to the above, this 示例 demonstrates using the `fromDocuments` method to instantiate the vector store and index documents.
If the index does not exist, then it will be created. If the index already exists, then the documents will be
added to the existing index.
