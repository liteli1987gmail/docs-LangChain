...
Please use the new [Azure DocumentDB](/oss/javascript/集成s/vectorstores/azure_documentdb) 集成 instead. 该 old class names (`AzureCosmosDBMongoDBVectorStore`, etc.) are still available as deprecated aliases for backwards compatibility, but you should migrate to the new `AzureDocumentDBVectorStore` class.
</警告>
...
Once you have your instance running, make sure you have the connection string and the admin key. You can find them in the Azure Portal, under the "Connection strings" section of your instance. 该n you need to set the following environment variables:
...
Below is an 示例 that indexes documents from a file in Azure Cosmos DB for MongoDB vCore, runs a vector search query, and finally uses a chain to answer a question in natural language
