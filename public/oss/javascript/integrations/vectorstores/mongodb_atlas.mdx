...
To use MongoDB Atlas vector stores, you'll need to configure a MongoDB Atlas cluster and 安装 `@langchain/mongodb` 集成 包.
...
In **manual embedding mode**, you provide an embeddings model and embed documents client-side. This guide uses [OpenAI embeddings](/oss/javascript/集成s/embeddings/openai) as an 示例. You can also use [other supported embeddings models](/oss/javascript/集成s/embeddings).

In **automated embedding mode**, MongoDB Atlas handles embedding generation server-side. 否 client-side embeddings 包 is required.

### 安装

<Tabs>
  <Tab title="Manual embedding">
    Install the core 包 plus an embeddings provider:
  ...
  <Tab title="Automated embedding">
    Only the core 包 and MongoDB driver are required:
...
Once you've set up your cluster and index, initialize your vector store. 该 constructor accepts two forms depending on whether you use manual or automated embedding.
...
MongoDB Atlas supports pre-filtering of results on other fields. 该y require you to define which metadata fields you plan to filter on by updating the index you created initially. 以下是 an 示例:
...
Above, the first item in `fields` is the vector index, and the second item is the metadata property you want to filter on. 该 name of the property is the value of the `path` key. 该refore the above index allows us to search on a metadata field named `source`.
