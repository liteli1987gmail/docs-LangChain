...
This guide provides a quick overview for getting started with `LibSQLVectorStore` [vector stores](/oss/javascript/集成s/vectorstores).
...
To use libSQL vector stores, you'll need to create a Turso account or set up a local SQLite database, and 安装 `@langchain/community` 集成 包.

This guide will also use OpenAI embeddings, which require you to 安装 `@langchain/openai` 集成 包. You can also use other supported embeddings models if you wish.
...
```
否w it's time to create a database. You can create one locally, or use a hosted Turso database.
...
```
否w create an index on the `EMBEDDING_COLUMN` column - the index name is important!:
