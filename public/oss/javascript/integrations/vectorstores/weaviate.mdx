...
To use Weaviate vector stores, you'll need to set up a Weaviate instance and 安装 `@langchain/weaviate` 集成 包. You should also 安装 `weaviate-client` 包 to initialize a client to connect to your instance with, and the `uuid` 包 if you want to assign indexed documents ids.
...
To use Weaviate's named vectors, vectorizers, reranker, generative-models etc., use the `schema` property when enabling the vector store. 该 collection name and other properties in `schema` will take precedence when creating the vector store.
...
In weaviate's v3, the client interacts with `collections` as the primary way to work with objects in the database. 该 `collection` object can be reused throughout the codebase

### Query directly

Performing a simple similarity search can be done as follows. 该 `Filter` helper class makes it easier to use filters with conditions. 该 v3 client streamlines how you use `Filter` so your code is cleaner and more concise.
...
- @param query 该 query to search for.
