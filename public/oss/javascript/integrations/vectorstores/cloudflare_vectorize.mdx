...
Finally, you'll need to 安装 LangChain Cloudflare 集成 包:
...
Below is an 示例 worker that adds documents to a vectorstore, queries it, or clears it depending on the path used. It also uses [Cloudflare Workers AI 嵌入](/oss/javascript/集成s/embeddings/cloudflare_ai).
