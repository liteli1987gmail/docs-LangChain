...
In order to use this 包, you first need to go through the following steps:
...
To use the PostgresVectorStore library, you'll need to 安装 `@langchain/google-cloud-sql-pg` 包 and then follow the steps below.
...
该 Maximal marginal relevance optimizes for similarity to the query and diversity among selected documents.
