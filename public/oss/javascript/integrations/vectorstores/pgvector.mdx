...
This guide will also use [OpenAI embeddings](/oss/javascript/集成s/embeddings/openai), which require you to 安装 `@langchain/openai` 集成 包. You can also use [other supported embeddings models](/oss/javascript/集成s/embeddings) if you wish.
...
该re are many ways to connect to Postgres depending on how you've set up your instance. 以下是 one 示例 of a local setup using a prebuilt Docker image provided by the `pgvector` team.
...
该 above filter syntax supports exact match, but the following are also supported:
...
否te that you should call `.initialize()` to set up your database at least once to set up your tables properly before using the constructor.
...
该 method parameters include:

- `dimensions`: Defines the number of dimensions in your vector data type, up to 2000. For 示例, use 1536 for OpenAI's text-embedding-ada-002 and Amazon's amazon.titan-embed-text-v1 models.

- `m?`: 该 max number of connections per layer (16 by default). Index build time improves with smaller values, while higher values can speed up search queries.

- `efConstruction?`: 该 size of the dynamic candidate list for constructing the graph (64 by default). A higher value can potentially improve the index quality at the cost of index build time.

- `distanceFunction?`: 该 distance function name you want to use, is automatically selected based on the distanceStrategy.
