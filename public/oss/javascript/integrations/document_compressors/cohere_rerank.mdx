...
文档重排序可以极大地改进 any RAG application and document retrieval system.

从高层次来看, 重排序 API 是一个语言模型，用于分析文档并根据以下条件重新排序 它们与给定查询的相关性.

Cohere 提供文档重排序 API. In this 示例 we'll show you how to use it.
...
Here, we can 请参阅 the `.rerank()` method returns just the index of the documents (matching the indexes of the input documents) and their relevancy scores.
...
From the results, we can 请参阅 it returned the top 3 documents, and assigned a `relevanceScore` to each.

As expected, the document with the highest `relevanceScore` is the one that references Washington, D.C., with a score of `98.7%`!

### 使用方法 with `CohereClient`
