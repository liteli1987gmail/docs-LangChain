...
To get started, 安装 `@langchain/mixedbread-ai` 包:
...
该 `MixedbreadAIReranker` class provides access to the reranking API. Here’s how to use it:

1. **Import the 类**: First, import the `MixedbreadAIReranker` class from the 包.

```typescript
import { MixedbreadAIReranker } from "@langchain/mixedbread-ai";
```

2. **Instantiate the 类**: Create an instance of `MixedbreadAIReranker` with your API key.

```typescript
const reranker = new MixedbreadAIReranker({ apiKey: "your-api-key" });
```

3. **Rerank Documents**: 使用 the `rerankDocuments` method to reorder documents based on a query.
