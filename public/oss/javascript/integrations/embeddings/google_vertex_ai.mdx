---
title: "VertexAI嵌入 集成"
description: "Integrate with the VertexAI嵌入 embedding model using LangChain JavaScript."
---
...
you're running in a 否de.js environment or a web environment.

To access `ChatVertexAI` models you'll need to setup Google VertexAI in your Google Cloud Platform (GCP) account, save the credentials file, and 安装 `@langchain/google-vertexai` 集成 包.
...
If running in a web environment, you should set the `GOOGLE_VERTEX_AI_WEB_CREDENTIALS` environment variable as a JSON stringified object, and 安装 `@langchain/google-vertexai-web` 包:
...
该 LangChain `VertexAI嵌入` 集成 lives in the `@langchain/google-vertexai` 包:
