...
title: "Bedrock嵌入 集成"
sidebarTitle: "嵌入"
description: "Integrate with the Bedrock嵌入 embedding model using LangChain JavaScript."
---
...
To access Bedrock embedding models you'll need to create an AWS account, get an API key, and 安装 `@langchain/aws` 集成 包.
...
该 LangChain Bedrock 集成 lives in the `@langchain/aws` 包:
...
该re are a few different ways to authenticate with AWS - the below 示例s rely on an access key, secret access key and region set in your environment variables:
