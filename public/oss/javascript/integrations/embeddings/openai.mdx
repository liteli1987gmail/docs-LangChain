...
title: "OpenAI嵌入 集成"
sidebarTitle: "嵌入"
description: "Integrate with the OpenAI嵌入 embedding model using LangChain JavaScript."
---
...
To access OpenAI嵌入 embedding models you'll need to create an OpenAI account, get an API key, and 安装 `@langchain/openai` 集成 包.
...
该 LangChain OpenAI嵌入 集成 lives in the `@langchain/openai` 包:
...
With the `text-embedding-3` class of models, you can specify the size of the embeddings you want returned. For 示例 by default `text-embedding-3-large` returns embeddings of dimension 3072:
...
If you are hosting on Azure OpenAI, 请参阅 the [dedicated page instead](/oss/javascript/集成s/embeddings/azure_openai).
