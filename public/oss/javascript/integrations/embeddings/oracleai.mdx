...
title: "Oracle嵌入 集成"
sidebarTitle: "嵌入"
description: "Integrate with the Oracle嵌入 embedding model using LangChain JavaScript."
---
...
This guide demonstrates how to generate embeddings for your content using `Oracle嵌入`.
...
To use Oracle嵌入, 安装 `@oracle/langchain-oracledb` helpers (plus `@langchain/core`) and make sure the Oracle Database driver prerequisites are met for your system.
...
该 third argument to the constructor is an optional `proxy` string. 使用 it when outbound requests must travel through an HTTP proxy (for 示例, when calling Hugging Face from within a private network).
...
Below is the 示例 code to upload an ONNX model into Oracle Database:
...
使用 the same embedding instances to prepare content for vector stores or hybrid retrieval in Oracle Database.
...
- Store embeddings with [`OracleVS`](/oss/javascript/集成s/vectorstores/oracleai) for hybrid search
