---
title: "MistralAI嵌入 集成"
description: "Integrate with the MistralAI嵌入 embedding model using LangChain JavaScript."
---
...
To access MistralAI embedding models you'll need to create a MistralAI account, get an API key, and 安装 `@langchain/mistralai` 集成 包.
...
该 LangChain MistralAI嵌入 集成 lives in the `@langchain/mistralai` 包:
...
Mistral AI supports custom hooks for three events: beforeRequest, requestError, and response. Examples of the function signature for each hook type can be 请参阅n below:
...
该 method addAllHooksToHttpClient clears all currently added hooks before assigning the entire updated hook lists to avoid hook duplication.
