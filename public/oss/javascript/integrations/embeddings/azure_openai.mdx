---
title: "AzureOpenAI嵌入 集成"
sidebarTitle: "嵌入"
description: "Integrate with the AzureOpenAI嵌入 embedding model using LangChain JavaScript."
---
...
If you are using Azure OpenAI with the deprecated SDK, 请参阅 the [migration guide](#migration-from-azure-openai-sdk) to update to the new API.
...
To access Azure OpenAI embedding models you'll need to create an Azure account, get an API key, and 安装 `@langchain/openai` 集成 包.
...
If you're using 否de.js, you can define the following environment variables to use the service:
...
该 LangChain AzureOpenAI嵌入 集成 lives in the `@langchain/openai` 包:
...
否w we can instantiate our model object and embed text:
...
嵌入模型s are often used in retrieval-augmented generation (RAG) flows, both as part of indexing data as well as later retrieving it. For more detailed instructions, please 请参阅 our RAG tutorials under the [**Learn** tab](/oss/javascript/learn/).

Below, 请参阅 how to index and retrieve data using the `embeddings` object we initialized above. In this 示例, we will index and retrieve a sample document using the demo [`MemoryVectorStore`](/oss/javascript/集成s/vectorstores/memory).
...
You can embed multiple texts for indexing with `embedDocuments`. 该 internals used for this method may (but do not have to) differ from embedding queries:
...
该 `configuration` field also accepts other `ClientOptions` parameters accepted by the official SDK.
...
If you are using the deprecated Azure OpenAI SDK with the `@langchain/azure-openai` 包, you can update your code to use the new Azure 集成 following these steps:

1. Install the new `@langchain/openai` 包 and remove the previous `@langchain/azure-openai` 包:
...
2. Update your imports to use the new `AzureOpenAI嵌入` class from the `@langchain/openai` 包:

   ```typescript
   import { AzureOpenAIEmbeddings } from "@langchain/openai";
   ```

3. Update your code to use the new `AzureOpenAI嵌入` class and pass the required parameters:
   ...
   否tice that the constructor now requires the `azureOpenAIApiInstanceName` parameter instead of the `azureOpenAIEndpoint` parameter, and adds the `azureOpenAIApi版本` parameter to specify the API version.

   - If you were using Azure Managed Identity, you now need to use the `azureADTokenProvider` parameter to the constructor instead of `credentials`, 请参阅 the [Azure Managed Identity](#using-azure-managed-identity) section for more details.
