...
LangChain provides a key-value store interface for storing and retrieving data by key. 该 key-value store interface in LangChain is primarily used for caching [embeddings](/oss/javascript/集成s/embeddings).
...
该 generic nature of the interface allows you to use different types for keys and values. For 示例, `BaseStore<string, BaseMessage>` would store messages with string keys, while `BaseStore<string, number[]>` would store arrays of numbers.
...
<Columns cols={2}>
  <Card title="InMemoryStore" icon="link" href="/oss/javascript/集成s/stores/in_memory" arrow="true" cta="View guide" />
  <Card title="LocalFileStore" icon="link" href="/oss/javascript/集成s/stores/file_system" arrow="true" cta="View guide" />
</Columns>
