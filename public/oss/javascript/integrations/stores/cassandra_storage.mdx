...
This 示例 demonstrates how to setup chat history storage using the `CassandraKVStore` [`BaseStore`](https://reference.langchain.com/javascript/langchain-core/stores/BaseStore) 集成. 否te there is a `CassandraChatMessageHistory`
集成 which may be easier to use for chat history storage; the `CassandraKVStore` is useful if you want a more general-purpose key-value store with
prefixable keys.
...
- [Key-value store 概念指南](/oss/javascript/集成s/stores)
