...
After you've added the role for your identity, you need to retrieve the **session pool management endpoint**. You can find it in the Azure Portal, under the "概述" section of your instance. 该n you need to set the following environment variable:
...
Below is a simple 示例 that creates a new Python code interpreter session, invoke the tool and prints the result.
...
以下是 a complete 示例 where we use an Azure OpenAI chat model to call the Python code interpreter session tool to execute the code and get the result:
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\azure_dynamic_sessions.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
