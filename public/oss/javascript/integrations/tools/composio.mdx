---
title: "Composio 集成"
description: Access 500+ tools and 集成s through Composio's unified API platform for AI agents, with OAuth handling, event-driven workflows, and multi-user support.
---
...
| 类 | 包 | 可序列化 | [Python support](/oss/javascript/集成s/tools/composio) | 版本 |
|:---|:---|:---:|:---:|:---:|
...
- **500+ Tool Access**: Prebuilt 集成s for GitHub, Slack, Gmail, Jira, 否tion, and more
...
该 集成 lives in the `@composio/langchain` 包.

```bash
npm install @composio/langchain @composio/core
```

Or using other 包 managers:
...
以下是 a complete 示例 using Composio tools with a LangGraph agent to interact with HackerNews:
...
以下是 an 示例 that stars a GitHub repository:
...
Composio allows you to create custom tools that can be used alongside built-in tools. 该re are two types:
...
详细文档请参阅 of all Composio features and configurations, visit:
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\composio.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
