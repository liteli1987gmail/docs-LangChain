...
This guide demonstrates how to generate document summaries using `OracleSummary` from the Oracle AI Vector Search LangChain 集成.
...
该 third constructor argument is an optional proxy string. Supply it only when outbound requests must traverse an HTTP proxy (for 示例, when invoking Hugging Face endpoints).
...
- Chunk and normalize with [`OracleTextSplitter`](/oss/javascript/集成s/document_loaders/file_loaders/oracleai#chunk-documents)
- Generate embeddings using [`Oracle嵌入`](/oss/javascript/集成s/embeddings/oracleai) and store them in [`OracleVS`](/oss/javascript/集成s/vectorstores/oracleai)
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\oracleai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
