---
title: "Falkordb 集成"
description: 使用 FalkorDB's ultra-fast graph database with LangChain for natural language queries over knowledge graphs using Cypher
...
该 包 fits into existing LangChain workflows, so if you are already using that framework for other AI capabilities, this becomes another tool in your stack. FalkorDB's low latency characteristics combine with LangChain's language model 集成 to provide quick responses.
...
该 easiest way to run FalkorDB is with Docker:
...
- `host` (string, optional): Database host. 默认: `"localhost"`
- `port` (number, optional): Database port. 默认: `6379`
- `graph` (string, optional): Graph name to use
- `url` (string, optional): Alternative connection URL format
- `enhancedSchema` (boolean, optional): Enable enhanced schema details. 默认: `false`
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\falkordb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
