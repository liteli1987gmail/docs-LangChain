...
该 `@langchain/openai` 包 provides LangChain-compatible wrappers for OpenAI's built-in tools. 该se tools can be bound to `ChatOpenAI` using `bindTools()` or [`createAgent`](https://reference.langchain.com/javascript/langchain/index/createAgent).

### Web search tool

该 web search tool allows OpenAI models to search the web for up-to-date information before generating a response. Web search supports three main types:

1. **否n-reasoning web search**: Quick lookups where the model passes queries directly to the search tool
2. **Agentic search with reasoning models**: 该 model actively manages the search process, analyzing results and deciding whether to keep searching
...
该 MCP tool allows OpenAI models to connect to remote MCP servers and OpenAI-maintained service connectors, giving models access to external tools and services.

该re are two ways to use MCP tools:

1. **Remote MCP servers**: Connect to any public MCP server via URL
2. **Connectors**: 使用 OpenAI-maintained wrappers for popular services like Google Workspace or Dropbox
...
该 Code Interpreter tool allows models to write and run Python code in a sandboxed environment to solve complex problems.

使用 Code Interpreter for:
...
> **否te**: Containers expire after 20 minutes of inactivity. While called "Code Interpreter", the model knows it as the "python tool" - for explicit prompting, ask for "the python tool" in your prompts.
...
该 File Search tool allows models to search your files for relevant information using semantic and keyword search. It enables retrieval from a knowledge base of previously uploaded files stored in vector stores.
...
该 Image Generation tool allows models to generate or edit images using text prompts and optional image inputs. It leverages the GPT Image model and automatically optimizes text inputs for improved performance.

使用 Image Generation for:
...
> **Prompting tips**: 使用 terms like "draw" or "edit" for best results. For combining images, say "edit the first image by adding this element" instead of "combine" or "merge".
...
该 Computer 使用 tool allows models to control computer interfaces by simulating mouse clicks, keyboard input, scrolling, and more. It uses OpenAI's Computer-Using Agent (CUA) model to understand screenshots and suggest actions.

> **Beta**: Computer use is in beta. 使用 in sandboxed environments only and do not use for high-stakes or authenticated tasks. Always implement human-in-the-loop for important decisions.
...
该 Local Shell tool allows models to run shell commands locally on a machine you provide. Commands are executed inside your own runtime—the API only returns the instructions.

> **Security 警告**: Running arbitrary shell commands can be dangerous. Always sandbox execution or add strict allow/deny-lists before forwarding commands to the system shell.
...
- `user` - 可选 user to run the command as

For more information, see [OpenAI's Local Shell Documentation](https://platform.openai.com/docs/guides/tools-local-shell).

### Shell tool

该 Shell tool allows models to run shell commands through your 集成. Unlike Local Shell, this tool supports executing multiple commands concurrently and is designed for `gpt-5.1`.
...
- `timeout_ms` - 可选 timeout in milliseconds (enforce your own limits)
- `max_output_length` - 可选 maximum characters to return per command
...
> **否te**: Only available through the Responses API with `gpt-5.1`. 该 `timeout_ms` from the model is only a hint—always enforce your own limits.

For more information, see [OpenAI's Shell Documentation](https://platform.openai.com/docs/guides/tools-shell).

### Apply patch tool

该 Apply Patch tool allows models to propose structured diffs that your 集成 applies. This enables iterative, multi-step code editing workflows where the model can create, update, and delete files in your codebase.
...
> **Security 警告**: Applying patches can modify files in your codebase. Always validate paths, implement backups, and consider sandboxing.
> **否te**: This tool is designed to work with `gpt-5.1` model.
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\openai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
