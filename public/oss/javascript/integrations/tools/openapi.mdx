...
<警告>
This agent can make requests to external APIs. 使用 with caution, especially when granting access to users.
...
该 `OpenAPIToolkit` has access to the following tools:
...
| `requests_post` | 使用 this when you want to POST to a website. Input should be a json string with two keys: "url" and "data". 该 value of "url" should be a string, and the value of "data" should be a dictionary of key-value pairs you want to POST to the URL as a JSON body. Be careful to always use double quotes for strings in the json string. 该 output will be the text response of the POST request. |
...
This toolkit lives in the `langchain` 包:
...
否w we can instantiate our toolkit. First, we need to define the LLM we would like to use in the toolkit.
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\openapi.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
