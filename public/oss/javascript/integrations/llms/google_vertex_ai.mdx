...
You may be looking for [this page instead](/oss/javascript/集成s/chat/google_vertex_ai/).
</警告>
...
To access VertexAI models you'll need to create a Google Cloud Platform (GCP) account, get an API key, and 安装 `@langchain/google-vertexai` 集成 包.
...
the `@langchain/google-vertexai-web` 包.

该n, you'll need to add your service account credentials directly as a `GOOGLE_VERTEX_AI_WEB_CREDENTIALS` environment variable:
...
该 LangChain VertexAI 集成 lives in the `@langchain/google-vertexai` 包:
