...
You may be looking for [this page instead](/oss/javascript/集成s/chat/bedrock/).
</警告>
...
To access Bedrock models you'll need to create an AWS account, get an API key, and 安装 `@langchain/community` 集成, along with a few peer dependencies.
...
该 LangChain Bedrock 集成 lives in the `@langchain/community` 包:
...
否te that some models require specific prompting techniques. For 示例, Anthropic's Claude-v2 model will throw an error if
