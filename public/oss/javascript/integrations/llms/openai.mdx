...
Unless you are specifically using `gpt-3.5-turbo-instruct`, you are probably looking for [this page instead](/oss/javascript/集成s/chat/openai/).
</警告>
...
To access OpenAI models you'll need to create an OpenAI account, get an API key, and 安装 `@langchain/openai` 集成 包.
...
该 LangChain OpenAI 集成 lives in the `@langchain/openai` 包:
...
If you are hosting on Azure OpenAI, 请参阅 the [dedicated page instead](/oss/javascript/集成s/llms/azure).
