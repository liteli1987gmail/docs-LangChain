...
- **[重放](#replay)：从先前的检查点重试。
- **[分叉](#fork)：从先前的检查点创建分支，并修改状态以探索替代路径。

两者都通过从先前的检查点恢复来工作。检查点之前的节点不会重新执行（结果已保存）。检查点之后的节点会重新执行，包括任何大型语言模型调用、API 请求和[中断](/oss/javascript/langgraph/interrupts)（可能产生不同的结果）。
...
重放会重新执行节点——不仅仅是读取缓存。大型语言模型调用、API 请求和[中断](/oss/javascript/langgraph/interrupts)会再次触发，可能返回不同的结果。从最终检查点重放（没有 `next` 节点）是一个空操作。
...
使用 [`getStateHistory`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#getStateHistory) 找到您想要重放的检查点，然后使用该检查点的配置调用 [`invoke`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.CompiledStateGraph.html#invoke)：
...
分叉从过去的检查点创建一个新分支。调用 [`update_state`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#updateState) 在先前的检查点上创建分叉，然后使用 `None` 调用 [`invoke`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.CompiledStateGraph.html#invoke) 继续执行。

![Fork](/oss/images/checkpoints_full_story.jpg)

<Warning>

`update_state` **不会**回滚线程。它创建一个从指定点分支的新检查点。原始执行历史保持不变。
...
当您调用 [`update_state`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#updateState) 时，值使用指定节点的写入器（包括 [reducers](/oss/javascript/langgraph/graph-api#reducers)）应用。检查点记录该节点产生了更新，执行从该节点的后继者恢复。

默认情况下，LangGraph 从检查点的版本历史推断 `as_node`。当从特定检查点分叉时，这种推断几乎总是正确的。

在以下情况下明确指定 `as_node`：

- **并行分支**：多个节点在同一步骤中更新了状态，LangGraph 无法确定哪个是最后的（`InvalidUpdateError`）。
- **没有执行历史**：在全新线程上设置状态（常见于[测试](/oss/javascript/langgraph/test)）。
...
如果您的图使用 [`interrupt`](https://reference.langchain.com/javascript/langchain-langgraph/index/interrupt) 进行[人工介入](/oss/javascript/langgraph/interrupts)工作流，中断总是在时间旅行期间重新触发。包含中断的节点会重新执行，`interrupt()` 为新的 `Command(resume=...)` 暂停。
...
如果您的图在多个点收集输入（例如，多步表单），您可以从中断之间进行分叉，以更改后续答案而无需重新询问 earlier 问题。
...
[子图](/oss/javascript/langgraph/use-subgraphs)的时间旅行取决于子图是否有自己的检查点。这决定了您可以从中进行时间旅行的检查点粒度。
...
您无法时间旅行到默认子图中节点*之间*的某个点——您只能从父级别进行时间旅行。
...
在子图上设置 `checkpointer=True` 以给它自己的检查点历史。这会在子图内的每个步骤创建检查点，允许您从其中的特定点进行时间旅行——例如，在两个中断之间。

使用带有 `subgraphs=True` 的 [`get_state`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#getState) 访问子图自己的检查点配置，然后从中分叉：
