...
加载技能时，CLI 会验证解析后的文件路径是否在技能根目录之一内。解析到所有技能根目录之外的符号链接会被拒绝。若要允许其他目录中的符号链接目标，请参见 [`[skills].extra_allowed_dirs`](/oss/javascript/deepagents/cli/configuration#skills-extra-allowed-directories)。
...
每个子代理是一个带有 YAML frontmatter（`name`、`description`、可选的 `model`）和一个 markdown 正文的 `AGENTS.md` 文件，用于系统提示词。参见[自定义子代理](/oss/javascript/deepagents/cli/overview#subagents)获取完整的格式参考。
