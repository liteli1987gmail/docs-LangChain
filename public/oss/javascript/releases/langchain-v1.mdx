...
<CardGroup cols={1}>
    ...
    <Card title="Simplified 包" icon="sitemap" href="#simplified-包" arrow>
        该 `langchain` 包 has been streamlined to focus on essential building blocks for agents, with 旧版 functionality moved to `@langchain/classic`.
    </Card>
</CardGroup>
...
For a complete list of changes, 请参阅 the [migration guide](/oss/javascript/migrate/langchain-v1).
...
更多信息请参阅, 请参阅 [Agents](/oss/javascript/langchain/agents).
...
- `humanIn该LoopMiddleware`: Require approval for sensitive tool calls
...
更多信息请参阅, 请参阅 [the complete middleware guide](/oss/javascript/langchain/middleware).
...
- **Main loop 集成**: 结构化输出 is now generated in the main loop instead of requiring an additional LLM call
- **结构化输出 strategy**: Models can choose between calling tools or using provider-side structured output generation
...
<否te>
    1.0 releases are available for most 包s. Only the following currently support new content blocks:
...
更多信息请参阅, 请参阅 our guide on [content blocks](/oss/javascript/langchain/messages#message-content)

---

## Simplified 包

LangChain v1 streamlines the `langchain` 包 namespace to focus on essential building blocks for agents. 该 包 exposes only the most useful and relevant functionality:
...
- [版本ing](/oss/javascript/versioning) – Understanding version numbers
