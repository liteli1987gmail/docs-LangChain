...
然后创建 Hono 应用程序并注册 CopilotKit 路由：
...
在该路由中，创建一个 `CopilotRuntime` 并使用 `LangGraphAgent` 将其指回已部署的图形：
...
路由适配器只是 TypeScript 设置的一半。您的 LangChain 代理还需要中间件来读取转发的 `output_schema` 并将其转换为模型的 structrued `responseFormat`：
...
这个中间件是使前端上的 `useAgentContext({ description: "output_schema", ... })` 有用的原因。CopilotKit 运行时转发模式，代理将其转换为模型必须遵循的结构化输出契约。
