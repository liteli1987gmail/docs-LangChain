...
LangChain 和 [Deep Agents](/oss/javascript/deepagents/overview) 为常见用例提供预构建中间件。每个中间件都已准备好投入生产，可针对您的特定需求进行配置。
...
<Tip>
    如果使用 `langchain@1.1.0`，则 `trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的 [profile 数据](/oss/javascript/langchain/models#model-profiles)。如果没有可用的数据，请使用其他条件或手动指定：
    ```typescript
...
在工具调用执行之前暂停代理执行以进行人工批准、编辑或拒绝。[Human-in-the-loop](/oss/javascript/langchain/human-in-the-loop) 适用于以下场景：
...
<Warning>
    Human-in-the-loop 中间件需要 [checkpointer](/oss/javascript/langgraph/persistence#checkpoints) 来在中断之间维护状态。
</Warning>
...
<Tip>
    有关完整的示例、配置选项和集成模式，请参阅 [Human-in-the-loop 文档](/oss/javascript/langchain/human-in-the-loop)。
</Tip>
...
`FilesystemMiddleware` 来自 [Deep Agents](/oss/javascript/deepagents/overview)，提供四个用于与短期和长期记忆交互的工具：
