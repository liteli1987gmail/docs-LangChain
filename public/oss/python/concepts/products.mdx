...
[LangChain](/oss/python/langchain/overview) 是一个提供结构化内容块、代理循环和中间件等抽象的代理框架。

LangChain 的抽象设计为易于上手，同时仍提供高级用例所需的灵活性。

虽然 LangChain 构建在 [LangGraph](/oss/python/langgraph/overview) 之上，但你不需要了解 LangGraph 也能使用 LangChain。
...
[LangGraph](/oss/python/langgraph/overview) 是一个低级编排框架和运行时，用于构建、管理和部署长时间运行的有状态代理。
...
[Deep Agents SDK](/oss/python/deepagents/overview) 构建在 LangGraph 之上，添加了规划能力、用于上下文管理的文件系统、生成子代理的能力等等。
...
| 技能（Skills） | [多代理技能](/oss/python/langchain/multi-agent/skills) | - | [技能](/oss/python/deepagents/skills) |
