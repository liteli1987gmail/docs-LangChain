...
两者都通过从先前的检查点恢复来工作。检查点之前的节点不会重新执行（结果已保存）。检查点之后的节点会重新执行，包括任何大型语言模型调用、API 请求和[中断](/oss/python/langgraph/interrupts)（可能产生不同的结果）。
...
重放会重新执行节点——不仅仅是读取缓存。大型语言模型调用、API 请求和[中断](/oss/python/langgraph/interrupts)会再次触发，可能返回不同的结果。从最终检查点重放（没有 `next` 节点）是一个空操作。

</Warning>

![Replay](/oss/images/re_play.png)

使用 [`get_state_history`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.get_state_history) 找到您想要重放的检查点，然后使用该检查点的配置调用 [`invoke`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.invoke)：
...
分叉从过去的检查点创建一个新分支。调用 [`update_state`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.update_state) 在先前的检查点上创建分叉，然后使用 `None` 调用 [`invoke`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.invoke) 继续执行。
...
当您调用 [`update_state`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.update_state) 时，值使用指定节点的写入器（包括 [reducers](/oss/python/langgraph/graph-api#reducers)）应用。检查点记录该节点产生了更新，执行从该节点的后继者恢复。
...
- **没有执行历史**：在全新线程上设置状态（常见于[测试](/oss/python/langgraph/test)）。
...
如果您的图使用 [`interrupt`](https://reference.langchain.com/python/langgraph/types/interrupt) 进行[人工介入](/oss/python/langgraph/interrupts)工作流，中断总是在时间旅行期间重新触发。包含中断的节点会重新执行，`interrupt()` 为新的 `Command(resume=...)` 暂停。
...
[子图](/oss/python/langgraph/use-subgraphs)的时间旅行取决于子图是否有自己的检查点。这决定了您可以从中进行时间旅行的检查点粒度。
...
使用带有 `subgraphs=True` 的 [`get_state`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.get_state) 访问子图自己的检查点配置，然后从中分叉：
