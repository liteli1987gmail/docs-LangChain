...
深度代理构建在 LangGraph 的流式传输基础设施之上，对子代理流有一流支持。当深度代理将工作委托给子代理时，你可以独立地从每个子代理流式传输更新 — 实时跟踪进度、Token 和工具调用。

深度代理流式传输可以实现的功能：

* <Icon icon="diagram-subtask" size={16} /> [**Stream subagent progress**](#subagent-progress)—跟踪每个子代理在并行运行时的执行。
* <Icon icon="square-binary" size={16} /> [**Stream LLM tokens**](#llm-tokens)—从主代理和每个子代理流式传输 token。
* <Icon icon="screwdriver-wrench" size={16} /> [**Stream tool calls**](#tool-calls)—查看子代理执行内的工具调用和结果。
* <Icon icon="table" size={16} /> [**Stream custom updates**](#custom-updates)—从子代理节点内部发出用户定义的信号。

## 启用子图流式传输

深度代理使用 LangGraph 的子图流式传输来暴露子代理执行中的事件。要接收子代理事件，在流式传输时启用 `stream_subgraphs`。
...
当 `subgraphs` 启用时，每个流式事件都包含一个 **namespace**，用于标识产生它的代理。namespace 是节点名称和任务 ID 的路径，表示代理层次结构。
