...
<ResponseField name="base_system_prompt" type="string">
    替换基础 Deep Agents 系统提示词（[Prompt assembly](/oss/python/deepagents/customization#prompt-assembly) 中的 `CUSTOM`）。
</ResponseField>

<ResponseField name="system_prompt_suffix" type="string">
    追加到组装的基础提示词的文本（[Prompt assembly](/oss/python/deepagents/customization#prompt-assembly) 中的 `SUFFIX`）；应用于主代理、声明式子代理和自动添加的通用目的子代理。
</ResponseField>
...
<ResponseField name="general_purpose_subagent" type="GeneralPurposeSubagentProfile">
    禁用、重命名或重新提示通用目的子代理。当此字段的 `system_prompt` 与 `base_system_prompt` 一起设置时，通用目的专用子代理提示词优先 — 参见[通用目的子代理提示词](/oss/python/deepagents/customization#general-purpose-subagent-prompt)。
</ResponseField>

<Note>
    调用者提供的 `system_prompt=` 始终位于组装提示词的前面，`system_prompt_suffix` 始终位于末尾 — 无论选择哪个模型。相同的覆盖规则适用于子代理：每个子代理针对其自己的模型重新运行 profile 解析。参见[提示词组装](/oss/python/deepagents/customization#prompt-assembly)获取每种情况的完整细分（主代理、子代理和通用目的子代理）。
</Note>
