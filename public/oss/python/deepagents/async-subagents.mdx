...
这是对[子代理](/oss/python/deepagents/subagents)的扩展，子代理同步运行并阻塞主代理直到完成。当任务长时间运行、可并行化或需要中途转向时，使用异步子代理。
...
将异步子代理定义为 [`AsyncSubAgent`](https://reference.langchain.com/python/deepagents/middleware/async_subagents/AsyncSubAgent) 规范列表，每个指向一个 Agent Protocol 服务器：
...
[`AsyncSubAgentMiddleware`](https://reference.langchain.com/python/deepagents/middleware/async_subagents/AsyncSubAgentMiddleware) 为主代理提供五个工具：
...
任务元数据存储在主代理图的专用状态通道（`async_tasks`）中，与消息历史分开。这很关键，因为深度代理在上下文窗口填满时会[压缩其消息历史](/oss/python/deepagents/context-engineering#summarization)。如果任务 ID 仅存在于工具消息中，它们会在压缩期间丢失。专用通道确保主代理即使在多轮压缩后仍能通过 `list_async_tasks` 召回其任务。
