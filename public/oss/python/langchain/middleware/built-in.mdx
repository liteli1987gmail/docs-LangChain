...
LangChain 和 [Deep Agents](/oss/python/deepagents/overview) 为常见用例提供预构建中间件。每个中间件都已准备好投入生产，可针对您的特定需求进行配置。
...
<Tip>
    如果使用 `langchain>=1.1`，则 `trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的 [profile 数据](/oss/python/langchain/models#model-profiles)。如果没有可用的数据，请使用其他条件或手动指定：
...
<ParamField body="model" type="string | BaseChatModel" required>
    用于生成摘要的模型。可以是模型标识符字符串（例如 `'openai:gpt-5.4-mini'`）或 `BaseChatModel` 实例。请参阅 [`init_chat_model`](https://reference.langchain.com/python/langchain/chat_models/base/init_chat_model) 获取更多信息。
</ParamField>
...
<ParamField body="trigger" type="ContextSize | list[ContextSize] | None">
    ...
    - [`ContextSize`](https://reference.langchain.com/python/langchain/agents/middleware/summarization/ContextSize) 元组列表（满足任何条件 - 或逻辑）
...
在工具调用执行之前暂停代理执行以进行人工批准、编辑或拒绝。[Human-in-the-loop](/oss/python/langchain/human-in-the-loop) 适用于以下场景：
...
<Warning>
    Human-in-the-loop 中间件需要 [checkpointer](/oss/python/langgraph/persistence#checkpoints) 来在中断之间维护状态。
</Warning>
...
<Tip>
    有关完整的示例、配置选项和集成模式，请参阅 [Human-in-the-loop 文档](/oss/python/langchain/human-in-the-loop)。
</Tip>
...
<ParamField body="model" type="string | BaseChatModel">
    用于工具选择的模型。可以是模型标识符字符串（例如 `'openai:gpt-5.4-mini'`）或 `BaseChatModel` 实例。请参阅 [`init_chat_model`](https://reference.langchain.com/python/langchain/chat_models/base/init_chat_model) 获取更多信息。
...
<ParamField body="model" type="string | BaseChatModel">
    用于生成模拟工具响应的模型。可以是模型标识符字符串（例如 `'google_genai:gemini-3.1-pro-preview'`）或 `BaseChatModel` 实例。如果未指定，默认为代理的主模型。请参阅 [`init_chat_model`](https://reference.langchain.com/python/langchain/chat_models/base/init_chat_model) 获取更多信息。
</ParamField>
...
`FilesystemMiddleware` 来自 [Deep Agents](/oss/python/deepagents/overview)，提供四个用于与短期和长期记忆交互的工具：
