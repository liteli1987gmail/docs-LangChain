...
Here are two prompt templates that meet your requirements:

## 1. SPARQL Generation Prompt
...
sparql_generation_prompt = PromptTemplate(
    ...
    template="""You are an expert SPARQL query generator for a knowledge graph.
...
## Task
Based on the schema above, generate a valid SPARQL query to answer the user's question.

## Rules
1. Use proper SPARQL syntax
2. Only use predicates/classes that exist in the schema
3. Prefix IRIs correctly
...
sparql_query = sparql_generation_prompt.format(
    schema="""Prefix ex: <http://example.org/>
    
    ex:Person rdf:type rdfs:Class .
    ex:name rdf:type rdf:Property .""",
    prompt="List all persons with their names"
)

# Answer question
answer = qa_prompt.format(
    context="The Eiffel Tower is located in Paris, France. It was built in 1889.",
    prompt="Where is the Eiffel Tower located?"
)
```

Would you like me to add any specific instructions or modify these templates?
