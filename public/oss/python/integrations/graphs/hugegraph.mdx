...
[HugeGraph](https://hugegraph.apache.org/) 是一个便捷、高效、可扩展的图数据库，兼容 `Apache TinkerPop3` 框架和 `Gremlin` 查询语言。

[Gremlin](https://en.wikipedia.org/wiki/Gremlin_(query_language)) 是由 `Apache TinkerPop`（属于 `Apache Software Foundation`）开发的图遍历语言和虚拟机。
