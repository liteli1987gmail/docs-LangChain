...
[Neo4j](https://neo4j.com/docs/getting-started/) 是一个由 Neo4j, Inc. 开发的图形数据库管理系统。


Neo4j 存储的数据元素包括节点、连接节点的边以及节点和边的属性。Neo4j 由其开发者描述为支持 ACID -compliant（原子性、一致性、隔离性、持久性）事务的数据库，具有原生图存储和处理能力。Neo4j 提供"社区版"，采用 GNU General Public License 的修改版本授权（不开放源代码），而在线备份和高可用性扩展则采用闭源商业许可证授权。Neo 还将这些扩展以闭源商业条款授权给 Neo4j。

本 notebook 展示了如何使用 LLMs 为图数据库提供自然语言界面，您可以使用 `Cypher` 查询语言对其进行查询。

[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式图查询语言，能够在 property graph 中进行表达力强且高效的数据查询。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\graphs\neo4j_cypher.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
