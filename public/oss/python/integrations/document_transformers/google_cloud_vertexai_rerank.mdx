...
[Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收一个文档列表，并根据这些文档与查询的相关性对文档进行重新排序。与 embeddings（仅查看文档和查询之间的语义相似性）相比，ranking API 可以为您提供文档回答特定查询的精确评分。ranking API 可用于在检索到初始候选文档集后提高搜索结果的质量。

Ranking API 是无状态的，因此在调用 API 之前不需要对文档进行索引。您只需传入查询和文档。这使得 API 非常适合对来自任何文档检索器的文档进行重排序。


欲了解更多，请参阅 [Rank and rerank documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
