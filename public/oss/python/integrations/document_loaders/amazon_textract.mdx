...
它超越了简单的光学字符识别 (OCR)，能够识别、理解并从表单和表格中提取数据。如今，许多公司手动从 PDF、图片、表格和表单等扫描文档中提取数据，或通过简单的 OCR 软件进行提取（这些软件需要手动配置，且当表单变更时通常需要更新）。为了克服这些手动且昂贵的流程，`Textract` 使用 ML 来读取和处理任何类型的文档，无需人工操作即可准确提取文本、手写内容、表格和其他数据。
