...
[阿里云MaxCompute](https://www.alibabacloud.com/product/maxcompute)（原名ODPS）是一个通用、完全托管、多租户的大规模数据仓库处理平台。MaxCompute支持多种数据导入解决方案和分布式计算模型，使用户能够有效查询海量数据集，降低生产成本，并确保数据安全。
