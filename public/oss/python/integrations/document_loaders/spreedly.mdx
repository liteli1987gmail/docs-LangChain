...
[Spreedly](https://docs.spreedly.com/) 是一项服务，允许您安全地存储信用卡，并使用它们与任意数量的支付网关（payment gateway）和第三方 API 进行交易。它通过同时提供卡片令牌化（tokenization）/保险库服务以及网关和接收器（receiver）集成服务来实现这一功能。通过 Spreedly 令牌化的支付方式（payment methods）存储在 Spreedly，让您能够独立存储卡片，然后根据业务需求将该卡片传递到不同的端点（endpoints）。
