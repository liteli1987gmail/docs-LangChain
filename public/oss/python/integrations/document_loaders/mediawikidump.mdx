...
[MediaWiki XML 转储](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps) 包含一个 wiki 的内容（带有所有修订版本的 wiki 页面），但不包含站点相关数据。XML 转储不会创建 wiki 数据库的完整备份，转储不包含用户账户、图片、编辑日志等。
