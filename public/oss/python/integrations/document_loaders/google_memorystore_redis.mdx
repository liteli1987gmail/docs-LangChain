...
> [Google Memorystore for Redis](https://cloud.google.com/memorystore/docs/redis/memorystore-for-redis-overview) 是一项全托管服务，由 Redis 内存数据存储提供支持，可构建应用缓存，提供亚毫秒级数据访问。结合 Memorystore for Redis 的 LangChain 集成，扩展您的数据库应用以构建 AI 驱动的体验。
