...
>[Diffbot](https://docs.diffbot.com/docs/getting-started-with-diffbot) 是一套基于 ML 的产品，能够轻松地将网页数据结构化。


> Diffbot的 [Extract API](https://docs.diffbot.com/reference/extract-introduction) 是一项服务，可对网页中的数据进行结构化和规范化处理。

不像传统的网络爬虫工具，`Diffbot Extract` 不需要任何规则来读取页面内容。它使用计算机视觉模型将页面分类为20种可能的类型之一，然后将原始 HTML 标记转换为 JSON。生成的 JSON 遵循一致的[基于类型的本体论](https://docs.diffbot.com/docs/ontology)，这使得使用相同的模式从多个不同的 Web 源提取数据变得容易。
