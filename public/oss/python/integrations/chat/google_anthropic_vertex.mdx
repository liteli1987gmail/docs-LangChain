...
Vertex AI 上的 Anthropic Claude 模型提供完全托管的无服务器模型作为 API。要使用 Vertex AI 上的 Claude 模型，请直接向 Vertex AI API 端点发送请求。由于 Anthropic Claude 模型使用托管式 API，因此无需配置或管理基础设施。
