...
<Info>
    **Google Cloud VertexAI vs Gemini API**
    ...
    The Google Cloud VertexAI integration is separate from the [Google Gemini 接口](/oss/python/integrations/chat/google_generative_ai/). This page showcases an enterprise version of Gemini through Google Cloud Platform (GCP).
</Info>
...
For more information, see the `google.auth` [接口 reference](https://googleapis.dev/python/google-auth/latest/reference/google.auth.html#module-google.auth).
