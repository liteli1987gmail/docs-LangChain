...
> 注意：如果在设置客户端时更改 `model` 或其他参数（如 `temperature` 或 `max_tokens`），它将覆盖之前在 LaunchPad 中使用的默认配置。
...
# System Prompt Override Explanation

You're correct! This is an important technical consideration when working with AI assistant applications.

## Key Points:

1. **Fixed System Prompt**: When deploying an AI assistant application on a platform (like a hosted service), the platform typically sets a fixed system prompt as the base configuration.

2. **Overriding Behavior**: If you place/insert a new system prompt within the application interface, it will **override** the original fixed system prompt set during deployment.

3. **Practical Impact**: This means:
   - The new prompt takes precedence
   - The original deployment configuration is effectively replaced
   - The assistant will follow the new system's instructions

## Example:

```
Original (Fixed during deployment):
"You are a helpful assistant for customer support."

If you add new system prompt:
"You are a coding assistant specializing in Python."

Result: The assistant will act as a Python coding assistant, not a customer support helper.
```

## Recommendation:

If you need to use a custom system prompt while keeping some functionality from the original, you should **merge** or **combine** both prompts rather than replacing entirely.

Would you like more details on how to properly handle system prompt configuration in your specific use case?
...
> 请注意：与 `model_name` 类似，当您调用 `repositories` 参数时，您可能会覆盖 launchpad 中已连接的配置库。
...
理想情况下，您不需要在此处连接 Repository IDs 就能获得 Retrieval Augmented Generation。如果您已经在 Prem 平台上连接了 repositories，仍然可以获得相同的结果。
