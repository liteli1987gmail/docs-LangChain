...
> 注意：如果在设置客户端时更改 `model` 或任何其他参数（如 `temperature` 或 `max_tokens`），它将覆盖在 LaunchPad 中使用的现有默认配置。

---

如果你需要我进一步解释这段内容的具体含义，或者需要我帮你实现相关功能，请告诉我具体需求。例如：

- **代码示例**：展示如何在客户端设置中覆盖这些参数
- **场景说明**：解释这个机制在实际应用中的使用场景
- **其他帮助**：根据你的具体需求提供相应支持
...
如果您在此处放置 system prompt，它将覆盖从平台部署应用程序时设置的固定 system prompt。

您可以在 PremAI SDK 文档中找到所有[可选参数](https://docs.premai.io/get-started/sdk#optional-parameters)。除[这些支持的参数](https://docs.premai.io/get-started/sdk#optional-parameters)之外的其他参数将在调用模型之前自动删除。
...
请注意：类似 `model_name`，当您调用参数 `repositories` 时，您可能会覆盖启动台中连接的仓库。
...
理想情况下，您不需要在此处连接 Repository ID 即可获得 Retrieval Augmented Generation。即使结果，您仍然可以通过在 Prem Platform 中连接 Repository 来获得相同的结果。
...
请注意：目前不支持流式RAG。但我们仍然通过API提供支持。您可以在[chat completion SSE文档](https://docs.premai.io/get-started/chat-completion-sse)中了解更多详情。
...
当前版本的 LangChain ChatPremAI 不支持带有流式传输支持的 function/tool 调用。流式传输支持与 function 调用即将推出。
...
**NOTE:**
当定义 function/tool schema 时，不要忘记在 function arguments 周围添加信息，否则会抛出 error。
