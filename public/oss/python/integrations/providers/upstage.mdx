...
[Upstage](https://upstage.ai) 是一家领先的人工智能（AI）公司，专注于提供超越人类水平性能的LLM组件。

**Solar Pro** 是一款企业级LLM，针对单GPU部署进行了优化，在指令遵循和处理HTML、Markdown等结构化格式方面表现出色。它支持英语、韩语和日语，具备卓越的多语言性能，并在金融、医疗和法律领域提供专业知识。


除 Solar 外，Upstage 还提供用于实际场景的 RAG（检索增强生成）功能，例如 **Document Parse** 和 **Groundedness Check**。
