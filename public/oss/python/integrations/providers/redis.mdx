...
[Redis (Remote Dictionary Server)](https://en.wikipedia.org/wiki/Redis) 是一个开源的内存存储，用作分布式、内存键值数据库、缓存和消息代理，支持可选的持久化功能。由于 Redis 将所有数据存储在内存中，加上其优秀的设计，`Redis` 能够提供低延迟的读写性能，使其特别适用于需要缓存的场景。Redis 是最受欢迎的 NoSQL 数据库，也是目前最流行的数据库之一。
