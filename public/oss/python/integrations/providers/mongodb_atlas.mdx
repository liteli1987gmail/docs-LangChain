...
[MongoDB Atlas](https://www.mongodb.com/docs/atlas/) 是一个完全托管的云数据库，可在 AWS、Azure 和 GCP 上使用。它现在支持原生的 Vector Search（向量搜索）功能，可对 MongoDB 文档数据进行搜索。
...
`Hybrid Search Retriever` 将向量搜索和全文搜索相结合，通过 `Reciprocal Rank Fusion`（`RRF`）算法进行加权融合。
