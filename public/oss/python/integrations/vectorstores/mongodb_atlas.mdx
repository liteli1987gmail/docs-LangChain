...
MongoDB Atlas 是一款全托管云数据库，可在 AWS、Azure 和 GCP 上使用。它支持原生向量搜索、全文搜索 (BM25) 以及针对 MongoDB 文档数据的混合搜索功能。

[MongoDB Vector Search](https://www.mongodb.com/products/platform/atlas-vector-search) 允许将嵌入向量存储在 MongoDB 文档中，创建向量搜索索引，并使用近似最近邻算法（Hierarchical Navigable Small Worlds）执行 KNN 搜索。它使用 [$vectorSearch MQL Stage](https://www.mongodb.com/docs/atlas/atlas-vector-search/vector-search-overview/)。


[MongoDB Atlas Embedding and Reranking API](https://www.mongodb.com/docs/atlas/atlas-vector-search/ai-integrations/langchain/) 使能够通过 MongoDB Atlas 访问 Voyage AI 模型。
...
> * 更多文档请参阅 [MongoDB's LangChain Docs](https://www.mongodb.com/docs/atlas/ai-integrations/langchain/)。
> * 此功能已正式发布 (Generally Available)，可用于生产环境部署。
