...
[Qdrant](https://qdrant.tech/documentation/)（发音：quadrant）是一个向量相似性搜索引擎。它提供了一个生产就绪的服务，配备便捷的 API，用于存储、搜索和管理向量，并支持额外的 payload 及扩展的过滤功能。这使其适用于各种神经网络或基于语义的匹配、分面搜索以及其他应用。
