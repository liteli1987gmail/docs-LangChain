...
[ClickHouse](https://clickhouse.com/) 是一个用于实时应用和分析的开源数据库，支持完整的 SQL。ClickHouse 支持精确向量搜索（例如，使用 `L2Distance` 等距离函数），以及使用向量相似度索引的近似向量搜索（ClickHouse 25.8+ 起可用）。详情请参阅[精确与近似向量搜索](https://clickhouse.com/docs/engines/table-engines/mergetree-family/annindexes)。
