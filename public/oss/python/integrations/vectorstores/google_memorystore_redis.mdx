...
[Google Memorystore for Redis](https://cloud.google.com/memorystore/docs/redis/memorystore-for-redis-overview) 是一项完全托管服务，由 Redis 内存数据存储提供支持，可构建亚毫秒级数据访问的应用缓存。通过利用 Memorystore for Redis 的 LangChain 集成扩展您的数据库应用，构建 AI 驱动的体验。
