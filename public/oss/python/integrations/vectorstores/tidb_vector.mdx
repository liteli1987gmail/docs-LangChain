...
TiDB Cloud 是一个全面的数据库即服务 (DBaaS) 解决方案，提供专用和 serverless 选项。TiDB Serverless 现在正在将内置的向量搜索集成到 MySQL 生态中。借助这一增强功能，您可以在无需新数据库或额外技术栈的情况下，使用 TiDB Serverless 无缝开发 AI 应用程序。创建免费的 TiDB Serverless 集群，并在 [pingcap.com/ai](https://pingcap.com/ai) 开始使用向量搜索功能。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\vectorstores\tidb_vector.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
