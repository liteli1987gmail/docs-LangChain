...
> [DataStax Astra DB](https://docs.datastax.com/en/astra-db-serverless/index.html) 是一个构建于 `Apache Cassandra®` 之上的无服务器 AI 就绪数据库，并通过易于使用的 JSON API 便捷提供。
...
> 注意：此单元格仅在集合支持 [find-and-rerank](https://docs.datastax.com/en/astra-db-serverless/api-reference/document-methods/find-and-rerank.html) 命令且向量存储能够识别此功能时才能运行。
