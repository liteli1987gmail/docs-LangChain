...
[Playwright](https://github.com/microsoft/playwright) 是一个由 `Microsoft` 开发的支持开源的自动化工具，它允许你以编程方式控制和自动化网页浏览器。它旨在用于端到端测试、网页抓取以及跨各种网页浏览器（如 `Chromium`、`Firefox` 和 `WebKit`）的自动化任务。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\playwright.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
