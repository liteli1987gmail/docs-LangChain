...
> [Apify Actors](https://docs.apify.com/platform/actors) 是专为各种网页抓取、爬虫和数据提取任务设计的云端程序。这些 Actors 能够自动化从网络收集数据，使用户能够高效地提取、处理和存储信息。Actors 可用于执行诸如抓取电商网站的产品详情、监控价格变化或收集搜索引擎结果等任务。它们与 [Apify Datasets](https://docs.apify.com/platform/storage/dataset) 无缝集成，允许将 Actors 收集的结构化数据存储、管理并以 JSON、CSV 或 Excel 等格式导出，以便进一步分析或使用。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\apify_actors.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
