...
> [Golden](https://golden.com) 提供了一套自然 language APIs，用于通过 Golden Knowledge Graph 进行查询和丰富数据，例如：`Products from OpenAI`、`Generative ai companies with series a funding` 和 `rappers who invest` 等查询可用于检索相关实体的结构化数据。
>
> `golden-query` LangChain tool 是 [Golden Query API](https://docs.golden.com/reference/query-api) 的包装器，可实现对这些结果的编程访问。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\golden_query.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
