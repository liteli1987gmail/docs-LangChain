...
# Zapier Natural Language Actions (NLA) 概述

## 什么是 Zapier NLA？

Zapier NLA 是一个让 LLM（大语言模型）通过自然语言调用超过 **5000+ 应用** 和 **20000+ 操作** 的 API 接口。

## 核心优势

| 特点 | 说明 |
|------|------|
| **无需 API 集成** | 不需要为每个应用单独开发 API 集成 |
| **自然语言交互** | 用自然语言描述即可执行操作 |
| **简化输出** | 返回格式化的、LLM 友好的结果 |
| **OAuth 认证** | 类似 OAuth 的设置流程，安全可靠 |
...
```
用户/AI → 自然语言请求 → Zapier NLA API → 底层 API 调用 → 格式化返回
```

## 典型使用场景

1. **邮件自动化**：让 AI 帮你发送邮件
2. **任务管理**：创建 Trello 卡片或 Asana 任务
3. **数据同步**：在 Google Sheets 中读写数据
4. **通知推送**：向 Slack 发送消息

## 如何使用？

1. 在 Zapier 中配置需要暴露的操作（通过 OAuth-like 设置）
2. 通过 REST API 查询和执行这些操作
3. 获取标准化的返回结果

---

如果你正在开发需要调用多个外部服务的 AI 应用，Zapier NLA 是一个值得考虑的解决方案，可以大大减少 API 集成的工作量。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\zapier.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
