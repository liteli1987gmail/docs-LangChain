...
> [PromptLayer](https://docs.promptlayer.com/introduction) 是一个用于提示工程的平台。它还能帮助实现 LLM 可观测性，可视化请求、对提示进行版本控制以及跟踪使用情况。
>
> 虽然 `PromptLayer` 确实有直接与 LangChain 集成的 LLM（例如 [`PromptLayerOpenAI`](/oss/python/integrations/llms/promptlayer_openai)），但使用 callback 是与 LangChain 集成的推荐方式。
