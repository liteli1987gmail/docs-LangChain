...
Argilla 是一个用于 LLM 的开源数据策划平台。
使用 Argilla，每个人都可以通过更快的数据策划（利用人工和机器反馈）来构建强大的语言模型。我们为 MLOps 周期中的每个步骤提供支持，从数据标注到模型监控。
...
📌 注意：目前，`FeedbackDataset.fields` 仅支持 prompt-response 对，因此 `ArgillaCallbackHandler` 只会跟踪 prompt（即 LLM input）和 response（即 LLM output）。
...
请注意，在此场景中我们将使用 Google Search API (Serp API)，因此您需要同时安装 `google-search-results`（使用 `pip install google-search-results`），并设置 Serp API Key 为 `os.environ["SERPAPI_API_KEY"] = "..."`（您可以在 [serpapi.com/dashboard](https://serpapi.com/dashboard) 找到它），否则下面的示例将无法运行。
