...
> [AskNews](https://asknews.app) 通过单一的自然语言查询为任何 LLM 提供最新的全球新闻（或历史新闻）。具体来说，AskNews 每天通过翻译、摘要、实体提取并将文章索引到热存储和冷存储向量数据库来丰富超过 30 万篇文章。AskNews 将这些向量数据库部署在低延迟的终端节点上。当您查询 AskNews 时，您会收到一个经过提示优化的字符串，其中包含所有最相关的丰富内容（例如实体、分类、翻译、摘要）。这意味着您不需要管理自己的新闻 RAG，也不需要担心如何以简洁的方式向您的 LLM 传递新闻信息。

> AskNews 还致力于透明度，这也是我们的覆盖范围受到监控并覆盖数百个国家、13 种语言和 5 万个来源的原因。如果您想追踪我们的来源覆盖情况，可以访问我们的[透明度仪表盘](https://asknews.app/en/transparency)。
