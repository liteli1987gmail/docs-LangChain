...
[Google Vertex AI Search](https://cloud.google.com/enterprise-search)（前身为 `Generative AI App Builder` 上的 `Enterprise Search`）是 [Vertex AI](https://cloud.google.com/vertex-ai) 机器学习平台的一部分。

`Vertex AI Search` 允许组织快速为客户和员工构建由生成式 AI 驱动的搜索引擎。它依托于多种 `Google Search` 技术，包括语义搜索（semantic search），该技术通过使用自然语言处理和机器学习技术来推断内容中的关系并理解用户查询意图，从而帮助提供比传统基于关键词的搜索技术更相关的结果。Vertex AI Search 还受益于 Google 在理解用户搜索行为方面的专业知识，并将内容相关性纳入搜索结果的排序考量中。

`Vertex AI Search` 在 `Google Cloud Console` 中可用，也可通过 `API` 用于企业工作流集成。
