...
[LLMLingua](https://github.com/microsoft/LLMLingua) 利用紧凑且训练良好的语言模型（例如 GPT2-small、LLaMA-7B）来识别并移除提示中的非必要 token。这种方法能够在使用大型语言模型（LLM）时实现高效推理，达到高达 **20 倍压缩**，同时性能损失极小。
