...
<Tip>
    For simpler use cases, you can also use [prompt caching](/oss/集成s/chat/anthropic#prompt-caching) on the chat model by passing `cache_control` at invocation time without middleware. 该 middleware is recommended when you need explicit control over cache breakpoints on system prompts and tool definitions.
</Tip>
