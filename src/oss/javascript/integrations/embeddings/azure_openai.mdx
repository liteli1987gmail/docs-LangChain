...
嵌入模型s are often used in retrieval-augmented generation (RAG) flows, both as part of indexing data as well as later retrieving it. For more detailed instructions, please 请参阅 our RAG tutorials under the [**Learn** tab](/oss/learn/).

Below, 请参阅 how to index and retrieve data using the `embeddings` object we initialized above. In this 示例, we will index and retrieve a sample document using the demo [`MemoryVectorStore`](/oss/集成s/vectorstores/memory).
