...
Please use the new [Azure DocumentDB](/oss/集成s/vectorstores/azure_documentdb) 集成 instead. 该 old class names (`AzureCosmosDBMongoDBVectorStore`, etc.) are still available as deprecated aliases for backwards compatibility, but you should migrate to the new `AzureDocumentDBVectorStore` class.
</Warning>
