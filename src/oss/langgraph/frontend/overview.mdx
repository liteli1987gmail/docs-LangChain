...
[LangChain 前端模式](/oss/langchain/frontend/overview)——markdown 消息、工具调用、乐观更新等——可与任何 LangGraph 图一起工作。无论您使用 `createAgent`、`createDeepAgent` 还是自定义 `StateGraph`，`useStream` 钩子都提供相同的核心 API。