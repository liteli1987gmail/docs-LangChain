---
title: 以编程方式使用文档
description: 将 LangChain 文档连接到您的 AI 工具和工作流程
---

我们希望使文档尽可能易于访问。我们提供了多种方式让您通过 AI 助手、代码编辑器和直接集成（如模型上下文协议（MCP））以编程方式使用这些文档。

## 快速访问选项

在我们文档的任何页面，您都可以在右上角找到一个上下文菜单下拉列表：
...
我们的文档包含一个内置的**模型上下文协议（MCP）服务器**，让 AI 应用程序能够实时查询最新文档。

LangChain 文档 MCP 服务器地址为：

```txt
https://docs.langchain.com/mcp
```

连接后，您可以向 AI 助手询问有关 LangChain、LangGraph 和 LangSmith 的问题，它会搜索我们的文档以提供准确、最新的答案。

### 连接 Claude Code

如果您使用的是 Claude Code，请在终端中运行此命令以将服务器添加到当前项目：
...
<Note>
    **项目（本地）范围**
    ...
    上述命令仅将 MCP 服务器添加到您当前的项目/工作目录。要全局添加 MCP 服务器并使其可在所有项目中使用，请通过添加 `--scope user` 到命令来添加用户范围：
...
如果您使用的是 OpenAI Codex CLI，请在终端中运行此命令以全局添加服务器：
