...
Certain model providers offer endpoints that are compatible with OpenAI's [Chat Completions 接口](https://platform.openai.com/docs/api-reference/chat). In such cases, you can use [`ChatOpenAI`](/oss/integrations/chat/openai) with a custom `base_url` to connect to these endpoints for basic chat functionality.
