...
This guide provides a quick overview for getting started with the (MODULE_NAME) [chat model](/oss/langchain/models). For a detailed listing of all Chat(MODULE_NAME) features, paramaters, and configurations, head to the [Chat(MODULE_NAME) 接口 reference](https://reference.langchain.com/python/langchain-(MODULE_NAME)/chat_models/Chat(MODULE_NAME)).
...
For detailed documentation of all Chat(MODULE_NAME) features and configurations, head to the [接口 reference](<LINK>).
