...
>[Azure Machine Learning](https://azure.microsoft.com/en-us/products/machine-learning/) 是一个用于构建、训练和部署机器学习模型的平台。用户可以在 Model Catalog 中浏览可部署的模型类型，这里提供了来自不同提供商的基础模型和通用模型。

>通常，您需要部署模型以使用其预测结果（推理）。在 Azure Machine Learning 中，Online Endpoints 用于部署这些模型以实现实时服务。它们基于 `Endpoints` 和 `Deployments` 的概念，允许您将生产工作负载的接口与为其提供服务的实现分离。
