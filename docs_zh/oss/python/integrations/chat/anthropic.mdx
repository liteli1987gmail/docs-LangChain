...
You can count tokens in messages before sending them to the model using @[`get_num_tokens_from_messages()`][ChatAnthropic.get_num_tokens_from_messages]. This uses Anthropic's official [token counting 接口](https://platform.claude.com/docs/en/build-with-claude/token-counting).
