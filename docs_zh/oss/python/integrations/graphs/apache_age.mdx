...
[Apache AGE](https://age.apache.org/) 是一个 PostgreSQL 扩展，提供图数据库功能。AGE 是 A Graph Extension 的缩写，受 Bitnine 的 PostgreSQL 10 分支 AgensGraph 启发，后者是一个多模型数据库。该项目的目标是创建单一存储，能够同时处理关系模型和图模型数据，让用户能够使用标准 ANSI SQL 以及 openCypher（图查询语言）。Apache AGE 存储的数据元素包括节点、连接节点的边，以及节点和边的属性。


本笔记本展示了如何使用 LLM 为图数据库提供自然语言接口，你可以通过 Cypher 查询语言对该图数据库进行查询。

[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式图查询语言，能够在属性图中实现表达性强且高效的数据查询。
