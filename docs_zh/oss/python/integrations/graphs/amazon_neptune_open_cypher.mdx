...
[亚马逊Neptune](https://aws.amazon.com/neptune/) 是一个高性能图分析和无服务器数据库，可提供卓越的可扩展性和可用性。

此示例展示了使用 `openCypher` 查询 `Neptune` 图数据库的 QA 链，并返回人类可读的响应。

[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式图查询语言，允许在属性图中进行表达性强的数据查询。
