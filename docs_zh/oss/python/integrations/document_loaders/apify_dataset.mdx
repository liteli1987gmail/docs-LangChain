...
Apify Dataset 是一个可扩展的仅追加（append-only）存储，具有顺序访问功能，专为存储结构化的网页抓取结果而构建，例如产品列表或 Google SERPs，然后可以将其导出为 JSON、CSV 或 Excel 等各种格式。Datasets 主要用于保存 Apify Actors 的结果——用于各种 web scraping、crawling 和数据提取用例的无服务器（serverless）云程序。
