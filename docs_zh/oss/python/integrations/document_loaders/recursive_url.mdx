...
You can similarly pass in a `metadata_extractor` to customize how Document metadata is extracted from the HTTP response. See the [接口 reference](https://reference.langchain.com/python/langchain-community/document_loaders/recursive_url_loader/RecursiveUrlLoader) for more on this.
...
For detailed information on configuring and calling the `RecursiveUrlLoader`, please see the [接口 reference](https://reference.langchain.com/python/langchain-community/document_loaders/recursive_url_loader/RecursiveUrlLoader).
