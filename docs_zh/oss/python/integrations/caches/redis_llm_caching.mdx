...
本笔记本演示如何使用 langchain-redis 包中的 `RedisCache` 和 `RedisSemanticCache` 类来实现 LLM 响应的缓存。
...
确保你有一个正在运行的 Redis 服务器。你可以使用 Docker 启动一个，命令如下：
...
本笔记本演示了 langchain-redis 包中 `RedisCache` 和 `RedisSemanticCache` 的用法。这些缓存机制可以通过减少冗余 API 调用并利用语义相似性实现智能缓存，从而显著提升基于 LLM 的应用程序性能。基于 Redis 的实现为分布式系统中的缓存提供了一种快速、可扩展且灵活的解决方案。
