...
[Hyperbrowser](https://hyperbrowser.ai) 是一个用于运行和扩展无头浏览器的平台。它可以让你快速启动和管理大规模浏览器会话，并为各种网页抓取需求提供易于使用的解决方案，例如抓取单个页面或爬取整个网站。

主要功能：

- 即时扩展 - 无需担心基础设施，即可在数秒内启动数百个浏览器会话
- 简单集成 - 与 Puppeteer 和 Playwright 等流行工具无缝配合
- 强大的 API - 易于使用的 API，可用于抓取/爬取任何网站，以及更多功能
- 绕过反爬虫机制 - 内置隐身模式、广告拦截、自动 CAPTCHA 解决和轮换代理
