...
PebbloRetrievalQA 是一种具有身份与语义强制的检索链，用于问答。
...
> **注意：** 建议使用 [PebbloSafeLoader](https://daxa-ai.github.io/pebblo/rag) 作为在摄取端加载带有身份验证和语义元数据文档的对应工具。`PebbloSafeLoader` 确保文档的安全高效加载，同时保持元数据的完整性。
