...
Head to [Google AI Studio](https://aistudio.google.com/apikey) to sign up and generate an API key. See the [Gemini 接口 keys documentation](https://ai.google.dev/gemini-api/docs/api-key) for more details. Once you've done this set the `GOOGLE_API_KEY` environment variable:
...
For detailed documentation on `GoogleGenerativeAIEmbeddings` features and configuration options, please refer to the [接口 reference](https://reference.langchain.com/python/langchain-google-genai/embeddings/GoogleGenerativeAIEmbeddings).