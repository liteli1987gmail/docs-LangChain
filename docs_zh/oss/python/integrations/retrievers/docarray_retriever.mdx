...
[DocArray](https://github.com/docarray/docarray) 是一个功能强大、开源的多模态数据管理工具。它让你可以自由地构建数据结构，并提供了使用多种文档索引后端进行存储和搜索的灵活性。更棒的是，你可以利用 DocArray 文档索引创建 DocArrayRetriever，从而构建出色的 LangChain 应用！
