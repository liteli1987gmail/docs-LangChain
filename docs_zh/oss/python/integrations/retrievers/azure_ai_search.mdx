...
+ An API key or Azure AD Token.
  + API keys are generated when you create the search service. If you're just querying an index, you can use the query API key, otherwise use an admin API key. See [Find your 接口 keys](https://learn.microsoft.com/azure/search/search-security-api-keys?tabs=rest-use%2Cportal-find%2Cportal-query#find-existing-keys) for details.
...
For detailed documentation of all `AzureAISearchRetriever` features and configurations head to the [接口 reference](https://reference.langchain.com/python/langchain-community/retrievers/azure_ai_search/AzureAISearchRetriever).
