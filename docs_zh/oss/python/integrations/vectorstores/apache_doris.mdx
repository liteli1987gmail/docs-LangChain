...
[Apache Doris](https://doris.apache.org/) 是一个用于实时分析的新一代数据仓库。
It delivers lightning-fast analytics on real-time data at scale.

通常情况下，`Apache Doris` 被归类为 OLAP，并且在 [ClickBench — a Benchmark For Analytical DBMS](https://benchmark.clickhouse.com/) 中展现了出色的性能。由于它拥有超快的向量化执行引擎，它也可以被用作快速的 vectordb。
