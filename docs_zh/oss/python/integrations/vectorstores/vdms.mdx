...
英特尔的 Visual Data Management System (VDMS) 是一种用于高效访问大规模"可视化"数据的存储解决方案，旨在通过以图形形式存储的视觉元数据来搜索相关视觉数据，实现云规模，并提供机器友好的视觉数据增强功能以实现更快访问。VDMS 采用 MIT 许可证。有关 VDMS 的更多信息，请访问 [VDMS wiki](https://github.com/IntelLabs/vdms/wiki)。
