...
[Supabase](https://supabase.com/docs) 是一个开源的 Firebase 替代方案。`Supabase` 构建于 `PostgreSQL` 之上，提供强大的 SQL 查询能力，并能够与现有工具和框架实现简单集成。

PostgreSQL，也称为 Postgres，是一个免费的开源关系型数据库管理系统（RDBMS），强调可扩展性和 SQL 兼容性。
