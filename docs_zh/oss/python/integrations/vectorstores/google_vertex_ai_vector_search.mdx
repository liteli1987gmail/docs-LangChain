...
> [Google Vertex AI Vector Search](https://cloud.google.com/vertex-ai/docs/vector-search/overview)（原名 Vertex AI Matching Engine）提供了业界领先的高规模低延迟 vector database。这类 vector database 通常被称为 vector similarity-matching 或 approximate nearest neighbor（ANN）服务。

**Note**: LangChain API expects an endpoint and deployed index already created.Index creation time can take upto one hour.

要查看如何创建索引，请参阅章节 [Create Index and deploy it to an Endpoint]。
...
批量索引（batch index）适用于需要定期批量更新索引的场景，数据在一段时间内累积，例如每周或每月处理一次的系统。流式索引（streaming index）则适用于希望索引数据在新数据添加到 datastore 时立即更新的场景，例如如果你经营一家书店，想要尽快在线展示新库存。选择哪种类型很重要，因为它们的设置和需求各不相同。
