...
[Transwarp Hippo](https://www.transwarp.cn/en/subproduct/hippo) 是一个企业级云原生分布式向量数据库，支持海量向量数据的存储、检索和管理。它能高效解决向量相似性搜索和高密度向量聚类等问题。Hippo 具有高可用性、高性能和易于扩展的特点。它具备多种功能，包括多种向量搜索索引、数据分区和分片、数据持久化、增量数据摄取、向量标量字段过滤和混合查询。它能有效满足企业对于海量向量数据的高实时性搜索需求。
