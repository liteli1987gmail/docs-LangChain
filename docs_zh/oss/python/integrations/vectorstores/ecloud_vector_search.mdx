...
>[中国移动云Elasticsearch VectorSearch](https://ecloud.10086.cn/portal/product/elasticsearch) 是一款全托管的企业级分布式搜索与分析服务。中国移动云Elasticsearch VectorSearch 为结构化/非结构化数据提供低成本、高性能、可靠的检索与分析平台级产品服务。作为一个 vector database，它支持多种 index types 和 similarity distance methods。
