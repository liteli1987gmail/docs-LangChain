...
[SQLite-Vec](https://alexgarcia.xyz/sqlite-vec/) 是一个专为向量搜索设计的 SQLite 扩展，强调本地优先操作和无外部服务器的应用集成。它是同一作者 [SQLite-VSS](https://alexgarcia.xyz/sqlite-vss/) 的后继者。它使用零依赖的 C 语言编写，设计为易于构建和使用。
