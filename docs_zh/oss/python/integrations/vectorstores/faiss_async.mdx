...
> [Facebook AI Similarity Search (Faiss)](https://engineering.fb.com/2017/03/29/data-infrastructure/faiss-a-library-for-efficient-similarity-search/) 是一个用于高效相似性搜索和稠密向量聚类的库。它包含的算法可以在任意大小的向量集上进行搜索，甚至可以处理可能无法全部放入RAM的向量集。它还包含用于评估和参数调优的支持代码。
>
