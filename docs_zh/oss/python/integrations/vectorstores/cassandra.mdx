...
> [Cassandra](https://cassandra.apache.org/) 是一个 NoSQL、行式存储、高可扩展性、高可用性的数据库。从 5.0 版本开始，该数据库自带[向量搜索功能](https://cassandra.apache.org/doc/trunk/cassandra/vector-search/overview.html)。
...
DataStax [Astra DB](https://docs.datastax.com/en/astra-serverless/docs/vector-search/quickstart.html) 是一个托管的无服务器数据库，基于 Cassandra 构建，提供相同的接口和优势。
...
Apache Cassandra、Cassandra 和 Apache 是 Apache Software Foundation 在美国和/或其他国家的注册商标。
