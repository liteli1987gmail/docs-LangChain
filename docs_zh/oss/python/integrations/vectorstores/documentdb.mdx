...
>[Amazon DocumentDB（与 MongoDB 兼容）](https://docs.aws.amazon.com/documentdb/)让您可以轻松地在云中设置、操作和扩展与 MongoDB 兼容的数据库。
> 使用 Amazon DocumentDB，您可以运行相同的应用程序代码，并使用与 MongoDB 相同的驱动和工具。
> Amazon DocumentDB 的向量搜索将基于 JSON 的文档数据库的灵活性与强大的查询能力，和向量搜索的强大功能相结合。
