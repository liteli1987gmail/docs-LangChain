...
本页介绍**同步**子代理，即主管阻塞直到子代理完成。对于长时间运行的任务、并行工作流或需要中途控制和取消的情况，请参阅[异步子代理](/oss/deepagents/async-subagents)。
...
- 要重命名或重新提示自动添加的版本，请在活动的[工具配置文件](/oss/deepagents/profiles#harness-profiles)上设置 `general_purpose_subagent=GeneralPurposeSubagentProfile(...)`。
- 要完全删除它，请在活动的工具配置文件上设置 `general_purpose_subagent=GeneralPurposeSubagentProfile(enabled=False)`。

如果此后没有保留同步子代理，深度代理不会添加 `task` 工具。此行为仅影响同步子代理。异步子代理仍使用[异步子代理](/oss/deepagents/async-subagents)中所述的异步任务工具。

### SubAgent（基于字典）

对于大多数用例，将子代理定义为与 @[`SubAgent`] 规范匹配的字典，具有以下字段：
...
| `model` | `str` \| `BaseChatModel` | 可选。覆盖主代理的模型。省略则使用主代理的模型。<br></br>默认继承自主代理。您可以传递模型标识符字符串如 `'openai:gpt-5.4'`（使用 `'provider:model'` 格式）或 LangChain 聊天模型对象（`init_chat_model("gpt-5.4")` 或 `ChatOpenAI(model="gpt-5.4")`）。 |
| `middleware` | `list[Middleware]` | 可选。用于自定义行为、日志记录或速率限制的其他中间件。<br></br>不继承自主代理。 |
| `interrupt_on` | `dict[str, bool]` | 可选。为特定工具配置[人工介入](/oss/deepagents/human-in-the-loop)。子代理值覆盖主代理。需要检查点器。<br></br>默认继承自主代理。子代理值覆盖默认值。 |
| `skills` | `list[str]` | 可选。[技能](/oss/deepagents/skills)源路径。指定后，子代理将从这些目录加载技能（例如 `["/skills/research/", "/skills/web-search/"]`）。这允许子代理拥有与主代理不同的技能集。<br></br>不继承自主代理。只有通用子代理继承主代理的技能。当子代理有技能时，它运行自己独立的 @[`SkillsMiddleware`] 实例。技能状态完全隔离——子代理的加载技能对父级不可见，反之亦然。 |
| `response_format` | `ResponseFormat` | 可选。子代理的[结构化输出](/oss/langchain/structured-output)模式。设置后，父代理将子代理的结果作为 JSON 接收，而不是自由格式文本。接受 Pydantic 模型、`ToolStrategy(...)`、`ProviderStrategy(...)` 或原始模式类型。请参阅[结构化输出](#structured-output)。 |
| `permissions` | `list[FilesystemPermission]` | 可选。子代理的[文件系统权限规则](/oss/deepagents/permissions)。设置后，**完全替换**父代理的权限。<br></br>默认继承自主代理。 |
...
| `interrupt_on` | `dict[str, bool]` | 可选。为特定工具配置[人工介入](/oss/deepagents/human-in-the-loop)。子代理值覆盖主代理。需要检查点器。<br></br>默认继承自主代理。子代理值覆盖默认值。 |
| `skills` | `list[str]` | 可选。[技能](/oss/deepagents/skills)源路径。指定后，子代理将从这些目录加载技能（例如 `["/skills/research/", "/skills/web-search/"]`）。这允许子代理拥有与主代理不同的技能集。<br></br>不继承自主代理。只有通用子代理继承主代理的技能。当子代理有技能时，它运行自己独立的 @[`SkillsMiddleware`] 实例。技能状态完全隔离——子代理的加载技能对父级不可见，反之亦然。 |
| `responseFormat` | `ResponseFormat` | 可选。子代理的[结构化输出](/oss/langchain/structured-output)模式。设置后，父代理将子代理的结果作为 JSON 接收，而不是自由格式文本。接受 Zod 模式、JSON 模式对象、`toolStrategy(...)` 或 `providerStrategy(...)`。请参阅[结构化输出](#structured-output)。 |

:::


<Tip>
    **CLI 用户：**您还可以将子代理定义为磁盘上的 `AGENTS.md` 文件，而不是在代码中。`name`、`description` 和 `model` 字段映射到 YAML frontmatter，markdown 正文成为 `system_prompt`。请参阅[自定义子代理](/oss/deepagents/cli/overview#subagents)了解文件格式。

    **部署用户：**将子代理定义为 `subagents/` 下的目录，它们自己的 `deepagents.toml` 和 `AGENTS.md`。捆绑器自动发现它们。请参阅[部署子代理](/oss/deepagents/deploy#subagents)获取完整的配置参考。
</Tip>
...
您可以使用 LangChain 的 @[`create_agent`] 创建自定义子代理，或使用[图 API](/oss/langgraph/graph-api)制作自定义 LangGraph 图。

如果您正在创建自定义 LangGraph 图，请确保图有一个[名为 "messages" 的状态键](/oss/langgraph/quickstart#2-define-state)：
...
子代理支持[结构化输出](/oss/langchain/structured-output)，因此父代理接收可预测的、可解析的 JSON 而不是自由格式文本。
...
在子代理配置上传递 `response_format`。当子代理完成时，其结构化响应被 JSON 序列化并作为 `ToolMessage` 内容返回给父代理。模式接受 @[`create_agent`] 支持的任何内容：Pydantic 模型、`ToolStrategy(...)`、`ProviderStrategy(...)` 或原始模式类型。
...
class ResearchFindings(BaseModel):
    """研究任务的结构化发现。"""
    summary: str = Field(description="发现的摘要")
...
有关模式类型和策略（工具调用 vs 提供商原生）的完整详细信息，请参阅[结构化输出](/oss/langchain/structured-output)。
...
email_agent = {
    "name": "email-sender",
    "tools": [send_email, validate_email],  # 只包含电子邮件相关
}
...
agent = create_deep_agent(
    model="google_genai:gemini-3.1-pro-preview",
    system_prompt="你协调数据分析和平面报告。使用子代理进行专门任务。",
    subagents=subagents
)
```
:::
...
当您使用[运行时上下文](/oss/langchain/runtime)调用父代理时，该上下文自动传播到所有子代理。每次子代理运行接收与您在父 `invoke` / `ainvoke` 调用上传递的相同的运行时上下文。
...
result = await agent.invoke(
    {"messages": [HumanMessage("研究此内容并验证声明")]},
    context=Context(
        user_id="user-123",
