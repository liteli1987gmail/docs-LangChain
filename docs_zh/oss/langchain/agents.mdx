...
代理将语言模型与[工具](/oss/langchain/tools)结合，创建能够推理任务、决定使用哪些工具并迭代地朝着解决方案工作的系统。

:::python
@[`create_agent`] 提供了一个可用于生产的代理实现。
:::
:::js
...
:::python
@[`create_agent`] 使用 [LangGraph](/oss/langgraph/overview) 构建基于**图**的代理运行时。图由节点（步骤）和边（连接）组成，定义您的代理如何处理信息。代理通过此图移动，执行节点如模型节点（调用模型）、工具节点（执行工具）或中间件。
:::
:::js
`createAgent()` 使用 [LangGraph](/oss/langgraph/overview) 构建基于**图**的代理运行时。图由节点（步骤）和边（连接）组成，定义您的代理如何处理信息。代理通过此图移动，执行节点如模型节点（调用模型）、工具节点（执行工具）或中间件。
:::

详细了解[图 API](/oss/langgraph/graph-api)。
...
[模型](/oss/langchain/models) 是您代理的推理引擎。它可以通过多种方式指定，支持静态和动态模型选择。
...
:::python
<Tip>
    模型标识符字符串支持自动推断（例如，"gpt-5.4" 将被推断为 "openai:gpt-5.4"）。请参阅 @[参考][init_chat_model(model)] 查看模型标识符字符串映射的完整列表。
</Tip>

要对模型配置进行更多控制，请直接使用 provider 包初始化模型实例。在本例中，我们使用 @[`ChatOpenAI`]。请参阅[聊天模型](/oss/integrations/chat)查看其他可用的聊天模型类。
...
模型实例为您提供配置的完全控制。当您需要设置特定[参数](/oss/langchain/models#parameters)（如 `temperature`、`max_tokens`、`timeouts`、`base_url`）和其他 provider 特定设置时使用它们。请参阅[参考](/oss/integrations/providers/all_providers)查看模型上可用的参数和方法。
:::
:::js
...
模型实例为您提供配置的完全控制。当您需要设置特定参数（如 `temperature`、`max_tokens`、`timeouts`）或配置 API 密钥、`base_url` 和其他 provider 特定设置时使用它们。请参阅 [API 参考](/oss/integrations/providers/) 查看模型上可用的参数和方法。
:::
...
<Warning>
使用结构化输出时，不支持预绑定模型（已调用 @[`bind_tools`][BaseChatModel.bind_tools] 的模型）。如果您需要带有结构化输出的动态模型选择，请确保传递给中间件的模型不是预绑定的。
</Warning>
...
<Tip>
有关模型配置详细信息，请参阅[模型](/oss/langchain/models)。有关动态模型选择模式，请参阅[中间件中的动态模型](/oss/langchain/middleware#dynamic-model)。
</Tip>
...
[工具装饰器](/oss/langchain/tools#create-tools)可用于自定义工具名称、描述、参数模式和其他属性。
</Tip>
...
<Tabs>
  <Tab title="过滤预注册工具">
    ...
    有关更多示例，请参阅[动态选择工具](/oss/langchain/middleware/custom#dynamically-selecting-tools)。
...
:::python
您可以通过提供提示词来塑造代理处理任务的方式。@[`system_prompt`] 参数可以作为字符串提供：
:::
...
:::python
当没有提供 @[`system_prompt`] 时，代理将直接从消息中推断其任务。

@[`system_prompt`] 参数接受 `str` 或 @[`SystemMessage`]。使用 `SystemMessage` 使您可以更好地控制提示词结构，这对于 provider 特定功能（如 [Anthropic 的提示词缓存](/oss/integrations/chat/anthropic#prompt-caching)）很有用：
...
`systemPrompt` 参数接受 `string` 或 `SystemMessage`。使用 `SystemMessage` 使您可以更好地控制提示词结构，这对于 provider 特定功能（如 [Anthropic 的提示词缓存](/oss/integrations/chat/anthropic#prompt-caching)）很有用：
...
对于需要根据运行时上下文或代理状态修改系统提示词的更高级用例，您可以使用[中间件](/oss/langchain/middleware)。

:::python

@[`@dynamic_prompt`] 装饰器创建根据模型请求生成系统提示词的中间件：
...
:::python
为代理设置一个可选的 @[`name`][create_agent(name)]。这在将代理作为子图添加到[多代理系统](/oss/langchain/multi-agent)时用作节点标识符：
...
:::
:::js
为代理设置一个可选的 `name`。这在将代理作为子图添加到[多代理系统](/oss/langchain/multi-agent)时用作节点标识符：
...
<Warning>
    代理名称首选 `snake_case`（例如，使用 `research_assistant` 而不是 `Research Assistant`）。某些模型提供商拒绝包含空格或特殊字符的名称并返回错误。仅使用字母数字字符、下划线和连字符可确保与所有提供商的兼容性。这同样适用于[工具名称](/oss/langchain/tools)。
</Warning>

## 调用

:::python
您可以通过传递对其 [`State`](/oss/langgraph/graph-api#state) 的更新来调用代理。所有代理都在其状态中包含[一系列消息](/oss/langgraph/use-graph-api#messagesstate)；要调用代理，请传递一条新消息：
:::
:::js
您可以通过传递对其 [`State`](/oss/langgraph/graph-api#state) 的更新来调用代理。所有代理都在其状态中包含[一系列消息](/oss/langgraph/use-graph-api#messagesvalue)；要调用代理，请传递一条新消息：
:::
...
有关从代理流式传输步骤和/或标记的信息，请参阅[流式处理](/oss/langchain/streaming)指南。

否则，代理遵循 LangGraph [图 API](/oss/langgraph/use-graph-api)并支持所有相关方法，如 `stream` 和 `invoke`。
...
在某些情况下，您可能希望代理以特定格式返回输出。LangChain 通过 @[`response_format`][create_agent(response_format)] 参数提供结构化输出策略。
...
存储在状态中的信息可以被认为是代理的[短期记忆](/oss/langchain/short-term-memory)：
...
定义自定义状态有两种方法：
1. 通过[中间件](/oss/langchain/middleware)（首选）
...
<Note>
从 `langchain 1.0` 开始，自定义状态模式**必须是** `TypedDict` 类型。不再支持 Pydantic 模型和数据类。请参阅 [v1 迁移指南](/oss/migrate/langchain-v1#state-type-restrictions) 了解更多详细信息。
</Note>
...
:::python
<Note>
    通过中间件定义自定义状态比通过 @[`state_schema`] 在 @[`create_agent`] 上定义更可取，因为它允许您将状态扩展概念上保持在相关中间件和工具的范围内。

    @[`state_schema`] 仍受支持以保持向后兼容。
</Note>
:::

<Tip>
要了解更多关于内存的信息，请参阅[内存](/oss/concepts/memory)。有关跨会话持久化的长期内存实现的信息，请参阅[长期记忆](/oss/langchain/long-term-memory)。
</Tip>
...
<Tip>
有关流式处理的更多详细信息，请参阅[流式处理](/oss/langchain/streaming)。
</Tip>

### 中间件

[中间件](/oss/langchain/middleware)为在执行的不同阶段自定义代理行为提供了强大的扩展性。您可以使用中间件来：
