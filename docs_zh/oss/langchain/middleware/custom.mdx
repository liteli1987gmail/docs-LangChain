...
- **Wrap 风格钩子**（`wrap_model_call`、`wrap_tool_call`）：对于模型调用，返回带有 @[`Command`] 的 @[`ExtendedModelResponse`] 来注入状态更新以及模型响应。对于工具调用，直接返回 @[`Command`]。当您需要根据在模型或工具调用期间运行的逻辑（如摘要触发点、使用元数据或从请求/响应计算的自定义字段）来跟踪或更新状态时，使用这些。
:::

:::js
- **Node 风格钩子**（`beforeAgent`、`beforeModel`、`afterModel`、`afterAgent`）：直接返回字典。使用图的 reducers 将字典应用到代理状态。
- **Wrap 风格钩子**（`wrapModelCall`、`wrapToolCall`）：对于模型调用，直接返回 @[`Command`] 来注入来自模型调用层的状态更新。对于工具调用，直接返回 @[`Command`]。当您需要根据在模型或工具调用期间运行的逻辑（如摘要触发点、使用元数据或从请求/响应计算的自定义字段）来跟踪或更新状态时，使用这些。
:::
...
@[`Command`] 通过图的 reducers 流动，因此更新被正确应用，消息是附加的而不是替换现有状态。
...
使用 `ModelRequest` 上的 `system_message` 字段来读取和修改系统提示词。它包含一个 @[`SystemMessage`] 对象（即使代理是用字符串 `system_prompt` 创建的）。
...
- 对于像缓存控制这样的高级用例，您可以直接将 @[`SystemMessage`] 对象传递给 `create_agent` 的 `system_prompt` 参数
</Note>
...
使用 `ModelRequest` 中的 `systemMessage` 字段来读取和修改系统提示词。它包含一个 @[`SystemMessage`] 对象（即使代理是用字符串 @[`systemPrompt`] 创建的）。
...
在运行时选择相关工具以提高性能和准确性。本节介绍过滤预注册工具的方法。有关在运行时发现的工具注册（例如来自 MCP 服务器），请参阅[运行时工具注册](/oss/langchain/agents#dynamic-tools)。
...
:::js
在中间件中使用 `ModelRequest` 中的 `systemMessage` 字段修改系统提示词。它包含一个 @[`SystemMessage`] 对象（即使代理是用字符串 @[`systemPrompt`] 创建的）。
