...
从[任何支持的模型提供商](/oss/integrations/providers/overview)（例如 Google Gemini 或 OpenAI）获取 API 密钥。
...
```
  ...
  <Tab title="其他">
    请参阅支持的[聊天模型集成](/oss/integrations/chat)的完整列表。
  </Tab>
</Tabs>
...
<Tip>
    您可以通过更改代码中的模型名称并设置适当的 API 密钥来使用[任何支持的模型](/oss/integrations/providers/overview)。
</Tip>
...
<Steps>
    ...
    <Step title="创建工具">
        [工具](/oss/langchain/tools) 允许模型通过调用您定义的函数来与外部系统交互。
        工具可以依赖于[运行时上下文](/oss/langchain/runtime)，也可以与[代理内存](/oss/langchain/short-term-memory)交互。
        ...
        <Tip>
            ...
            LangChain 的 @[`@tool` 装饰器][@tool] 添加元数据并启用带有 `ToolRuntime` 参数的运行时注入。
    ...
    <Step title="配置您的模型">
        使用正确的参数设置您的[语言模型](/oss/langchain/models)以适应您的用例。例如：
    ...
    <Step title="添加内存">
        ...
        为您的代理添加[内存](/oss/langchain/short-term-memory)以在交互之间保持状态。这允许
        代理记住之前的对话和上下文。
        ...
        <Info>
            ...
            有关更多详细信息，请参阅[添加和管理内存](/oss/langgraph/add-memory#manage-short-term-memory)。
        </Info>
...
- **LangChain 代理**：[添加和管理内存](/oss/langgraph/add-memory#manage-short-term-memory)、[部署到生产环境](/oss/langgraph/deploy)
- **Deep Agents**：[自定义选项](/oss/deepagents/customization)、[持久化内存](/oss/deepagents/long-term-memory)、[部署到生产环境](/oss/langgraph/deploy)