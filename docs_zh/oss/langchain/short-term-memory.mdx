...
记忆是一个记住以前交互信息的系统。对于 AI 代理来说，记忆至关重要，因为它让他们能够记住以前的交互、从反馈中学习并适应用户偏好。随着代理处理更复杂的任务和大量用户交互，这种能力对于效率和用户满意度都变得必不可少。

短期记忆让你的应用程序在单个线程或对话中记住以前的交互。

<Note>
    一个线程在会话中组织多个交互，类似于电子邮件将消息分组在单个对话中的方式。
</Note>

对话历史是短期记忆最常见的形式。长对话对当今的大型语言模型构成了挑战；完整的历史可能无法适应大型语言模型的上下文窗口，导致上下文丢失或错误。

即使你的模型支持完整的上下文长度，大多数大型语言模型在长上下文上仍然表现不佳。它们会被过时或离题的内容"分心"，同时还会因响应时间变慢和成本增加而受到影响。

聊天模型使用[消息](/oss/langchain/messages)接受上下文，其中包括指令（系统消息）和输入（人类消息）。在聊天应用中，消息在人类输入和模型响应之间交替，导致消息列表随着时间推移而增长。由于上下文窗口有限，许多应用可以从使用技术删除或"忘记"过时信息中受益。

<Tip>
需要**跨**对话记住信息？使用[长期记忆](/oss/langchain/long-term-memory)在不同的线程和会话之间存储和调用用户特定或应用级别的数据。
</Tip>
...
<Info>
    ...
    通过将这些存储在图形状态中，代理可以访问给定对话的完整上下文，同时保持不同线程之间的分离。
...
<Note>
    有关 SQLite、Postgres 和 Azure Cosmos DB 的更多检查点选项，请参阅持久化文档中的[检查点库列表](/oss/langgraph/persistence#checkpointer-libraries)。
</Note>
...
你可以扩展 @[`AgentState`] 以添加额外字段。自定义状态模式使用 @[`state_schema`] 参数传递给 @[`create_agent`]。
...
:::js
你可以通过创建具有状态模式的自定义中间件来扩展代理状态。自定义状态模式可以使用中间件中的 `stateSchema` 参数传递。最好使用 `StateSchema` 类进行状态定义（也支持纯 Zod 对象）。
...
启用[短期记忆](#usage)后，长对话可能会超过大型语言模型的上下文窗口。常见解决方案是：

<CardGroup cols={2}>
    <Card title="修剪消息" icon="scissors" href="#trim-messages" arrow>
        删除前或后 N 条消息（在调用大型语言模型之前）
    </Card>
...
这允许代理跟踪对话而不超过大型语言模型的上下文窗口。

### 修剪消息

大多数大型语言模型有一个最大支持的上下文窗口（以 token 为单位）。

:::python
决定何时截断消息的一种方法是计算消息历史中的 token 数量，并在接近该限制时进行截断。如果你使用 LangChain，你可以使用修剪消息工具并指定要保留的 token 数量以及用于处理边界 的 `strategy`（例如，保留最后 `max_tokens`）。
:::
:::js
决定何时截断消息的一种方法是计算消息历史中的 token 数量，并在接近该限制时进行截断。如果你使用 LangChain，你可以使用修剪消息工具并指定要保留的 token 数量以及用于处理边界的 `strategy`（例如，保留最后 `maxTokens`）。
:::
...
:::js
要在代理中修剪消息历史，请使用带有 `beforeModel` 钩子的 @[`createMiddleware`]：
...
你可以从图形状态中删除消息来管理消息历史。

当你想要删除特定消息或清除整个消息历史时，这很有用。

:::python
要从图形状态中删除消息，你可以使用 `RemoveMessage`。

要使 `RemoveMessage` 工作，你需要使用具有 @[`add_messages`] [reducer](/oss/langgraph/graph-api#reducers) 的状态键。
...
:::js
要从图形状态中删除消息，你可以使用 `RemoveMessage`。要使 `RemoveMessage` 工作，你需要使用具有 @[`messagesStateReducer`][messagesStateReducer] [reducer](/oss/langgraph/graph-api#reducers) 的状态键，如 `MessagesValue`。
...
<Warning>
    删除消息时，**确保** resulting 的消息历史是有效的。检查你使用的大型语言模型提供商的限制。例如：
...
如上所示，修剪或删除消息的问题在于你可能会因消除消息队列而丢失信息。正因为如此，一些应用受益于使用聊天模型对消息历史进行摘要的更复杂方法。

![摘要](/oss/images/summary.png)


:::python
要在代理中摘要消息历史，请使用内置的[``SummarizationMiddleware``](/oss/langchain/middleware#summarization)：
