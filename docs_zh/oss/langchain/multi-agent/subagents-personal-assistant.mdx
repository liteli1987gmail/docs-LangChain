...
**监督者模式**是一种[多代理](/oss/langchain/multi-agent)架构，其中中央监督者代理协调专门的 Worker 代理。这种方法在任务需要不同类型的专业知识时表现出色。与其构建一个跨领域管理工具选择的代理，不如创建由了解整体工作流程的监督者协调的专注专家。

在本教程中，您将构建一个通过真实工作流程展示这些优势的个人助手系统。该系统将协调两个职责根本不同的专家：

- 一个**日历代理**，负责处理日程安排、可用性检查和事件管理。
- 一个**邮件代理**，负责管理通信、起草消息和发送通知。

我们还将加入[人工介入审核](/oss/langchain/human-in-the-loop)，允许用户根据需要批准、编辑和拒绝操作（如外发邮件）。

### 为什么使用监督者？

多代理架构允许您将[工具](/oss/langchain/tools)分配给各个 Worker，每个 Worker 都有自己独立的提示词或指令。考虑一个直接访问所有日历和邮件 API 的代理：它必须从许多相似的工具中进行选择，理解每个 API 的确切格式，并同时处理多个领域。如果性能下降，将相关工具和相关提示词分组到逻辑单元可能会有帮助（部分是为了管理迭代改进）。
...
- [人工介入审核](/oss/langchain/human-in-the-loop)
...
首先定义需要结构化输入的工具。在实际应用中，这些会调用真实的 API（Google Calendar、SendGrid 等）。对于本教程，您将使用桩来演示该模式。
...
日历代理理解自然语言日程请求，并将其转换为精确的 API 调用。它处理日期解析、可用性检查和事件创建。
...
邮件代理处理消息撰写和发送。它专注于提取收件人信息、撰写适当的主题行和正文文本，以及管理邮件通信。
...
代理从非正式请求中推断出收件人，撰写专业的主题行和正文，调用 `send_email`，并返回确认。每个子代理都有狭窄的专注领域和特定领域的工具及提示词，使其能够出色地完成特定任务。
...
工具描述帮助监督者决定何时使用每个工具，因此要使其清晰且具体。我们只返回子代理的最终响应，因为监督者不需要看到中间推理或工具调用。

## 4. 创建监督者代理

现在创建协调子代理的监督者。监督者只看到高级工具，并在领域级别而非单个 API 级别做出路由决策。
...
<Tip>
要全面透明地了解信息流，包括每个聊天模型调用的提示词和响应，请查看上述运行的 [LangSmith trace](https://smith.langchain.com/public/91a9a95f-fba9-4e84-aff0-371861ad2f4a/r)。
</Tip>
...
监督者识别出这需要日历和邮件操作，为会议调用 `schedule_event`，然后为提醒调用 `manage_email`。每个子代理完成其任务，监督者将两个结果合成为一个连贯的响应。

<Tip>
请参阅 [LangSmith trace](https://smith.langchain.com/public/95cd00a3-d1f9-4dba-9731-7bf733fb6a3c/r) 查看上述运行的详细信息流，包括各个聊天模型的提示词和响应。
</Tip>
...
您的系统有三层。底层包含需要精确格式的严格 API 工具。中间层包含接受自然语言、将自然语言翻译为结构化 API 调用并返回自然语言确认的子代理。顶层包含路由到高级能力并综合结果的监督者。

这种关注点分离提供了几个好处：每一层都有专注的职责，您可以添加新领域而不影响现有领域，并且可以独立测试和迭代每一层。

## 6. 添加人工介入审核

对于敏感操作纳入[人工介入审核](/oss/langchain/human-in-the-loop)可能是明智的。LangChain 包含[内置中间件](/oss/langchain/human-in-the-loop#configuring-interrupts)来审核工具调用，在本例中是子代理调用的工具。

让我们为两个子代理添加人工介入审核：
- 我们将 `create_calendar_event` 和 `send_email` 工具配置为中断，允许所有[响应类型](/oss/langchain/human-in-the-loop)（`approve`、`edit`、`reject`）
- 我们仅向顶层代理添加一个[检查点](/oss/langchain/short-term-memory)**。这是暂停和恢复执行所必需的。
...
我们可以通过使用 @[`Command`] 引用其 ID 来为每个中断指定决策。有关更多详细信息，请参阅[人工介入指南](/oss/langchain/human-in-the-loop)。出于演示目的，这里我们将接受日历事件，但编辑出站邮件的主题：
...
默认情况下，子代理只从监督者那里接收请求字符串。您可能希望传递额外的上下文，例如对话历史或用户偏好。
...
这允许子代理看到完整的对话上下文，这对于解析"明天同一时间安排"等歧义很有用（引用之前的对话）。

<Tip>
您可以在 LangSmith trace 的[聊天模型调用](https://smith.langchain.com/public/c7d54882-afb8-4039-9c5a-4112d0f458b0/r/6803571e-af78-4c68-904a-ecf55771084d)中看到子代理收到的完整上下文。
</Tip>
...
**重要提示：** 确保子代理的提示词强调其最终消息应包含所有相关信息。常见的失败模式是执行工具调用但不将结果包含在最终响应中的子代理。

:::js
<Tip>
要查看完整的监督者模式工作示例，包括人工介入审核和高级信息流控制，请查看 LangChain.js 示例中的 [`supervisor_complete.ts`](https://github.com/langchain-ai/langchainjs/blob/main/examples/src/createAgent/supervisor.ts)。
</Tip>
:::

## 8. 关键要点

监督者模式创建了抽象层，其中每一层都有明确的职责。在设计监督者系统时，从清晰的领域边界开始，为每个子代理提供专注的工具和提示词。为监督者编写清晰的工具描述，在集成前独立测试每一层，并根据您的特定需求控制信息流。

<Tip>
**何时使用监督者模式**

当您有多个不同的领域（日历、邮件、CRM、数据库），每个领域有多个工具或复杂逻辑，您需要集中式工作流控制，且子代理不需要与用户直接对话时，请使用监督者模式。

对于只有几个工具的更简单情况，请使用单个代理。当代理需要与用户对话时，请改用[交接](/oss/langchain/multi-agent/handoffs)。对于代理之间的对等协作，请考虑其他多代理模式。
</Tip>

## 下一步

了解[交接](/oss/langchain/multi-agent/handoffs)以实现代理间对话，探索[上下文工程](/oss/langchain/context-engineering)以微调信息流，阅读[多代理概述](/oss/langchain/multi-agent)以比较不同模式，并使用 [LangSmith](https://smith.langchain.com) 调试和监控您的多代理系统。
