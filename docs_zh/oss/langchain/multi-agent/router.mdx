...
在**路由器**架构中，路由步骤对输入进行分类并将其定向到专门的[代理](/oss/langchain/agents)。当你有明确的**垂直领域**（需要各自代理的独立知识领域）时，这很有用。
...
当你有明确的垂直领域（需要各自代理的独立知识领域）、需要并行查询多个来源，并希望将结果合成为组合响应时，使用路由器模式。

## 基本实现

路由器对查询进行分类并将其定向到适当的代理。使用 [`Command`](/oss/langgraph/graph-api#command) 进行单代理路由，或使用 [`Send`](/oss/langgraph/graph-api#send) 并行向多个代理扇出。
...
构建一个并行查询 GitHub、Notion 和 Slack 的路由器，然后将结果合成为连贯答案。涵盖状态定义、专门代理、使用 `Send` 并行执行和结果合成。
</Card>
...
每个请求独立路由——调用之间没有记忆。关于多轮对话，请参阅[有状态路由器](#stateful)。

<Tip>
**路由器 vs. 子代理**：两种模式都可以将工作分派给多个代理，但它们在路由决策的方式上有所不同：

- **路由器**：专用路由步骤（通常是单个大型语言模型调用或基于规则的逻辑）对输入进行分类并分派给代理。路由器本身通常不维护对话历史或执行多轮编排——它是一个预处理步骤。
- **子代理**：主监督代理在 ongoing 对话中动态决定调用哪些[子代理](/oss/langchain/multi-agent/subagents)。主代理维护上下文，可以在多轮中调用多个子代理，并编排复杂的多步工作流程。

当你有明确的输入类别并想要确定性或轻量级分类时使用**路由器**。当你需要灵活的、对话感知的编排时使用**监督器**，其中大型语言模型根据不断发展的上下文决定下一步做什么。
</Tip>
...
如果你需要路由器本身维护状态，请使用[持久化](/oss/langchain/short-term-memory)存储消息历史。在路由到代理时，从状态中获取先前消息并有选择地将其包含在代理的上下文中——这是[上下文工程](/oss/langchain/context-engineering)的杠杆。

<Warning>
**有状态路由器需要自定义历史管理。** 如果路由器在跨轮次切换代理，对最终用户来说对话可能不会感觉流畅，因为代理具有不同的语气或提示词。使用并行调用时，你需要在路由器级别（输入和合成输出）维护历史，并在路由逻辑中利用此历史。考虑使用[交接模式](/oss/langchain/multi-agent/handoffs)或[子代理模式](/oss/langchain/multi-agent/subagents)——两者都为多轮对话提供了更清晰的语义。
</Warning>