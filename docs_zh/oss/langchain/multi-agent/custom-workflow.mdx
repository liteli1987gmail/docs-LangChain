...
在**自定义工作流**架构中，你使用 [LangGraph](/oss/langgraph/overview) 定义自己的定制执行流程。你对图结构拥有完全控制——包括顺序步骤、条件分支、循环和并行执行。
...
当标准模式（子代理、技能等）不适合你的需求时使用自定义工作流，你需要混合确定性逻辑与代理行为，或者你的用例需要复杂路由或多阶段处理。

工作流中的每个节点可以是简单的函数、大型语言模型调用或具有[工具](/oss/langchain/tools)的整个[代理](/oss/langchain/agents)。你也可以在自定义工作流中组合其他架构——例如，将多代理系统作为单个节点嵌入。
...
<Card
    ...
    arrow cta="了解更多"
>
    [路由器模式](/oss/langchain/multi-agent/router) 是自定义工作流的一个例子。本教程详细介绍构建一个路由器，该路由器并行查询 GitHub、Notion 和 Slack，然后将结果合成。
</Card>

## 基本实现

核心洞察是你可以在任何 LangGraph 节点内直接调用 LangChain 代理，将自定义工作流的灵活性与预构建代理的便利性相结合：
...
一个常见的用例是将[检索](/oss/langchain/retrieval)与代理相结合。本示例构建了一个 WNBA 统计助手，可以从知识库检索并获取实时新闻。

<Accordion title="自定义 RAG 工作流">

该工作流展示了三种类型的节点：

- **模型节点**（重写）：使用[结构化输出](/oss/langchain/structured-output)重写用户查询以获得更好的检索。
- **确定性节点**（检索）：执行向量相似性搜索——不涉及大型语言模型。
- **代理节点**（代理）：对检索到的上下文进行推理，并通过工具获取额外信息。
...
<Tip>
你可以使用 LangGraph 状态在工作流步骤之间传递信息。这允许工作流的每个部分读取和更新结构化字段，使得在节点之间共享数据和上下文变得容易。
</Tip>
