...
:::python
[模型上下文协议 (MCP)](https://modelcontextprotocol.io/introduction) 是一个开放协议，标准化了应用程序如何向大型语言模型提供工具和上下文。LangChain 代理可以使用 [`langchain-mcp-adapters`](https://github.com/langchain-ai/langchain-mcp-adapters) 库中 MCP 服务器上定义的工具。
:::
:::js
[模型上下文协议 (MCP)](https://modelcontextprotocol.io/introduction) 是一个开放协议，标准化了应用程序如何向大型语言模型提供工具和上下文。LangChain 代理可以使用 [`@langchain/mcp-adapters`](https://github.com/langchain-ai/langchainjs/tree/main/libs/langchain-mcp-adapters) 库中 MCP 服务器上定义的工具。
:::
...
<Note>
    `MultiServerMCPClient` 默认**是无状态的**。每次工具调用都会创建一个新的 MCP `ClientSession`，执行工具，然后清理。参见[有状态会话](#stateful-sessions)部分了解更多详情。
</Note>
...
<Note>
    `MultiServerMCPClient` 默认**是无状态的**。每次工具调用都会创建一个新的 MCP `ClientSession`，执行工具，然后清理。
</Note>
...
:::js
要创建你自己的 MCP 服务器，你可以使用 `@modelcontextprotocol/sdk` 库。这个库提供了一个简单的方法来定义[工具](https://modelcontextprotocol.io/docs/learn/server-concepts#tools-ai-actions)并将它们作为服务器运行。
...
:::python
<CodeGroup>
```python title="数学服务器 (stdio transport)" icon="device-floppy"
...
```python title="天气服务器 (streamable HTTP transport)" icon="wifi"
from fastmcp import FastMCP
...
:::js
```typescript title="数学服务器 (stdio transport)" icon="device-floppy"
...
```typescript title="天气服务器 (SSE transport)" icon="wifi"
import { Server } from "@modelcontextprotocol/sdk/server/index.js";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        case "get_weather": {
            const { location } = request.params.arguments as { location: str };
            return {
                content: [
                    {
                        type: "text",
...
:::python
通过 HTTP 连接到 MCP 服务器时，你可以使用连接配置中的 `headers` 字段包含自定义请求头（例如，用于身份验证或追踪）。这支持 `sse`（MCP 规范已弃用）和 `streamable_http` 传输。
...
:::python
`langchain-mcp-adapters` 库在底层使用官方的 [MCP SDK](https://github.com/modelcontextprotocol/python-sdk)，它允许你通过实现 `httpx.Auth` 接口来提供自定义身份验证机制。
...
:::python
<Note>
    与 HTTP 传输不同，`stdio` 连接本质上是**有状态的**：子进程在客户端连接的生命周期内持续存在。但是，当使用 `MultiServerMCPClient` 而没有显式会话管理时，每次工具调用仍然会创建一个新会话。参见[有状态会话](#stateful-sessions)以获取管理持久连接的信息。
</Note>
...
如果你需要控制 MCP 会话的[生命周期](https://modelcontextprotocol.io/specification/2025-03-26/basic/lifecycle)（例如，当使用维护跨工具调用上下文的有状态服务器时），你可以使用 `client.session()` 创建一个持久的 `ClientSession`。

```python 使用 MCP ClientSession 实现有状态工具使用
from langchain_mcp_adapters.client import MultiServerMCPClient
...
[工具](https://modelcontextprotocol.io/docs/concepts/tools) 允许 MCP 服务器暴露可执行的函数，大型语言模型可以调用这些函数来执行操作——如查询数据库、调用 API 或与外部系统交互。LangChain 将 MCP 工具转换为 LangChain [工具](/oss/langchain/tools)，使它们可以直接在任何 LangChain 代理或工作流中使用。

#### 加载工具

:::python
使用 `client.get_tools()` 从 MCP 服务器检索工具并将其传递给你的代理：
...
:::js
使用 `client.getTools()` 从 MCP 服务器检索工具并将其传递给你的代理：
...
MCP 工具可以返回与人类可读文本响应一起的[结构化内容](https://modelcontextprotocol.io/specification/2025-03-26/server/tools#structured-content)。当工具需要返回机器可解析的数据（如 JSON）以及显示给模型的文本时，这很有用。

当 MCP 工具返回 `structuredContent` 时，适配器将其包装在 [`MCPToolArtifact`](https://reference.langchain.com/python/langchain_mcp_adapters/#langchain_mcp_adapters.tools.MCPToolArtifact) 中并将其作为工具的 artifact 返回。你可以使用 `ToolMessage` 上的 `artifact` 字段访问它。你也可以使用[拦截器](#tool-interceptors)来自动处理或转换结构化内容。

**从 artifact 中提取结构化内容**

调用你的代理后，你可以从响应中的工具消息访问结构化内容：
...
MCP 工具可以在其响应中返回[多模态内容](https://modelcontextprotocol.io/specification/2025-03-26/server/tools#tool-result)（图像、文本等）。当 MCP 服务器返回多部分内容（例如文本和图像）时，适配器将它们转换为 LangChain 的[标准内容块](/oss/langchain/messages#standard-content-blocks)。你可以通过 `ToolMessage` 上的 `content_blocks` 属性访问标准化表示：
...
[资源](https://modelcontextprotocol.io/docs/concepts/resources) 允许 MCP 服务器暴露可由客户端读取的数据——如文件、数据库记录或 API 响应。LangChain 将 MCP 资源转换为 [Blob](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Blob) 对象，它们提供了处理文本和二进制内容的统一接口。
...
你也可以直接使用[`load_mcp_resources`](https://reference.langchain.com/python/langchain_mcp_adapters/#langchain_mcp_adapters.resources.load_mcp_resources)与会话一起以获得更多控制：
...
[提示词](https://modelcontextprotocol.io/docs/concepts/prompts) 允许 MCP 服务器暴露可由客户端检索和使用的可重用提示词模板。LangChain 将 MCP 提示词转换为[消息](/oss/langchain/messages)，使它们易于集成到基于聊天的工作流中。
...
你也可以直接使用[`load_mcp_prompt`](https://reference.langchain.com/python/langchain_mcp_adapters/#langchain_mcp_adapters.prompts.load_mcp_prompt)与会话一起以获得更多控制：
...
MCP 服务器作为单独的进程运行——它们无法访问 LangGraph 运行时信息，如[存储](/oss/langgraph/persistence#memory-store)、[上下文](/oss/langchain/context-engineering)或代理状态。**拦截器弥合了这一差距**，让你在 MCP 工具执行期间访问此运行时上下文。
...
| [状态更新和命令](#state-updates-and-commands) | 使用 `Command` 更新代理状态或控制图形流程 |
| [编写拦截器](#custom-interceptors) | 用于修改请求、组合拦截器和错误处理的模式 |

#### 访问运行时上下文

当 MCP 工具在 LangChain 代理中使用时（通过 `create_agent`），拦截器接收对 `ToolRuntime` 上下文的访问。这提供了对工具调用 ID、状态、配置和存储的访问——实现访问用户数据、持久化信息和控制代理行为的强大模式。

<Tabs>
  <Tab title="运行时上下文">
    访问特定于用户的配置，如用户 ID、API 密钥或在调用时传递的权限：
...
更多上下文工程模式，参见[上下文工程](/oss/langchain/context-engineering)和[工具](/oss/langchain/tools)。

#### 状态更新和命令

拦截器可以返回 `Command` 对象来更新代理状态或控制图形执行流程。这对于跟踪任务进度、在代理之间切换或提前结束执行很有用。
...
使用 `request.override()` 创建一个修改后的请求。这遵循不可变模式，保持原始请求不变。
...
[征集](https://modelcontextprotocol.io/specification/2025-11-25/client/elicitation#elicitation) 允许 MCP 服务器在工具执行期间请求用户的额外输入。服务器可以交互式地根据需要询问信息，而不是要求所有输入提前提供。

#### 服务器设置

定义一个使用 `ctx.elicit()` 请求用户输入的工具，并带有模式：
...
通过向 `MultiServerMCPClient` 提供回调来处理征集请求：
...
async def on_elicitation(
    ...
    """处理来自 MCP 服务器的征集请求。"""
    # 在真实应用中，你会根据 params.message 和 params.requestedSchema 提示用户输入
