...
状态是代理中所有节点可访问的共享[内存](/oss/concepts/memory)。将其视为代理用来跟踪其在处理流程中学习和决定的所有内容的笔记本。
...
要使用 `interrupt()` 启用[人工介入](/oss/langgraph/interrupts)，我们需要使用[检查点](/oss/langgraph/persistence)编译图，以在运行之间保存状态：
...
:::python
图结构是最小的，因为路由通过 @[`Command`] 对象发生在节点内部。每个节点使用类型提示（如 `Command[Literal["node1", "node2"]]`）声明它可以去哪里，使流程变得明确和可追踪。
:::
...
<CardGroup cols={2}>
    ...
    <Card title="人工输入是第一等的" icon="user" href="/oss/langgraph/interrupts">
...
**弹性考虑：** LangGraph 的[持久执行](/oss/langgraph/durable-execution)在节点边界创建检查点。当工作流在中断或失败后恢复时，它从执行停止的节点的开头开始。较小的节点意味着更频繁的检查点，这意味着如果出现问题，要重复的工作更少。如果您将多个操作组合成一个大型节点，接近末尾的失败意味着从该节点的开头重新执行所有内容。
...
性能考虑：更多节点并不意味着更慢的执行。LangGraph 默认在后台写入检查点（[异步持久化模式](/oss/langgraph/durable-execution#durability-modes)），因此您的图继续运行而无需等待检查点完成。这意味着您可以获得频繁的检查点，同时对性能的影响最小。如果需要，您可以调整此行为——使用 `"exit"` 模式仅在完成时创建检查点，或使用 `"sync"` 模式阻止执行直到每个检查点写入完成。
</Accordion>
...
<CardGroup cols={2}>
    <Card title="人工介入模式" icon="user-check" href="/oss/langgraph/interrupts">
    ...
    <Card title="流式处理" icon="broadcast" href="/oss/langgraph/streaming">
        添加流式处理以向用户显示实时进度
    </Card>
