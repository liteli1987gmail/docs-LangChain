...
[**函数式 API**](/oss/langgraph/functional-api) 允许您以最小的改动将 LangGraph 的关键功能（[持久化](/oss/langgraph/persistence)、[内存](/oss/langgraph/add-memory)、[人工介入](/oss/langgraph/interrupts)和[流式处理](/oss/langgraph/streaming)）添加到您的应用程序中。
