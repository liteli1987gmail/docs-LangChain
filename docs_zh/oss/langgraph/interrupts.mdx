...
中断允许您在特定点暂停图执行，并等待外部输入后再继续。这使得人机交互模式成为可能，在这些模式中您需要外部输入才能继续。当触发中断时，LangGraph 使用其[持久化](/oss/langgraph/persistence)层保存图状态，并无限期等待直到您恢复执行。
...
@[`interrupt`] 函数暂停图执行并将值返回给调用者。当您在节点内调用 @[`interrupt`] 时，LangGraph 保存当前图状态并等待您恢复执行并输入。
...
- 恢复时节点从 @[`interrupt`] 被调用的节点的开头重新启动，因此在 @[`interrupt`] 之前运行的任何代码都会再次执行
- 您可以传递任何 JSON 可序列化的值作为恢复值

<Warning>

`Command(resume=...)` 是唯一打算作为 `invoke()`/`stream()` 输入的 `Command` 模式。其他 `Command` 参数（`update`、`goto`、`graph`）设计用于[从节点函数返回](/oss/langgraph/graph-api#command)。不要传递 `Command(update=...)` 作为输入来继续多轮对话——而是传递普通的输入字典。
...
`new Command({ resume: ... })` 是唯一打算作为 `invoke()`/`stream()` 输入的 `Command` 模式。其他 `Command` 参数（`update`、`goto`、`graph`）设计用于[从节点函数返回](/oss/langgraph/graph-api#command)。不要传递 `new Command({ update: ... })` 作为输入来继续多轮对话——而是传递普通的输入对象。
...
有时您需要验证来自人类的输入，如果无效则再次询问。您可以使用循环中的多个 @[`interrupt`] 调用来实现此目的。
...
当您在节点内调用 @[`interrupt`] 时，LangGraph 通过抛出表示运行时暂停的特殊异常来挂起执行。此异常通过调用栈向上传播，被运行时捕获，通知图保存当前状态并等待外部输入。

当执行恢复后（您提供请求的输入后），运行时从头开始重启整个节点——它不会从调用 @[`interrupt`] 的确切行恢复。这意味着在 @[`interrupt`] 之前运行的任何代码都会再次执行。因此，在使用中断时，需要遵循一些重要的规则以确保它们按预期运行。

:::python
### 不要将 `interrupt` 调用包装在 try/except 中

@[`interrupt`] 通过抛出特殊异常在调用点暂停执行的方式。如果将 @[`interrupt`] 调用包装在 try/except 块中，您将捕获此异常，中断将不会传递回图。
...
@[`interrupt`] 通过抛出特殊异常在调用点暂停执行的方式。如果将 @[`interrupt`] 调用包装在 try/catch 块中，您将捕获此异常，中断将不会传递回图。
...
* 🔴 不要将 @[`interrupt`] 调用包装在裸露的 try/catch 块中
...
因为中断通过重新运行调用它们的节点来工作，所以在 @[`interrupt`] 之前调用的副作用应该是（理想情况下）幂等的。作为背景，幂等性意味着相同的操作可以应用多次而不会改变初始执行之后的结果。

例如，您可能在节点内部有一个更新记录的 API 调用。如果在调用之后调用 @[`interrupt`]，则在节点恢复时会重新运行多次，可能会覆盖初始更新或创建重复记录。
...
在节点内调用子图时，父图将从**调用子图的节点的开头**恢复执行，并且 @[`interrupt`] 被触发的位置。同样，**子图**也将从调用 @[`interrupt`] 的节点的开头恢复。
...
<Note>
静态中断不推荐用于人机交互工作流。请改用 @[`interrupt`] 函数。
</Note>
