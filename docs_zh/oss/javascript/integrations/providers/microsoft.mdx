...
请参阅 a [usage 示例](/oss/集成s/chat/azure)
...
```
请参阅 a [usage 示例](/oss/集成s/llms/azure).
...
请参阅 a [usage 示例](/oss/集成s/embeddings/azure_openai)
...
```
请参阅 a [usage 示例](/oss/集成s/vectorstores/azure_aisearch).
...
```
请参阅 a [usage 示例](/oss/集成s/vectorstores/azure_cosmosdb_nosql).
...
npm install @langchain/azure-cosmosdb @langchain/core
```
请参阅 a [usage 示例](/oss/集成s/vectorstores/azure_documentdb).
...
> 该 Semantic Cache feature is supported with Azure Cosmos DB for 否SQL 集成, enabling users to retrieve cached responses based on semantic similarity between the user input and previously cached results. It leverages [AzureCosmosDB否SQLVectorStore](/oss/集成s/vectorstores/azure_cosmosdb_nosql), which stores vector embeddings of cached prompts. 该se embeddings enable similarity-based searches, allowing the system to retrieve relevant cached results.

```bash npm
npm install @langchain/azure-cosmosdb @langchain/core
```
请参阅 a [usage 示例](/oss/集成s/llm_caching/azure_cosmosdb_nosql).
...
```
请参阅 a [usage 示例 for the Azure Blob Storage](/oss/集成s/document_loaders/web_loaders/azure_blob_storage_container).

```typescript
import { AzureBlobStorageContainerLoader } from "@langchain/community/document_loaders/web/azure_blob_storage_container";
```
请参阅 a [usage 示例 for the Azure Files](/oss/集成s/document_loaders/web_loaders/azure_blob_storage_file).
...
```
请参阅 a [usage 示例](/oss/集成s/tools/azure_dynamic_sessions).
