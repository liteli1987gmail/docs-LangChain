...
该 示例 below assumes that you already created a table with vector and metadata columns and generated embeddings using [`Oracle嵌入`](/oss/集成s/embeddings/oracleai).
...
- Load content with [`OracleDocLoader`](/oss/集成s/document_loaders/file_loaders/oracleai)
- Generate embeddings with [`Oracle嵌入`](/oss/集成s/embeddings/oracleai)
- Summarize documents using [`OracleSummary`](/oss/集成s/tools/oracleai)
