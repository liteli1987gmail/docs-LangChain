...
Install the `@sap/hana-langchain` external 集成 包, as well as the other 包s used throughout this notebook.
...
`HanaDB` 支持 following distance similarity algorithms:
...
A vector index can significantly speed up top-k nearest neighbor queries for vectors. 使用rs can create a Hierarchical Navigable Small World (HNSW) vector index using the `createHnswIndex` function.
...
默认 values: m=64, ef_construction=128, ef_search=200

该 default index name will be: "\<TABLE_NAME>_idx"

## Advanced filtering

In addition to the basic value-based filtering capabilities, it is possible to use more advanced filtering. 该 table below shows the available filter operators.
...
| `$nin` | 否t contained in a set of given values (not in) |
...
## 使用方法 for retrieval-augmented generation

For guides on how to use this vector store for retrieval-augmented generation (RAG), 请参阅 the following sections:
...
该 table can contain additional columns. When new Documents are inserted into the table, these additional columns must allow NULL values.
...
该 special columns are completely transparent to the rest of the langchain interface. Everything works as it did before, just more performant.
...
First, 安装 `@langchain/textsplitters` 包:
...
Add the loaded document chunks to the table. For this 示例, we delete any previous content from the table which might exist from previous runs.
...
Query the same content with "Euclidean Distance". 该 results should be the same as with "Cosine Similarity".
...
`Maximal marginal relevance` optimizes for similarity to query AND diversity among selected documents. 该 first 20 (fetch_k) items will be retrieved from the DB. 该 MMR algorithm will then find the best 2 (k) matches.
...
- **Similarity Function**: 该 similarity function for the index is **cosine similarity** by default. If you want to use a different similarity function (e.g., `L2` distance), you need to specify it when initializing the `HanaDB` instance.
- **默认 Parameters**: In the `createHnswIndex` function, if the user does not provide custom values for parameters like `m`, `efConstruction`, or `efSearch`, the default values (e.g., `m=64`, `efConstruction=128`, `efSearch=200`) will be used automatically. 该se values ensure the index is created with reasonable performance without requiring user intervention.
