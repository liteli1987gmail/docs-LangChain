...
本指南将帮助您开始使用 OpenRouter [聊天模型](/oss/langchain/models)。OpenRouter 是一个统一 API，通过单一端点提供对多个提供商（OpenAI、Anthropic、Google、Meta 等）模型的访问。
...
OpenRouter 支持接受多模态输入的模型的[多模态输入](/oss/langchain/messages#multimodal)。可用模态取决于您选择的模型——有关详细信息，请查看 [OpenRouter 模型页面](https://openrouter.ai/models)。
