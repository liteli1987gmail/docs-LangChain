...
这将帮助您开始使用 `ChatGoogle` [聊天模型](/oss/langchain/models)。
...
<Tip>
  ...
  有关迁移详细信息，请参阅[旧包](/oss/integrations/providers/google#legacy-packages)。
</Tip>
...
`ChatGoogle` 支持标准 LangChain [工具调用](/oss/langchain/tools)以及 Gemini 特定的"专业工具"（如代码执行和接地）。
