...
[Anthropic](https://www.anthropic.com/) 是一家 AI 安全和研究公司。他们是 Claude 的创造者。

这将帮助您开始使用 `ChatAnthropic` [聊天模型](/oss/langchain/models)。有关所有 `ChatAnthropic` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-anthropic/ChatAnthropic)。
...
Anthropic 模型与大多数其他模型的一个关键区别在于，单个 Anthropic @[`AIMessage`] 的内容可以是单个字符串或**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/langchain/tools)时，工具调用是消息内容的一部分（同时也暴露在标准化的 `AIMessage.tool_calls` 字段中）：
...
Anthropic 支持[缓存提示词的某些部分](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)，以降低需要长上下文的用例的成本。您可以缓存工具以及整个消息和单个块。

包含一个或多个块或工具定义的初始请求（带有 `"cache_control": { "type": "ephemeral" }` 字段）将自动缓存提示词的该部分。这个初始缓存步骤将产生额外费用，但后续请求将按降低的费率计费。缓存的生命周期为 5 分钟，但每次缓存命中都会刷新此时间。要进行更长时间的缓存，请在 `cache_control` 字段中指定 `"ttl": "1h"`。

存在一个最小可缓存提示词长度，因模型而异。有关更多信息，请参阅[提示词缓存详细信息](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#cache-limitations)。

以下是在包含 LangChain [概念文档](/oss/concepts/)的系统消息中缓存部分的示例：
...
我们可以看到原始使用量字段中有一个名为 `cache_creation_input_tokens` 的新字段。
...
有关提示词缓存工作原理的更多信息，请参阅 [Anthropic 的文档](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#how-prompt-caching-works)。

## 自定义客户端

Anthropic 模型[可能托管在 Google Vertex 等云服务上](https://platform.claude.com/docs/en/build-with-claude/claude-on-vertex-ai)，这些服务依赖与主要 Anthropic 客户端具有相同接口的不同底层客户端。您可以通过提供返回初始化后的 Anthropic 客户端实例的 `createClient` 方法来访问这些服务。以下是一个示例：
...
Anthropic 支持[引用](https://platform.claude.com/docs/en/build-with-claude/citations)功能，允许 Claude 根据用户提供的源材料在回答中附加上下文。此源材料可以作为[文档内容块](https://platform.claude.com/docs/en/build-with-claude/citations#document-types)（描述完整文档）或[搜索结果](https://platform.claude.com/docs/en/build-with-claude/citations#search-results)（描述检索系统中返回的相关段落或片段）提供。当在查询中包含 `"citations": { "enabled": true }` 时，Claude 可以在其回答中生成对所提供的材料的直接引用。

### 文档示例

在这个示例中，我们传递一个[纯文本文档](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)。在后台，Claude[自动将](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)输入文本拆分为句子，这在生成引用时使用。
...
在这个示例中，我们在消息内容中传递[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)。这允许 Claude 在其回答中引用您自己的检索系统中的特定段落或片段。

当您希望 Claude 引用特定知识集中的信息，但希望直接提供您自己预取/缓存的内容，而不是让模型自动搜索或检索时，这种方法会很有帮助。
...
您还可以使用工具来提供模型可以在回答中引用的搜索结果。这非常适合 RAG（或[检索增强生成](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)）工作流程，Claude 可以决定何时何地从工具中检索信息。当将这些信息作为[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)返回时，它使 Claude 能够根据工具返回的材料创建引用。

以下是如何创建以 Anthropic 引用 API 期望的格式返回搜索结果的工具：
...
详细了解 [LangChain 中 RAG 的工作原理](https://js.langchain.com/docs/concepts/rag/)

[详细了解工具调用](/oss/langchain/tools)

### 与文本分割器一起使用

Anthropic 还允许您使用[自定义文档](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents)类型指定您自己的拆分。LangChain 文本分割器可用于为此目的生成有意义的拆分。请参阅下面的示例，我们在其中拆分 LangChain.js README（一个 markdown 文档）并将其作为上下文传递给 Claude：
...
请参阅 [Anthropic 文档](https://platform.claude.com/docs/en/build-with-claude/context-editing) 了解详细信息和配置选项。
