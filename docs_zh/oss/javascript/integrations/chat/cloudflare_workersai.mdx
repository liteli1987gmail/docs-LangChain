...
[Workers AI](https://developers.cloudflare.com/workers-ai/) 允许您在自己的代码中从 Cloudflare 网络上运行机器学习模型。

这将帮助您开始使用 `ChatCloudflareWorkersAI` [聊天模型](/oss/langchain/models)。有关所有 `ChatCloudflareWorkersAI` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-cloudflare/ChatCloudflareWorkersAI)。
...
前往[此页面](https://developers.cloudflare.com/workers-ai/)注册 Cloudflare 并生成 API 密钥。完成后，记下您的 `CLOUDFLARE_ACCOUNT_ID` 和 `CLOUDFLARE_API_TOKEN`。

暂不支持在 Cloudflare Worker 内传递绑定。
...
const llm = new ChatCloudflareWorkersAI({
  model: "@cf/meta/llama-2-7b-chat-int8", // Default value // 默认值
  cloudflareAccountId: CLOUDFLARE_ACCOUNT_ID,
  cloudflareApiToken: CLOUDFLARE_API_TOKEN,
  // Pass a custom base URL to use Cloudflare AI Gateway
  // 传递自定义 base URL 以使用 Cloudflare AI Gateway
