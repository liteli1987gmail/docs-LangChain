...
这将帮助您开始使用 `BedrockChat` [聊天模型](/oss/langchain/models)。有关所有 `BedrockChat` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-community/chat_models/bedrock/BedrockChat)。

<Tip>
更新的 [`ChatBedrockConverse` 聊天模型现已通过专用的 `@langchain/aws`](/oss/integrations/chat/bedrock_converse) 集成包提供。使用此包可以通过更多模型使用[工具调用](/oss/langchain/tools)。
</Tip>
...
Bedrock 模型的工具调用与[其他模型](/oss/langchain/tools)类似，但请注意并非所有 Bedrock 模型都支持工具调用。请参阅 [AWS 模型文档](https://docs.aws.amazon.com/bedrock/latest/APIReference/welcome.html) 了解更多信息。
