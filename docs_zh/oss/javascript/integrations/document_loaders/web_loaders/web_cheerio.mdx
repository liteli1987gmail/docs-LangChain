...
本笔记本提供了开始使用 `CheerioWebBaseLoader` [文档加载器](/oss/integrations/document_loaders/) 的快速概述。有关所有 `CheerioWebBaseLoader` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/cheerio/CheerioWebBaseLoader)。
...
但是，Cheerio 不会模拟 Web 浏览器，因此无法执行页面上的 JavaScript 代码。这意味着它无法从需要 JavaScript 呈现的动态网页中提取数据。为此，您可以使用 [`PlaywrightWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_playwright) 或 [`PuppeteerWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_puppeteer) 代替。
