...
description: "使用 LangChain JavaScript 与 Google cloud SQL for PostgreSQL 文档加载器集成。"
---

[Cloud SQL](https://cloud.google.com/sql) 是一个完全托管的关系数据库服务，提供高性能、无缝集成和令人印象深刻的可扩展性，并提供 PostgreSQL 等数据库引擎。

本指南提供如何使用 Cloud SQL for PostgreSQL 使用 `PostgresLoader` 类加载文档的快速概述。
...
2.  [为您的项目启用账单。](https://cloud.google.com/billing/docs/how-to/modify-project#enable_billing_for_a_project)
...
使用 `gcloud auth login` 命令在本地向您的 Google Cloud 账户进行身份验证。
...
首先，您需要登录您的 Google Cloud 账户并根据您的 Google Cloud 项目设置以下环境变量；这些将根据您要如何配置（fromInstance、fromEngine、fromEngineArgs）您的 PostgresEngine 实例来定义：
...
加载器使用第一列作为 page_content，所有其他列作为元数据从表中返回文档列表。默认表将第一列作为 page_content，第二列作为元数据（JSON）。每一行成为一个文档。
...
query 参数允许用户指定自定义 SQL 查询，其中可以包括过滤器以从数据库加载特定文档。
...
加载器返回文档列表，每行一个文档，页面内容采用指定的字符串格式，即 text（空格分隔连接）、JSON、YAML、CSV 等。JSON 和 YAML 格式包含标题，而 text 和 CSV 不包含字段标题。