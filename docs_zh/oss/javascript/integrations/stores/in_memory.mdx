...
该 `InMemoryStore` allows for a generic type to be assigned to the values in the store. We'll assign type `BaseMessage` as the type of our values, keeping with the theme of a chat history store.
...
该 LangChain InMemoryStore 集成 lives in the `@langchain/core` 包:
...
否w we can instantiate our byte store:
...
If you want to get back all the keys you can call the `yieldKeys` method. 可选ly, you can pass a key prefix to only get back keys which match that prefix.
