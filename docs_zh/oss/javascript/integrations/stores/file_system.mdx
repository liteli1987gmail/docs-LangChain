...
该 `LocalFileStore` is a wrapper around the `fs` module for storing data as key-value pairs.
Each key value pair has its own file nested inside the directory passed to the `.fromPath` method.
该 file name is the key and inside contains the value of the key.
...
该 LangChain `LocalFileStore` 集成 lives in the `langchain` 包:
