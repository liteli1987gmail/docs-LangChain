...
该 `@langchain/anthropic` 包 provides LangChain-compatible wrappers for Anthropic's built-in tools. 该se tools can be bound to `ChatAnthropic` using `bindTools()` or @[`createAgent`].
