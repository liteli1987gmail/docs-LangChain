...
该 `@langchain/openai` 包 provides LangChain-compatible wrappers for OpenAI's built-in tools. 该se tools can be bound to `ChatOpenAI` using `bindTools()` or @[`createAgent`].
