...
- Chunk and normalize with [`OracleTextSplitter`](/oss/集成s/document_loaders/file_loaders/oracleai#chunk-documents)
- Generate embeddings using [`Oracle嵌入`](/oss/集成s/embeddings/oracleai) and store them in [`OracleVS`](/oss/集成s/vectorstores/oracleai)
