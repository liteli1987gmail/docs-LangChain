---
title: 向追踪添加元数据和标签
sidebarTitle: 向追踪添加元数据和标签
---

LangSmith 支持随追踪一起发送任意元数据和标签。

标签是可用于分类或标记追踪的字符串。元数据是一个键值对字典，可用于存储有关追踪的其他信息。

两者都可用于将额外信息与追踪关联，例如执行的环境、启动它的用户或内部关联 ID。有关标签和元数据的更多信息，请参阅[概念](/langsmith/observability-concepts#tags)页面。有关如何按元数据和标签查询追踪和运行的信息，请参阅[在应用程序中过滤追踪](/langsmith/filter-traces-in-application)页面。
...
<Tip>
**LangSmith 部署**：要在 Agent Server 部署中动态添加每次调用的元数据，我们建议在[工厂函数](/langsmith/graph-rebuild)中使用 `tracing_context`。有关示例，请参阅[自定义部署代理中的追踪](/langsmith/conditional-tracing#customize-tracing-in-deployed-agents)。
</Tip>
