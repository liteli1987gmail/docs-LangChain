...
如上所述，我们将定义两个评估器：一个评估检索文档相对于输入查询的相关性，另一个评估生成的答案相对于检索文档的幻觉。我们将使用 LangChain LLM 包装器以及 @[`with_structured_output`][BaseChatModel.with_structured_output] 来定义幻觉的评估器。
