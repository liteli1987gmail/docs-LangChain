...
您可能会在我们的文档中看到这些术语交替使用，但它们都指代相同的基础数据结构。

我们正在努力统一文档和 API 的术语。
</Check>

当您使用 Python 或 TypeScript SDK 运行实验时，您可以使用 `read_project`/`readProject` 方法获取实验的性能指标。
...
* `latency_p50`：第 50 个百分位延迟（秒）。
* `latency_p99`：第 99 个百分位延迟（秒）。
...
* `error_rate`：实验的错误率。
* `first_token_p50`：生成第一个 Token 时间的第 50 个百分位延迟（如果使用流式）。
* `first_token_p99`：生成第一个 Token 时间的第 99 个百分位延迟（如果使用流式）。

以下是如何使用 Python 和 TypeScript SDK 获取实验性能指标的示例。

首先，作为先决条件，我们将创建一个简单数据集。这里，我们仅在 Python 中演示，但您可以在 TypeScript 中执行相同的操作。请查看评估[操作指南](/langsmith/evaluate-llm-application)获取更多详细信息。
