...
description: 了解如何使用 OpenTelemetry 在 LangSmith 中追踪 Temporal 工作流和活动。
---

[Temporal](https://temporal.io/) 是一个持久执行平台，使开发者能够构建弹性的分布式应用程序。本指南展示如何使用 OpenTelemetry 在 LangSmith 中追踪 Temporal 工作流和活动。

LangSmith 支持 OpenTelemetry（OTEL）追踪摄取，这与 Temporal 的原生 OpenTelemetry 拦截器无缝集成。这使得您的工作流执行、活动及其中的任何 LLM 调用能够进行完整的分布式追踪。

## 前置条件

- 一个 [LangSmith 账户](https://smith.langchain.com/) 和 API 密钥
- Temporal 服务器运行中（本地或云端）
...
| 变量                | 必填 | 描述                                    |
| ------------------- | ---- | -------------------------------------- |
| `LANGSMITH_API_KEY` | 是   | 您从 Settings 获取的 LangSmith API 密钥。|
| `LANGSMITH_PROJECT` | 否   | 项目名称（默认为 `"default"`）。        |

<Note>
对于 EU 区域或自托管 LangSmith 安装，还要将 `LANGCHAIN_BASE_URL` 设置为您的 LangSmith 实例 URL。
</Note>
...
创建一个配置为将追踪发送到 LangSmith 的 OpenTelemetry `TracerProvider` 和 OTLP 导出器：
...
创建一个配置为将追踪发送到 LangSmith 的 `NodeTracerProvider` 和 OTLP 导出器：
...
创建带有 OpenTelemetry 拦截器的活动和工作流导出器的工作线程：
...
配置后，追踪将出现在您的 LangSmith 项目中：

1. 导航到您的 LangSmith 实例。
2. 选择您的项目。
3. 在 **Tracing** 选项卡中查看追踪。
...
- **Python/TypeScript**：确保 OpenTelemetry SDK 正确初始化了追踪传播器

### 工作线程关闭挂起

如果追踪未刷新，确保使用正确的超时调用关闭方法：
