...
Agent Server 支持对 checkpoint 数据和元数据进行静态加密。您可以选择使用单个密钥的基本加密，或为高级用例使用自定义加密。

## 选择加密方法

| 方法 | 加密内容 | 用例 |
|--------|------------------|----------|
| **基本加密** | Checkpoint blobs，可选 JSON 字段 | 单个静态密钥，自动 AES 加密 |
| **自定义加密** | Checkpoints、threads、runs、assistants、crons 和 stores | 每租户密钥、KMS 集成、选择性字段加密 |

## 基本加密

对于使用单个静态密钥的简单加密，请设置 `LANGGRAPH_AES_KEY` 环境变量。LangGraph 将自动使用 AES 加密 checkpoint blobs。
...
2. 将 `LANGGRAPH_AES_KEY` 环境变量设置为 16、24 或 32 字节的密钥（分别为 AES-128、AES-192 或 AES-256）。

### 加密 JSON 字段

要同时加密特定的 JSON 字段，请将 `LANGGRAPH_AES_JSON_KEYS` 设置为要加密的密钥的逗号分隔列表：
...
这些密钥在 thread、assistant、run、cron 和 store 数据中出现的任何地方都会被加密。

<Warning>
加密字段无法被搜索或过滤。
</Warning>

系统字段无法被加密：`langgraph_version`、`langgraph_api_version`、`langgraph_plan`、`langgraph_host`、`langgraph_api_url`、`langgraph_request_id`、`langgraph_auth_user_id` 和 `langgraph_auth_permissions`。
...
<Warning>
Agent Server 版本 0.5.34–0.6.21 包含自定义加密的预发布版本。使用这些版本加密的数据在升级到 0.6.22+ 时将损坏。请勿在这些版本上使用自定义加密。
</Warning>

当您需要以下功能时，请使用自定义加密：

- **每租户密钥隔离** — 不同客户使用不同的加密密钥
- **KMS 集成** — AWS KMS、Google Cloud KMS 或 HashiCorp Vault 用于密钥管理、轮换和审计日志
- **选择性字段加密** — 加密敏感元数据字段，同时保持其他字段可搜索

### 工作原理

1. 在 `langgraph.json` 中[配置](#configuration)加密模块路径
2. 为 blob 和 JSON 加密[定义您的加密模块](#defining-your-encryption-module)
3. 通过 `X-Encryption-Context` 头[传递加密上下文](#passing-encryption-context)（如租户 ID）
4. LangGraph 在存储和检索数据之前和之后调用您的处理程序

有关使用密钥轮换和审计日志的生产部署，请参阅[使用 AWS Encryption SDK 的信封加密](#envelope-encryption-with-aws-encryption-sdk)。
...
<Note>
如果您正在从基本加密迁移，请保持 `LANGGRAPH_AES_KEY` 配置。自定义加密处理新写入，而现有的 AES 加密数据仍可读取。
</Note>
...
Blob 处理程序加密 checkpoint 数据——图执行的序列化状态。以下是使用每租户密钥和 [Fernet](https://cryptography.io/en/latest/fernet/)（来自 `cryptography` 库的对称加密方案）的简化示例：
...
`ctx.metadata` 字典来自 `X-Encryption-Context` 头，并与加密数据一起以明文形式存储，因此在解密时使用正确的密钥。

#### JSON 加密（元数据）

JSON 处理程序加密结构化数据，如 thread 元数据、assistant 上下文和 run kwargs。与 blob 加密不同，您选择要加密的字段——保持某些字段未加密以供搜索和过滤。
...
<Warning>
**加密字段无法被搜索或过滤。** 设计您的元数据架构，使您需要查询的字段保持未加密。
</Warning>

<Warning>
**JSON 加密器必须保留密钥结构。** SQL JSONB 合并操作在密钥级别工作。改变密钥的加密器——无论是合并字段（例如将敏感数据移入 `__encrypted__`）还是加密密钥本身——都会在合并时导致数据丢失。使用每密钥加密：在保留密钥的同时就地转换值。
</Warning>

<Note>
**迁移注意事项：** 在加密值中使用可识别的前缀或格式，以便您的解密器可以检测并跳过未加密的数据。这允许您将来加密其他字段而无需重新加密现有记录。上面的示例使用了这种模式。
</Note>

<Note>
**性能注意事项：** 每密钥加密意味着每个字段一次加密调用。如果您的加密涉及对外部服务（例如 KMS）的往返，这可能会显著影响延迟。考虑在本地缓存数据密钥或使用信封加密，您在其中用 KMS 加密本地数据密钥并将其用于多个字段。
</Note>

用于授权的用户定义字段（例如 `tenant_id`、`owner`）通常应保持**未加密**，用于搜索和过滤的字段也是如此。此外，**某些系统管理的字段永远不会加密**：

- 资源标识符（`thread_id`、`run_id`、`assistant_id`、`graph_id`、`checkpoint_id`、`task_id`）
- 大多数以 `langgraph_` 开头的字段（`langgraph_auth_user` 除外）
- 必需的 checkpoint 元数据（`source`、`step`、`parents`、`run_attempt`）
...
- 在运行的 `config.configurable` 中指定的 Thread TTL 更新（`ttl`）
...
[某些字段被排除在加密之外。](#what-gets-encrypted)除非另有说明，否则这些排除适用于嵌套 JSON 对象的每个级别，而不仅仅是根级别。

**Blob 处理程序**（`@encryption.encrypt.blob` / `@encryption.decrypt.blob`）应用于 checkpoint blobs（图执行状态）。


#### 从身份验证派生上下文

而不是显式传递 `X-Encryption-Context`，从经过身份验证的用户派生加密上下文：
...
通过 `X-Encryption-Context` 头传递加密上下文。上下文是您定义的任意数据——您控制模式，可以包含加密逻辑需要的任何字段（例如 `tenant_id`、`key_version`）。上下文在您的处理程序中作为 `ctx.metadata` 可用，并与加密数据一起以明文形式存储以供解密使用。
...
<Note>
加密上下文以明文形式存储。在解密时，它会自动恢复——调用者在读取时不需要传递头。
</Note>

### 使用 AWS Encryption SDK 的信封加密

对于 AWS 上的生产部署，请将 [AWS Encryption SDK](https://docs.aws.amazon.com/encryption-sdk/latest/developer-guide/python.html) 与 AWS KMS 或云提供商中的等效产品一起使用。这种方法：
...
`encryption_context` 通过 KMS 加密绑定到密文——如果上下文不匹配，解密失败。上下文嵌入在密文中，因此解密处理程序不需要引用 `ctx.metadata`。

#### 密钥轮换

KMS 自动处理主密钥轮换。当您在 KMS 密钥上启用自动轮换时，旧的加密数据密钥仍可解密，而新操作使用轮换的密钥材料。无需重新加密现有数据。
