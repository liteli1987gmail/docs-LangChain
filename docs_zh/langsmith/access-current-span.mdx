...
sidebarTitle: 在追踪函数中访问当前运行（span）
---

在某些情况下，您会想要在追踪函数中访问当前的运行（span）。这对于从当前运行中提取 UUID、标签或其他信息非常有用。

您可以通过在 Python 或 TypeScript SDK 中分别调用 `get_current_run_tree`/`getCurrentRunTree` 函数来访问当前运行。
