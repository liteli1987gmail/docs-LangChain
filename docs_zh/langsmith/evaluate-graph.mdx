...
`langgraph` 是一个用于使用大型语言模型构建有状态的多参与者应用程序的库，用于创建代理和多代理工作流。评估 `langgraph` 图可能具有挑战性，因为单个调用可能涉及许多 LLM 调用，而哪些 LLM 调用被做出可能取决于先前调用的输出。在本指南中，我们将重点介绍如何将图和图节点传递给 `evaluate()` / `aevaluate()` 的机制。在构建代理时了解评估技术和最佳实践，请前往 [langgraph 文档](https://langchain-ai.github.io/langgraph/tutorials/#evaluation)。
...
workflow.add_conditional_edges(
    ...
    "agent",
    # 接下来，我们传入将决定哪个节点被调用的函数。
    should_continue,
)

# 现在我们添加从 'tools' 到 'agent' 的正常边。
# 这意味着在 'tools' 被调用后，'agent' 节点被调用。
workflow.add_edge("tools", 'agent')

# 最后，我们编译它！
# 这将它编译成 LangChain Runnable，
...
现在我们可以运行我们的评估并探索结果。我们只需要将我们的图函数包装起来，使其能够以示例存储的格式接收输入：

<Note>
如果您的所有图节点都定义为同步函数，那么您可以使用 `evaluate` 或 `aevaluate`。如果您的任何节点定义为异步，您需要使用 `aevaluate`
</Note>
...
通常，评估代理的最终输出以及它采取的中间步骤是有价值的。`langgraph` 的好处是图的输出是一个状态对象，通常已经携带了有关所采取的中间步骤的信息。通常，我们可以通过查看状态中的消息来评估我们感兴趣的内容。例如，我们可以查看消息以断言模型在第一步调用了 'search' 工具。
...
如果我们需要访问不在状态中的有关中间步骤的信息，我们可以查看 Run 对象。这包含所有节点输入和输出的完整追踪：

<Check>
请参阅此[操作指南](/langsmith/code-evaluator-ui)了解更多关于您可以传递给自定义评估器的参数。
</Check>
...
有时您希望直接评估单个节点以节省时间和成本。`langgraph` 使这变得容易。在这种情况下，我们甚至可以继续使用我们一直在使用的评估器。
