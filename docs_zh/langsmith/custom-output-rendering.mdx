---
title: 自定义输出渲染
sidebarTitle: 自定义输出渲染
---

自定义输出渲染允许您使用自己的自定义 HTML 页面可视化运行输出和数据集参考输出。这对于以下情况特别有用：

-   **特定领域格式化**：以原生格式显示医疗记录、法律文档或其他专业数据类型。
-   **自定义可视化**：从数字或结构化输出数据创建图表、图形或图表。

在本页面中，您将学习如何：

-   **[在 LangSmith UI 中配置自定义渲染](#configure-custom-output-rendering)**。
-   **[构建自定义渲染器](#build-a-custom-renderer)** 以显示输出数据。
-   **[了解自定义渲染出现的位置](#where-custom-rendering-appears)**。

## 配置自定义输出渲染

在两个级别配置自定义渲染：

-   **对于数据集**：将自定义渲染应用到与该数据集关联的所有运行，无论它们出现在实验、运行详情窗格还是标注队列中。
-   **对于标注队列**：将自定义渲染应用到特定标注队列内的所有运行，而不管它们来自哪个数据集。这优先于数据集级别的配置。

### 对于追踪项目

要为追踪项目配置自定义输出渲染：

![显示自定义输出渲染配置的追踪项目设置](/langsmith/images/tracing-project-custom-output-rendering-settings.png)

1. 导航到 **Tracing Projects** 页面。
2. 点击一个现有的追踪项目或创建一个新的。
3. 在编辑追踪项目窗格中，向下滚动到 **Custom Output Rendering** 部分。
4. 切换 **Enable custom output rendering**。
5. 在 **URL** 字段中输入网页 URL。
6. 点击 **Save**。

### 对于数据集

要为数据集配置自定义输出渲染：

![显示 Custom Output Rendering 选项的数据集页面三点菜单](/langsmith/images/custom-output-rendering-menu.png)

1. 在 **Datasets & Experiments** 页面中导航到您的数据集。
2. 点击右上角的 **⋮**（三点菜单）。
3. 选择 **Custom Output Rendering**。
4. 切换 **Enable custom output rendering**。
5. 在 **URL** 字段中输入网页 URL。
6. 点击 **Save**。

![填写了字段的自定义输出渲染模态框](/langsmith/images/custom-output-rendering-modal.png)

### 对于标注队列

要为标注队列配置自定义输出渲染：

![显示自定义输出渲染配置的标注队列设置](/langsmith/images/annotation-queue-custom-output-rendering-settings.png)

1. 导航到 **Annotation Queues** 页面。
2. 点击一个现有的标注队列或创建一个新的。
3. 在标注队列设置窗格中，向下滚动到 **Custom Output Rendering** 部分。
4. 切换 **Enable custom output rendering**。
5. 在 **URL** 字段中输入网页 URL。
6. 点击 **Save** 或 **Create**。

<Info>当在多个级别应用自定义渲染设置时，优先级如下：标注队列 > 数据集 > 追踪项目。</Info>

## 构建自定义渲染器

### 了解消息格式

您的 HTML 页面将通过 [postMessage API](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) 接收输出数据。LangSmith 发送具有以下结构的消息：

```typescript
{
  type: "output" | "reference",
  data: {
    // 输出（实际输出或参考输出）
    // 结构因您的应用程序而异
  },
  metadata: {
    inputs: {
      // 生成此输出的输入
      // 结构因您的应用程序而异
    }
  }
}
```

-   `type`：指示这是实际输出 (`"output"`) 还是参考输出 (`"reference"`)。
-   `data`：输出数据本身。
-   `metadata.inputs`：生成此输出的输入数据，作为上下文提供。

<Note>**消息传递时间**：LangSmith 使用指数退避重试机制来确保您的页面即使加载缓慢也能接收数据。消息最多发送 6 次，延迟递增（100ms、200ms、400ms、800ms、1600ms、3200ms）。</Note>

### 示例实现

此示例监听传入的 postMessage 事件并在页面上显示它们。每条消息都有编号并格式化为 JSON，便于检查 LangSmith 发送到渲染器的数据结构。

```html
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8" />
        <title>PostMessage Echo</title>
        <link rel="stylesheet" href="https://unpkg.com/sakura.css/css/sakura.css" />
    </head>
    <body>
        <h1>PostMessage Messages</h1>
        <div id="messages"></div>
        <script>
            let count = 0;
            window.addEventListener("message", (event) => {
                count++;
                const header = document.createElement("h3");
                header.appendChild(document.createTextNode(`Message ${count}`));
                const code = document.createElement("code");
                code.appendChild(document.createTextNode(JSON.stringify(event.data, null, 2)));
                const pre = document.createElement("pre");
                pre.appendChild(code);
                document.getElementById("messages").appendChild(header);
                document.getElementById("messages").appendChild(pre);
            });
        </script>
    </body>
</html>
```

## 自定义渲染出现的位置

启用后，您的自定义渲染将替换以下位置的默认输出视图：

-   **实验比较视图**：在跨多个实验比较输出时：

![显示自定义渲染的实验比较视图](/langsmith/images/custom-output-rendering-experiment-comparison.png)

-   **运行详情窗格**：在查看与数据集关联的运行时：

![显示自定义渲染的运行详情窗格](/langsmith/images/custom-output-rendering-run-details.png)

-   **标注队列**：在标注队列中审查运行时：

![显示自定义渲染的标注队列](/langsmith/images/custom-output-rendering-annotation-queue.png)