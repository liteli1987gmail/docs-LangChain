---
title: 追踪 PydanticAI 应用程序
sidebarTitle: PydanticAI
---

LangSmith 可以使用其内置的 OpenTelemetry 检测来捕获 PydanticAI 生成的追踪。本指南向您展示如何自动捕获 PydanticAI 智能体中的追踪，并将其发送到 LangSmith 进行监控和分析。
...
<Note>
您不需要设置任何 OpenTelemetry 环境变量或手动配置导出器——`configure()` 自动处理一切。
</Note>

### 3. 创建并运行您的 PydanticAI 智能体

配置完成后，您的 PydanticAI 智能体将自动向 LangSmith 发送追踪：
...
您可以使用 OpenTelemetry 跨度属性向追踪添加自定义元数据：
