...
有些用户喜欢在 LangSmith 外部管理他们的数据集和运行实验，但希望使用 LangSmith UI 来查看结果。这通过我们的端点得到支持。


本指南将展示如何使用 REST API 上传评估，以 Python 中的 `requests` 库为例。但是，相同的原则适用于任何语言。

## 请求体模式

上传实验需要为您指定实验和数据集的相关高级信息，以及实验中示例和运行的个人数据。`results` 中的每个对象代表实验中的一"行"——一个单独的数据集示例以及相关的运行。注意，`dataset_id` 和 `dataset_name` 指的是您外部系统中的数据集标识符，用于将外部实验分组到单个数据集中。它们不应指代 LangSmith 中的现有数据集（除非该数据集是通过此端点创建的）。
...
响应 JSON 将是一个包含 `experiment` 和 `dataset` 键的字典，每个都是一个包含所创建实验和数据集相关信息的对象。

## 注意事项

您可以通过在多个调用之间提供相同的 dataset\_id 或 dataset\_name 来将多个实验上传到同一数据集。您的实验将被分组在单个数据集下，您将能够[使用比较视图比较实验结果](/langsmith/compare-experiment-results)。

确保您的个人行的开始和结束时间都在实验的开始和结束时间之间。

您必须提供 dataset\_id 或 dataset\_name 之一。如果您仅提供 ID 且数据集尚不存在，我们会为您生成一个名称，反之亦然如果您仅提供名称。

您不能将实验上传到不是通过此端点创建的数据集。上传实验仅支持外部管理的数据集。

## 示例请求

下面是到 `/datasets/upload-experiment` 的简单调用示例。这是一个基本示例，仅使用最重要的字段作为说明。
...
请注意，实验结果中的延迟和反馈统计为 null，因为运行还没有机会被持久化，这可能需要几秒钟。如果您保存了实验 ID 并在几秒钟后再次查询，您将看到所有统计信息（尽管令牌/成本仍将为 null，因为我们没有在请求正文中询问此信息）。

## 在 UI 中查看实验

现在，登录 UI 并点击您新创建的数据集！您应该会看到一个单独的实验：![已上传实验表](/langsmith/images/uploaded-dataset.png)

您的示例将被上传：![已上传示例](/langsmith/images/uploaded-dataset-examples.png)

点击您的实验将带您进入比较视图：![已上传实验比较视图](/langsmith/images/uploaded-experiment.png)

随着您将更多实验上传到您的数据集，您将能够比较结果并在比较视图中轻松识别回归。