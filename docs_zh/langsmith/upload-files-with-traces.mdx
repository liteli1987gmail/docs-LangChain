---
title: 使用追踪上传文件
sidebarTitle: 使用追踪上传文件
---

当您使用 [`@traceable` 装饰器或 `traceable` 包装器](/langsmith/annotate-code#use-%40traceable-%2F-traceable) 进行追踪时，LangSmith 支持将二进制文件（如图像、音频、视频、PDF 和 CSV）与您的追踪一起上传。这在处理使用多模态输入或输出的 LLM 管道时特别有用。

在 [Python](#python) 和 [TypeScript](#typescript) SDK 中，您可以通过指定每个文件的 MIME 类型和二进制内容来向追踪添加附件。本页说明如何在 Python 中使用 `Attachment` 类型以及在 TypeScript 中使用 `Uint8Array` / `ArrayBuffer` 来定义和追踪附件。
...
您还可以使用形式为 `(mime_type, data)` 的元组来方便地定义附件。

有两种提供文件数据的方法：

- 自己加载字节并直接传递它们（在所有环境中都有效），或
- 传递一个 `Path` 对象并通过在您的 `@traceable` 装饰器上设置 `dangerously_allow_filesystem=True` 让 SDK 读取文件。

    <Note>
    `dangerously_allow_filesystem` 标志作为防护措施存在，用于服务器和多租户环境，在这些环境中用户控制的输入可能影响文件路径。在受信任的环境中（本地脚本或您拥有文件路径的受控管道），可以安全地启用。
    </Note>

使用 `@traceable` 装饰函数，并将您的 `Attachment` 实例作为参数包含。以下示例演示了两种方法：手动将文件字节加载到 `Attachment` 中，以及传递带有 `dangerously_allow_filesystem=True` 的 `Path` 对象：
...
在 [TypeScript SDK](/langsmith/smith-js-ts-sdk) 中，您可以使用 `Uint8Array` 或 `ArrayBuffer` 作为数据类型向追踪添加附件。每个附件的 MIME 类型在 `extractAttachments` 中指定：

- `Uint8Array`：用于直接处理二进制数据。
- `ArrayBuffer`：表示固定长度的二进制数据，您可以根据需要将其转换为 `Uint8Array`。

在 TypeScript SDK 中，`extractAttachments` 是 `traceable` 配置中的可选参数。当调用被 traceable 包装的函数时，它会从您的输入中提取二进制数据（例如图像、音频文件）并与其他追踪数据一起记录它们，指定其 MIME 类型。

<Note>
您不能直接在 TypeScript SDK 中传递文件路径，因为在所有运行时环境中都不支持访问本地文件。
</Note>

使用 `traceable` 包装您的函数，并在 `extractAttachments` 选项中包含您的附件。签名是：
...
- [设置 LLM 即评判在线评估器](/langsmith/online-evaluations-llm-as-judge)