...
在本教程中，我们将构建一个客户支持机器人，帮助用户浏览数字音乐商店。然后，我们将介绍三种最有效的评估聊天机器人的方法：

* **[最终响应](#final-response-evaluator)**：评估代理的最终响应。
* **[轨迹](#trajectory-evaluator)**：评估代理是否采取了预期路径（例如，工具调用）来达到最终答案。
* **[单步](#single-step-evaluators)**：单独评估任何代理步骤（例如，对于给定的步骤，它是否选择了适当的第一工具）。

我们将使用 [LangGraph](https://github.com/langchain-ai/langgraph) 构建我们的代理，但这里展示的技术和 LangSmith 功能与框架无关。
...
我们将创建一个 [LangGraph](https://langchain-ai.github.io/langgraph/) 代理，对我们的数据库进行有限访问。对于演示目的，我们的代理将支持两种基本类型的请求：

* 查找：客户可以基于其他标识信息查找歌曲标题、艺术家姓名和专辑。例如："你们有哪些 Jimi Hendrix 的歌曲？"
* 退款：客户可以对他们过去的购买申请退款。例如："我叫 Claude Shannon，我想对我上周购买的一笔订单退款，您能帮我吗？"

为简单起见，在本演示中，我们将通过删除相应的数据库记录来实现退款。我们将跳过实现用户认证和其他生产安全措施。

代理的逻辑将结构化为两个独立的子图（一个用于查找，一个用于退款），以及一个将请求路由到适当子图的父图。
...
为使测试更容易，我们将为这些函数添加一个"模拟"模式。当启用模拟模式时，这些函数将模拟数据库操作，而不实际修改任何数据。
...
现在让我们定义我们的图。我们将使用一个简单的架构，包含三条主要路径：

1. 从对话中提取客户和购买信息

2. 将请求路由到三条路径之一：

   * 退款路径：如果我们有足够的购买详细信息（Invoice ID 或 Invoice Line ID）来处理退款
   * 查找路径：如果我们有足够的客户信息（姓名和电话）来搜索他们的购买历史
   * 响应路径：如果我们需要更多信息，向用户响应，请求提供所需的具体详细信息
...
对于查找（即问答）代理，我们将使用一个简单的 ReACT 架构，并给予代理工具来根据各种过滤器查找歌曲名称、艺术家名称和专辑名称。例如，您可以查找特定艺术家发行的专辑、按特定名称发布的歌曲的艺术家等。
...
现在让我们定义一个组合两个任务特定代理的父代理。父代理唯一的工作是通过分类用户的当前意图来路由到其中一个子代理，并将输出编译成后续消息。
...
现在我们有了可测试版本的代理，让我们运行一些评估。代理评估可以关注至少三个方面：

* [最终响应](#final-response-evaluator)：输入是提示词和可选的工具列表。输出是最终的代理响应。
* [轨迹](#trajectory-evaluator)：与之前一样，输入是提示词和可选的工具列表。输出是工具调用列表。
* [单步](#single-step-evaluators)：与之前一样，输入是提示词和可选的工具列表。输出是工具调用。

让我们运行每种类型的评估：

### 最终响应评估器

首先，让我们创建一个[数据集](/langsmith/evaluation-concepts#datasets)来评估代理的端到端性能。为简单起见，我们将使用相同的数据集进行最终响应和轨迹评估，因此我们将为每个示例问题添加地面真实响应和轨迹。我们将在下一节介绍轨迹。
...
我们将创建一个自定义的 [LLM-as-judge](/langsmith/evaluation-concepts#llm-as-judge) 评估器，使用另一个模型将我们的代理在每个示例上的输出与参考响应进行比较，并判断它们是否等价：
...
现在我们可以运行我们的评估了。我们的评估器假设我们的目标函数返回一个'response'键，所以我们需要定义一个这样的目标函数。

还要记住，在我们的退款图中我们使退款节点可配置，这样如果我们指定 `config={"env": "test"}`，我们将在不实际更新数据库的情况下模拟退款。当调用我们的图时，我们将在目标 `run_graph` 方法中使用这个可配置变量：
...
您可以在这里查看这些结果的样子：[LangSmith link](https://smith.langchain.com/public/708d08f4-300e-4c75-9677-c6b71b0d28c9/d)。

### 轨迹评估器

随着代理变得越来越复杂，它们有更多的潜在失败点。与其使用简单的通过/失败评估，不如使用能够给予部分信用的评估通常更好，当代理采取一些正确的步骤，即使它没有达到正确的最终答案。

这就是轨迹评估的用武之地。轨迹评估：

1. 将代理采取的实际步骤序列与预期序列进行比较
2. 根据正确完成的预期步骤数量计算分数

对于这个示例，我们的端到端数据集包含我们期望代理采取的有序步骤列表。让我们创建一个评估器，检查代理的实际轨迹与这些预期步骤，并计算完成的比例：
...
现在我们可以运行我们的评估了。我们的评估器假设我们的目标函数返回一个'trajectory'键，所以我们需要定义一个这样的目标函数。我们需要使用 [LangGraph 的流式处理能力](https://langchain-ai.github.io/langgra/langsmith/observability-concepts/streaming/) 来记录轨迹。

请注意，我们正在重用与最终响应评估相同的数据集，因此我们可以一起运行两个评估器并定义一个返回"response"和"trajectory"的目标函数。在实践中，为每种类型的评估使用单独的数据集通常很有用，这就是为什么我们在这里单独展示它们的原因：
...
虽然端到端测试为您提供关于代理性能的最大信号，但为了调试和迭代您的代理，能够精确定位困难的特定步骤并直接评估它们通常很有帮助。

在我们的例子中，我们代理的一个关键部分是它能够将用户的意图正确路由到"退款"路径或"问答"路径。让我们创建一个数据集并运行一些评估来直接压力测试这一个组件。
...
您可以在这里查看这些结果的样子：[LangSmith link](https://smith.langchain.com/public/f133dae2-8a88-43a0-9bfd-ab45bfa3920b/d)。
