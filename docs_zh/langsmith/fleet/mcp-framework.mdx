...
- [创建自定义工具](#create-a-custom-toolkit)，并与 LangSmith 的[代理身份验证](/langsmith/agent-auth)集成以进行 OAuth 身份验证
- [构建 MCP 网关](#use-as-an-mcp-gateway)，用于您自己构建的代理（在 Fleet 之外）
...
使用 @tool 装饰器定义您的工具。有关工具模式、返回值、错误处理和 ToolRuntime 的更多信息，请参阅[工具指南](/oss/langchain/tools)。
...
LangSmith 工具服务器可以作为 MCP 网关运行，将多个 MCP 服务器的工具聚合到单个端点。在您的 toolkit.toml 中配置 MCP 服务器：
...
所有连接的 MCP 服务器的工具都通过您服务器的 /mcp 端点暴露。MCP 工具以服务器名称为前缀以避免冲突（例如，weather_get_forecast、math_add）。

## 身份验证

### 第三方 API 的 OAuth

对于需要访问第三方 API（如 Google、GitHub、Slack 等）的工具，您可以将 OAuth 身份验证与[代理身份验证](/langsmith/agent-auth)一起使用。

在您的工具中使用 OAuth 之前，您需要在 LangSmith 工作区设置中配置 OAuth 提供商。请参阅[代理身份验证文档](/langsmith/agent-auth)获取设置说明。
...
- 指定至少一个 scope
- 使用 context.token 进行已身份验证的 API 调用
...
处理程序在每个请求上运行，必须返回一个带有 identity（以及可选的 permissions）的字典。
