...
title: 代理服务器的分布式追踪
sidebarTitle: 分布式追踪
description: 使用 RemoteGraph 或 SDK 从另一个服务调用已部署的代理服务器时统一追踪。
---

当您从另一个服务调用已部署的 [Agent Server](/langsmith/agent-server) 时，您可以传播追踪上下文，以便整个请求在 LangSmith 中显示为单个统一追踪。这使用 LangSmith 的[分布式追踪](/langsmith/distributed-tracing)功能，该功能通过 HTTP 头传播上下文。
...
2. **服务器**读取头并将其添加到运行的配置和元数据中，作为 `langsmith-trace` 和 `langsmith-project` 可配置值。您可以选择使用这些来在为您的代理运行给定运行时设置追踪上下文。

使用的头是：
- `langsmith-trace`：包含追踪的点分顺序。
- `baggage`：指定 LangSmith 项目和其他可选标签和元数据。

要选择加入分布式追踪，客户端和服务器都需要选择加入。

## 配置服务器

要接受分布式追踪上下文，您的图必须从配置中读取追踪头并设置追踪上下文。头通过 `configurable` 字段传递，作为 `langsmith-trace` 和 `langsmith-project`。
