...
本教程将向您展示如何使用 LangSmith 与流行测试工具（Pytest、Vitest 和 Jest）的集成来评估您的大型语言模型应用程序。我们将创建一个能够回答公开交易股票问题的 ReAct 智能体，并为其编写一套全面的测试用例。

## 设置

本教程使用 [LangGraph](https://langchain-ai.github.io/langgraph/tutorials/introduction/) 进行智能体编排、[OpenAI 的 GPT-4o](https://platform.openai.com/docs/models#gpt-4o) 进行搜索、[Tavily](https://tavily.com/) 进行搜索、[E2B](https://e2b.dev/) 的代码解释器以及 [Polygon](https://polygon.io/stocks) 来检索股票数据，但只需稍作修改即可适配其他框架、模型和工具。Tavily、E2B 和 Polygon 均可免费注册。
...
为了定义我们的 ReAct 智能体，我们将使用 LangGraph/LangGraph.js 进行编排，使用 LangChain 处理大型语言模型和工具。

### 定义工具

首先，我们定义将在智能体中使用的工具。将有 3 个工具：
...
现在我们已经定义了智能体，让我们编写一些测试以确保基本功能。本教程中，我们将测试智能体的工具调用能力是否正常工作，智能体是否知道忽略无关问题，以及它是否能够回答涉及使用所有工具的复杂问题。
...
第一个测试将是一个简单检查，确认智能体不会对无关查询使用工具。
...
某些工具调用比其他工具调用更容易测试。通过股票代码查找，我们可以断言正确的股票代码被搜索到。对于编码工具，工具的输入和输出的约束要少得多，并且有多种方法可以获得正确答案。在这种情况下，通过运行完整的智能体并断言它既调用了编码工具又得到了正确答案来测试工具是否被正确使用会更简单。
...
我们将通过运行大型语言模型作为评判者的评估来确保智能体的答案是基于搜索结果的。为了将大型语言模型作为评判者的调用与我们的智能体分开追踪，我们将在 Python 中使用 LangSmith 提供的 `trace_feedback` 上下文管理器，在 JS/TS 中使用 `wrapEvaluator` 函数。
...
一旦您设置好了配置文件（如果您使用的是 Vitest 或 Jest），可以使用以下命令运行测试：
...
请记得也将 Vitest 和 Jest 的配置文件添加到您的项目中。
