---
title: 追踪 LangGraph 应用程序
sidebarTitle: LangGraph
---

LangSmith 与 LangGraph（Python 和 JS）顺利集成，帮助您追踪智能体，无论您使用的是 LangChain 模块还是其他 SDK。

## 使用 LangChain

如果您在 LangGraph 中使用 LangChain 模块，您只需要设置几个环境变量即可启用追踪。

本指南将引导您完成一个基本示例。有关配置的更多详细信息，请参阅[使用 LangChain 追踪](/langsmith/trace-with-langchain)指南。
...
设置好环境后，您可以像平常一样调用 LangChain runnable。LangSmith 将推断正确的追踪配置：
...
点击追踪，然后在右上角切换到**详情**视图。您的 LangSmith 中的追踪应该[像这样](https://smith.langchain.com/public/79061a0f-c602-4012-b022-03fd46bce89e/r)。
...
如果您在 LangGraph 中使用其他 SDK 或自定义函数，您需要[适当地包装或装饰它们](/langsmith/annotate-code#use-%40traceable-%2F-traceable)（在 Python 中使用 `@traceable` 装饰器或在 JS 中使用 `traceable` 函数，或使用例如 `wrap_openai` 之类的 SDK）。如果这样做，LangSmith 将自动嵌套来自这些包装方法的追踪。

这是一个示例。您也可以查看此页面以获取更多信息。
